...
- CLIProxyAPI context overflow promotion is available only for OpenAI Chat Completions and Responses, only before downstream commit, and only once to a model's explicit `context_overflow_promotion_target_id`. Chat streaming requests can promote before dispatch when `stream=true`, tokenizer-backed estimation shows the source target is over its usable window, and the promoted target fits; that path opens zero source upstream requests and records `trigger_phase=pre_dispatch_estimate`. Responses streaming requests can replay only when `stream=true` and the source returns a pre-stream JSON provider-overflow body before any client-visible bytes; normal SSE is never replayed. Existing non-stream provider-overflow replay remains available and records `trigger_phase=provider_overflow`.
