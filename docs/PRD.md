...
- CLIProxyAPI context overflow promotion is available only for OpenAI Chat Completions and Responses, only before client-visible source bytes, and only once to a model's explicit `context_overflow_promotion_target_id`. Eligible Chat Completions and Responses requests, streaming or non-stream, can promote before dispatch when tokenizer-backed estimation proves the source target is over its usable window and the promoted target fits; that path opens zero source upstream requests and records `trigger_phase=pre_dispatch_estimate`. Provider-overflow replay is fallback-only for unavailable estimation, missing context-window metadata, provider-confirmed overflow despite local fit, and bounded pre-visible Responses streaming replay. Responses streaming fallback can replay when `stream=true` and either the source returns a pre-stream JSON provider-overflow body before downstream commit, or a bounded pre-visible SSE prelude made only of `response.created` and `response.in_progress` is followed by a code-classified overflow `error`. Responses SSE replay is disabled by non-overflow errors, unknown prelude events, semantic output such as `response.output_text.delta`, prelude caps, timeout, or any client-visible source byte. Non-stream provider-overflow fallback remains available and records `trigger_phase=provider_overflow`.
- The known upstream for context overflow promotion is CLIProxyAPI, not official OpenAI, so provider-overflow paths require body-confirmed overflow evidence; plain `429` never promotes. Native paths can use OpenAI-style or unambiguous flat gateway JSON bodies, while translated paths reject flat gateway JSON for promotion in v1. Exact facades keep their selected child restriction and never reopen siblings. Promotion never runs after headers, flush, SSE, or any client-visible bytes commit downstream output.
...
- Failed source attempts from context overflow promotion are logged as attempt-level observability, while final usage, status, and pricing ownership come from the final returned response. Pre-dispatch promotion has no failed source upstream attempt because Prism does not open the source upstream. Provider-overflow fallback logs the source overflow attempt plus the promoted final attempt. Request-log detail preserves the requested public model identity, from/to target identities, attempt counts, estimation fields when present or an unavailable-estimation reason when absent, and `trigger_phase`.
...
- Request logs remain one row per upstream attempt; `ingress_request_id` groups all attempts from one incoming runtime request. Context overflow promotion detail records `trigger_phase=pre_dispatch_estimate` for eligible Chat Completions and Responses pre-dispatch promotion, streaming or non-stream, and `trigger_phase=provider_overflow` for provider-overflow fallback replay.
