...
- CLIProxyAPI context overflow promotion is available only for OpenAI Chat Completions and Responses, only before client-visible source bytes, and only once to a model's explicit `context_overflow_promotion_target_id`. Chat streaming requests can promote before dispatch when `stream=true`, tokenizer-backed estimation shows the source target is over its usable window, and the promoted target fits; that path opens zero source upstream requests and records `trigger_phase=pre_dispatch_estimate`. Responses streaming requests can replay when `stream=true` and either the source returns a pre-stream JSON provider-overflow body before downstream commit, or a bounded pre-visible SSE prelude made only of `response.created` and `response.in_progress` is followed by a code-classified overflow `error`. Responses SSE replay is disabled by non-overflow errors, unknown prelude events, semantic output such as `response.output_text.delta`, prelude caps, timeout, or any client-visible source byte. Existing non-stream provider-overflow replay remains available and records `trigger_phase=provider_overflow`.
...
- Failed source attempts from context overflow promotion are logged as attempt-level observability, while final usage, status, and pricing ownership come from the final returned response. Chat pre-dispatch promotion has no failed source upstream attempt because Prism does not open the source upstream. Responses streaming replay logs the source overflow attempt plus the promoted final attempt. Request-log detail preserves the requested public model identity, from/to target identities, attempt counts, estimate fields when present, and `trigger_phase`.
