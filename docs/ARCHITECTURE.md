...
- The backend image runs as `prism:prism`, UID/GID `1000:1000`. Container deployments that bind mount `/etc/prism` or any other `PRISM_CONFIG_PATH` parent must make that host directory writable by UID/GID `1000:1000`; new and existing root-owned mounts should be prepared once with `sudo chown -R 1000:1000 <prism-config-dir>` and `sudo chmod 0700 <prism-config-dir>`.
...
Frontend /sidecars
  ...
  -> Prism persists snapshots, latest observed quota state, pending repairs, scan runs, and retained action history in sidecar_* tables
...
The sidecar watchdog is a sweep engine with probe-first quota authority. It does not use snapshot `quota_*` fields as the source of truth for recovery or quota holds. A sweep pins the active policy revision, freezes an eligible auth-file snapshot, sends bounded CLIProxyAPI `/api-call` quota probes, records sanitized observations, updates latest observed quota state, and then restores, deprioritizes, or extends holds from those probe outcomes. Public policy fields include `watchdog_sweep_interval_seconds`, `using_priority`, `quota_exceeded_priority`, `working_priority`, `empty_quota_priority`, `initial_priority`, `error_priority`, `probe_concurrency`, `probe_timeout_seconds`, `probe_batch_cooldown_seconds`, `probe_jitter_min_ms`, `probe_jitter_max_ms`, `cooldown_jitter_percent`, `quota_inventory_enabled`, `initial_scan_enabled`, `rolling_refresh_enabled`, and `rolling_refresh_after_seconds`. The frontend labels `probe_concurrency` as `Probe batch size`; it limits how many auth files are probed in one sweep batch. `probe_batch_cooldown_seconds` paces later batches inside the same active sweep, while `watchdog_sweep_interval_seconds` delays only the next sweep after the current sweep completes. Policy `PUT` and `PATCH` calls create pending immutable revisions; `POST /api/sidecars/{sidecar_id}/watchdog-policy/apply` explicitly promotes a pending revision to active, and an active sweep stays pinned to the revision it started with. In v1, provider-specific quota probes are supported only for `codex` and `chatgpt` and use the ChatGPT usage endpoint through the sidecar wrapper.

Quota inventory is split by purpose. `sidecar_auth_quota_states` is the public latest observed quota view for each auth file, including `quota_band`, `priority_state`, `probe_status`, `reason_code`, `quota_reset_at`, `blocking_window`, `last_probed_at`, safe `last_error_code`, whether the auth index is present, and whether a watchdog hold is active. The only public quota bands are `using`, `quota_exceeded`, and `error`; `priority_state` is separate watchdog priority meaning with four values: `working`, `empty-quota`, `initial`, and `error`. `sidecar_quota_scan_runs` tracks asynchronous initial, manual, and scheduled scans with `using_count`, `quota_exceeded_count`, `error_count`, `skipped_count`, and cancellation state. Auto initial scans and rolling refresh remain part of the contract, but scan cadence is independent from the watchdog sweep interval and intra-sweep batch cooldown. Scan routes expose progress and latest state through `/api/sidecars/{sidecar_id}/quota-scans`, `/api/sidecars/{sidecar_id}/quota-scans/current`, `/api/sidecars/{sidecar_id}/quota-scans/{scan_id}/cancel`, and `/api/sidecars/{sidecar_id}/quota-states`; internal scan position is not part of public responses.
