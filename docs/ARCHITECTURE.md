...
3. OpenAI Chat Completions and Responses requests that use `cheapest_eligible_context` run local preflight context estimation before terminal-target choice. The deterministic methods are `openai_chat_tokenizer_v1` and `openai_responses_tokenizer_v1`. When estimation is unavailable for these two operations, Prism now passes the request through the normal resolved target path instead of returning local `400 context_estimation_unavailable`; hard no-fit `413 context_window_exceeded` still requires a successful estimate.
...
6. Release 1 exact OpenAI facade routing is a backend-first planner specialization on that same exact requested-model lookup. It activates only when the requested OpenAI model is `facade_enabled = true`, `facade_selection_policy = "ordered_eligible_context"`, and `facade_fallback_policy = "skip_ineligible_targets"`, evaluates same-family model targets only, skips ineligible children, follows flat target `position` and stable IDs, and returns one selected child plan.
...
If all eligible candidates are unavailable inside the current retry window, the gateway returns `503` with routing-availability detail. If context fit is evaluated and no terminal target fits, the gateway returns HTTP `413` before provider transport with `error="context_window_exceeded"` and context-routing detail for skipped terminal targets. Exact facade routing adds no regex matching, capability-metadata expansion, frontend facade authoring, hidden weight or tier semantics, sibling-target provider-failure failover after child selection, or response-body model rewriting. Request logs and usage events keep the existing requested-model and resolved-target fields, while `context_routing.facade_selection` and matching `prism.runtime.facade_*` trace attributes carry the additive facade metadata.

Context overflow promotion is a CLIProxyAPI-specific routing path, not official OpenAI behavior and not a generic failover rule. The known upstream for this path is CLIProxyAPI, which can return different native and translated OpenAI-compatible error envelopes. Prism supports narrow promotion paths for `openai.chat_completions` and `openai.responses`, only through explicit `context_overflow_promotion_target_id` links and only before client-visible source bytes.

Ordinary routing remains flat where compatible. The requested model first resolves its normal ordered access-target pool with the selected load-balance strategy, model-target child expansion, runtime-state filtering, context-fit filtering, and exact facade rules. Context-overflow pre-dispatch planning is the separate path: if the current model's normal route has no fitting terminal target and local estimation is available, Prism may follow that model's explicit promotion target as the next planning frame. Each frame repeats ordinary routing for the new current model and stops as soon as a fitting terminal plan exists. Prism never infers a target from a larger window, cost, label, endpoint, or facade sibling.

Recursive pre-dispatch planning is bounded and local. The runtime owns a fixed limit of `3` promotion transitions, tracks visited model IDs, makes no provider calls, and records explicit stop reasons. `estimation_unavailable` and `missing_context_window` stop recursion and keep the safe normal route eligible for later provider-overflow fallback. `cycle` and `max_depth` reject invalid explicit chains. Missing explicit promotion targets also stop planning without guessing. Fit decisions are based on resolved Terminal Target usable-window metadata, not on model labels or desired context values.

Provider-overflow promotion remains a one-shot fallback for cases Prism could not prove pre-dispatch promotion: unavailable estimation, missing selected context-window metadata, provider-confirmed misses where local estimation said the source fit, and bounded pre-visible Responses streaming replay. It does not recurse through a promotion chain. If recursive pre-dispatch planning already selected the final promoted model, a later provider overflow from that selected model is final. Responses streaming provider-overflow fallback can use a pre-stream JSON overflow body before downstream commit, or a bounded SSE prelude made only of `response.created` and `response.in_progress`, capped at `16 KiB`, `2` events, and `250 ms`, when the next staged event is a code-classified overflow `error`. Non-overflow errors, unknown prelude events, semantic output such as `response.output_text.delta`, cap expiry, timeout, or any client-visible source byte disable replay and continue the source stream unchanged.

Promotion classification for provider-overflow paths is body-aware. Plain `429` never promotes; only body-confirmed overflow `429` can promote. Native non-stream paths may accept an OpenAI-style top-level `error` object or unambiguous flat CLIProxyAPI gateway JSON, while translated non-stream paths accept only top-level `error` objects and reject translated flat-gateway JSON for promotion in v1. Streaming Responses provider-overflow replay uses the same body-confirmed classifier before downstream commit or inside the bounded pre-visible SSE prelude. Pre-dispatch promotion is not provider-overflow classification; it is a tokenizer estimate decision over explicit recursive links.

The final selected model inherits the same planner behavior as ordinary terminal resolution: native-compatible attempts remain preferred, and adapter-approved OpenAI text sibling translations can run when the selected terminal target's `openai_text_capability` requires translation. Direct Terminal Targets on exact OpenAI facade models still reject during recursive promotion target evaluation, while normal requested facade planning remains unchanged and model-target-only. Replay paths reuse the original buffered ingress body exactly once and preserve `previous_response_id`, `store`, and other continuation fields without reconstructing hidden Responses state. Final status, usage, pricing, and finalized usage-event ownership come from the final response returned to the client. Failed source attempts remain visible as attempt-level request-log and optional audit rows; recursive pre-dispatch promotion has no source attempt row because no source upstream request is opened.
...
- Context-routing metadata when preflight routing ran, including policy, selected terminal target, selected endpoint, selected preferred-context band, `estimation_status`, estimator method and estimated token totals when present, `estimation_unavailable_reason` when unavailable, ranking method, skipped terminal-target reasons, and additive `context_overflow_promotion` detail when one-shot CLIProxyAPI overflow promotion ran. Promotion detail includes `trigger_phase` values `pre_dispatch_estimate` or `provider_overflow`, from/to resolved target IDs, from/to selected terminal target IDs, from/to usable context windows, source and final attempt counts, `estimation_status`, `estimation_method` and token totals when present, and `estimation_unavailable_reason` when unavailable.
...
Runtime traces keep `prism.runtime.operation_name` as ingress-led. Additive trace attributes include sanitized `prism.runtime.upstream_operation_name`, `prism.runtime.operation_translation_mode`, `prism.runtime.upstream_request_path`, `prism.runtime.preferred_context_band`, and `prism.runtime.selected_terminal_target_id`. When promotion metadata is attached, promotion trace attributes add `trigger_phase`, from/to target fields, usable-window fields, attempt counts, `estimation_status`, `estimation_unavailable_reason` when unavailable, and estimator method plus token totals when present. The upstream path attribute is normalized through the operation registry so path-bound model IDs do not leak into spans.
