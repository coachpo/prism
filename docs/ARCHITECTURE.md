...
Context overflow promotion is a CLIProxyAPI-specific replay path, not official OpenAI behavior and not a generic failover rule. The known upstream for this path is CLIProxyAPI, which can return different native and translated OpenAI-compatible error envelopes. Prism supports narrow promotion paths for `openai.chat_completions` and `openai.responses`, all before client-visible source bytes, only once, and only to the source model's explicit `context_overflow_promotion_target_id`.

Tokenizer-backed pre-dispatch promotion applies to eligible Chat Completions and Responses requests, both streaming and non-stream, when local estimation is present, source non-fit is proven against the selected source target's usable context window, the explicit promotion target fits the same estimate, and native or sibling OpenAI text translation compatibility is valid. The source plan is built but not executed, no source upstream request is opened, the request streaming mode is preserved, and the promoted response becomes the first client-visible response.

Provider-overflow promotion remains a narrow fallback for cases Prism could not prove pre-dispatch promotion: unavailable estimation, missing selected context-window metadata, provider-confirmed misses where local estimation said the source fit, and bounded pre-visible Responses streaming replay. Responses streaming provider-overflow fallback can use a pre-stream JSON overflow body before downstream commit, or a bounded SSE prelude made only of `response.created` and `response.in_progress`, capped at `16 KiB`, `2` events, and `250 ms`, when the next staged event is a code-classified overflow `error`. Non-overflow errors, unknown prelude events, semantic output such as `response.output_text.delta`, cap expiry, timeout, or any client-visible source byte disable replay and continue the source stream unchanged.

Promotion classification for provider-overflow paths is body-aware. Plain `429` never promotes; only body-confirmed overflow `429` can promote. Native non-stream paths may accept an OpenAI-style top-level `error` object or unambiguous flat CLIProxyAPI gateway JSON, while translated non-stream paths accept only top-level `error` objects and reject translated flat-gateway JSON for promotion in v1. Streaming Responses provider-overflow replay uses the same body-confirmed classifier before downstream commit or inside the bounded pre-visible SSE prelude. Pre-dispatch promotion is not provider-overflow classification; it is a tokenizer estimate decision.

The promoted model inherits the same planner behavior as ordinary terminal resolution: native-compatible attempts remain preferred, and adapter-approved OpenAI text sibling translations can run when the promoted target's `openai_text_capability` requires translation. Promotion remains a one-shot explicit replay path, not a promotion-only fallback exception. A promoted-target failure is final and is returned to the client. The selected-child exact-facade restriction is preserved: after a facade selects one child, promotion can start only from that selected child and cannot reopen sibling facade targets. Replay paths reuse the original buffered ingress body exactly once and preserve `previous_response_id`, `store`, and other continuation fields without reconstructing hidden Responses state. Final status, usage, pricing, and finalized usage-event ownership come from the final response returned to the client. Failed source attempts remain visible as attempt-level request-log and optional audit rows; pre-dispatch promotion has no source attempt row because no source upstream request is opened.
