...
Restart-required boundaries are structural process resources: listener host and port, docs route enablement, PostgreSQL URL and pool budgets, runtime secret encryption key, auth JWT signing key, and the state-transfer bundle key. Those values can be written through the bootstrap API, but they do not change the running process until Prism restarts.
