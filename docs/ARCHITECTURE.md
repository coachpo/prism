...
3. OpenAI Chat Completions and Responses requests that use `cheapest_eligible_context` run local preflight context estimation before terminal-target choice. The deterministic methods are `openai_chat_tokenizer_v1` and `openai_responses_heuristic_v1`. When estimation is unavailable for these two operations, Prism now passes the request through the normal resolved target path instead of returning local `400 context_estimation_unavailable`; hard no-fit `413 context_window_exceeded` still requires a successful estimate.
...
Context overflow promotion is a CLIProxyAPI-specific replay path, not official OpenAI behavior and not a generic failover rule. The known upstream for this path is CLIProxyAPI, which can return different native and translated OpenAI-compatible error envelopes. Prism supports three narrow promotion paths for `openai.chat_completions` and `openai.responses`, all before client-visible source bytes, only once, and only to the source model's explicit `context_overflow_promotion_target_id`.

Chat streaming pre-dispatch promotion applies only to `POST /v1/chat/completions` with `stream=true` when tokenizer-backed estimation is available, the source target is over its usable context window, and the promoted target fits the same estimate. The source plan is built but not executed, no source upstream request is opened, and the promoted stream becomes the first client-visible response. Responses streaming replay applies only to `POST /v1/responses` with `stream=true` before client-visible source bytes. It can use a pre-stream JSON provider-overflow body before downstream commit, or a bounded SSE prelude made only of `response.created` and `response.in_progress`, capped at `16 KiB`, `2` events, and `250 ms`, when the next staged event is a code-classified overflow `error`. Non-overflow errors, unknown prelude events, semantic output such as `response.output_text.delta`, cap expiry, timeout, or any client-visible source byte disable replay and continue the source stream unchanged. Existing non-stream provider-overflow replay for Chat Completions and Responses remains a separate path.

Promotion classification for provider-overflow paths is body-aware. Plain `429` never promotes; only body-confirmed overflow `429` can promote. Native non-stream paths may accept an OpenAI-style top-level `error` object or unambiguous flat CLIProxyAPI gateway JSON, while translated non-stream paths accept only top-level `error` objects and reject translated flat-gateway JSON for promotion in v1. Streaming Responses provider-overflow replay uses the same body-confirmed classifier before downstream commit or inside the bounded pre-visible SSE prelude. Chat streaming pre-dispatch promotion is not provider-overflow classification; it is a tokenizer estimate decision.

The promoted model inherits the same planner behavior as ordinary terminal resolution: native-compatible attempts remain preferred, and adapter-approved OpenAI text sibling translations can run when the promoted target's `openai_text_capability` requires translation. Promotion remains a one-shot explicit replay path, not a promotion-only fallback exception. A promoted-target failure is final and is returned to the client. The selected-child exact-facade restriction is preserved: after a facade selects one child, promotion can start only from that selected child and cannot reopen sibling facade targets. Replay paths reuse the original buffered ingress body exactly once and preserve `previous_response_id`, `store`, and other continuation fields without reconstructing hidden Responses state. Final status, usage, pricing, and finalized usage-event ownership come from the final response returned to the client. Failed source attempts remain visible as attempt-level request-log and optional audit rows; Chat pre-dispatch promotion has no source attempt row because no source upstream request is opened.
