...
The sidecar watchdog is probe-first for quota authority. It does not use snapshot `quota_*` fields as the source of truth for recovery or quota holds. Instead, bounded reconcile work selects auth files, honors `probe_batch_cooldown_seconds`, sends active CLIProxyAPI `/api-call` probes, records sanitized observations, updates the latest observed quota band, and then restores, deprioritizes, or extends holds from those probe outcomes. Public policy fields include `using_priority`, `quota_exceeded_priority`, `error_priority`, `probe_batch_size`, `probe_timeout_seconds`, `probe_batch_cooldown_seconds`, `probe_jitter_min_ms`, `probe_jitter_max_ms`, `cooldown_jitter_percent`, `quota_inventory_enabled`, `initial_scan_enabled`, `rolling_refresh_enabled`, and `rolling_refresh_after_seconds`. In v1, provider-specific quota probes are supported only for `codex` and `chatgpt` and use the ChatGPT usage endpoint through the sidecar wrapper.
