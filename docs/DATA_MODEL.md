...
- Context overflow promotion stores additive `context_routing.context_overflow_promotion` detail for Chat streaming pre-dispatch promotion, Responses streaming pre-commit provider-overflow replay, and existing non-stream provider-overflow replay. Metadata preserves the requested model identity plus from/to resolved target IDs and from/to selected terminal target IDs. It records `trigger_phase=pre_dispatch_estimate` for Chat streaming planning decisions and `trigger_phase=provider_overflow` for Responses streaming replay and non-stream replay, with resolved promoted-target metadata and estimate fields when present. Source overflow rows keep the source resolved model and terminal target, promoted rows keep the promoted resolved model and terminal target, and related rows stay grouped by the same `ingress_request_id`.
