...
- P2: OpenAI chat/responses conversion is implemented as an explicit bridge, not implicit provider fallback. In the current hard-cut model, planning chooses a translation mode from the OpenAI connection's `openai_text_capability`, not from health probe behavior. Evidence: `backend/internal/httpapi/runtime/coding_agent_format_bridge.go`; `backend/internal/httpapi/runtime/operation_translation.go`; `backend/internal/providercompat/providercompat.go`.
