...
- A streaming attempt that returns `2xx` but whose first SSE event is a provider error also fails over to the next candidate, as long as one remains. This covers upstreams that reject a request in-band instead of with an HTTP status, such as an unsupported parameter value. Nothing reaches the client, the attempt is logged as a stream error, and the target receives the same retry/ban feedback as a failover-triggering status. The last candidate's stream is passed through unchanged. See [the streaming runtime flow](architecture.md#33-runtime-request-streaming).
