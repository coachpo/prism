...
- Model detail renders process-local Ban Policy runtime state for each Terminal Target: `state` (available / retry_wait / banned), next-retry or ban expiry, last success time, `last_success_response_headers_latency_ms` (latest successful attempt latency from request start to upstream response headers; a single sample, not a percentile and not TTFT), and in-flight stream/non-stream counts. Missing runtime rows mean the process has not observed the connection yet. These are Ban Policy observations, not probe health, success-rate or availability proof.
...
- `/observe` provides three tabs sharing one URL time preset: Overview (Now strip with rolling 30-minute RPM/TPM, Window KPI grid with TTFT P50/P95 headline, request preview and main chart), Analytics (single main chart with metric/group switching and a semantic data table), and Events (loadbalance timeline). Observe fragments load independently and never synthesize zeros on failure.
...
- Model detail exposes read-only static routing diagnostics (`GET /api/models/{id}/routing-diagnostics` and a proposed-change preview) and the models list embeds a compact `routing_summary`; both share the backend analyzer with the runtime planner and never read Ban/retry/admission state.
