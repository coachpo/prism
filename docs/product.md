...
- Operation-registered proxy support for OpenAI Chat Completions, Responses/input-token/compact, image generations/edits, Anthropic Messages/count-token, and Gemini generate/stream/count-token operations
- Supports native streaming (SSE) and non-streaming generation responses; token-count operations and Responses compact remain non-streaming
...
- OpenAI models that accept text carry an `openai_accepted_format` of `responses_only`, `chat_completions_only`, or `dual_native`; strict mode equality requires every access target (model or Terminal Target) to use the identical mode, including matching `null` for image-only models
...
- For an image-only model, the Terminal Target dialog hides text capability controls and preserves the absent text mode when creating or editing, including a name-only edit. Prefilling from another Terminal Target preserves the destination model's text mode instead of importing the source's capability
...
- Terminal Targets own an explicit persisted `upstream_model_id` (migration `000031_terminal_target_upstream_model_identity.sql`): create-omitted writes the owner model's current `model_id` explicitly to the database, explicit null/blank/over-200-Unicode-character values reject `422`, PATCH omission preserves and the field can never be cleared, model rename never cascades, and batch copy preserves the source value byte-for-byte after the original value's create-time trim. Clients keep addressing the stable entry `model_id`; each upstream attempt sends the selected target's frozen upstream identity instead. The model detail form prefills the owner `model_id` with manual decoupling, composite create supports the field, the access-target display and copy dialog show it, and the Requests surface exposes it as an optional hidden-by-default column plus detail/CSV values that render a reasoned `—` for historical rows instead of inferring one
...
- `model_configs.direct_request_enabled` is the only persisted client-entry qualification. It is `BOOLEAN NOT NULL DEFAULT TRUE`; it is not an `entry_only`/`outbound_only`/`both` state. A false row remains a valid enabled Model Target and may be reached recursively by a direct parent. Direct runtime requests for that ID return the existing model-not-configured `404` before side effects; `/v1/models` and client export omit it, setup readiness does not count it as a root, and proxy-key self-tests do not offer it.
...
- For models with multiple reachable Terminal Targets:
  - **Automatic failover** when an attempt returns a failover-triggering status (`401`, `403`, `408`, `422`, `429`, `500`, `502`, `503`, `504`, `529` by default) or raises connection/timeout errors
  - **Request-scoped reroute** when an attempt returns one of the strategy's reroute status codes (`400` by default), which is how providers reject a request that one target cannot serve, such as an image sent to a text-only model. The same request moves to the next candidate. The rejecting target's retry window, retry counts, and ban state are untouched, so later requests still reach it in authored order. Reroute codes are 4xx only, never `429`, and never overlap the failure status codes; an empty set disables rerouting, and a strategy that already fails over on `400` keeps that behavior. When every candidate rejects, or no later candidate produces a response, the client receives the last rejecting upstream's original status and body rather than a gateway error. The attempt launched after a rejection records the `reroute` trigger, which the Requests chain shows apart from failover.
...
- A streaming attempt that returns `2xx` but whose first SSE event is a provider error is also rerouted to the next candidate, as long as one remains. This covers upstreams that reject a request in-band instead of with an HTTP status, such as an unsupported parameter value. Nothing reaches the client and the attempt is logged as a stream error. As with a reroute status, the target's retry window, retry counts, and ban state are untouched, and the next attempt records the `reroute` trigger. No error type is exempt, so a target that keeps answering with in-band errors, overload errors included, is still tried in authored order instead of cooling down. The last candidate's stream is passed through unchanged. See [the streaming runtime flow](architecture.md#33-runtime-request-streaming).
- Failover-worthy HTTP responses are governed by the attached strategy's configured failure status codes and retry-window settings
  - Client errors outside the configured failure status set (for example `404`, or a rerouted `400` with default policies) do not force-clear existing Ban Policy state
...
- Request-log materialization distinguishes actual upstream attempts from synthetic failures. Telemetry-eligible planning errors carrying `PlanningFailure` and terminal execution errors enter the scheduled activity handoff. All-transport `502` and the 64-launch safety bound preserve real launched attempts plus the finalized gateway result; zero-launch execution failures such as `admission_exhausted` produce a diagnostic row. Registry rejections, unknown/non-entry models, API-family mismatches, and malformed bodies rejected before an eligible planning/execution handoff do not create request history. Scheduled failure handoffs can be lost if their queue rejects or cannot drain; they do not have the synchronous success-response durability gate.
...
- The Models page renders plain telemetry text for each model: 24-hour success rate, P95 latency, and 24-hour request count, plus a 30-day spend value. Missing success data is shown as `- Success`; there are no colored success-rate thresholds or health badges. The models-list telemetry columns follow the page's three-scope switch (入口请求/最终承载/路由尝试), so every value states its attribution basis rather than implying one universal caliber.
...
- View model configurations and their reachable exits (the `/route/models` management surface for `model_configs`). The default view shows only direct entries; a URL-backed segmented selector switches to non-entry configurations (`仅模型目标`) or the full inventory. KPI/counts and search operate on the selected view. The list's 出口映射 column projects the direct `access_targets` rows in `(position, id)` order and shows the first two: Terminal Target rows as endpoint → persisted `upstream_model_id`, Model Target rows as the logical target id; rows beyond the second are a `还有 N 项，见详情` pointer to model detail, and the projection never follows Model Target rows recursively. Missing endpoint or upstream evidence renders a reasoned `—` and is never backfilled from the owning model configuration's `model_id`; disabled rows stay visible with a 未参与 state. The upstream identity is shown as-is, without a label saying whether it matches the client model name. Non-entry rows expose their incoming Model Target count and a warning when that count is zero.
- Add/edit/delete model configurations with ordered access targets. The form exposes the direct-entry switch; omission on create defaults to true and omission on update preserves the stored bit. The page's stats switch is a controlled single-select segmented control over the three named metric scopes (`ingress`/`final_execution`/`route_attempt`, URL-backed through `scope`) with a fixed attribution note per scope, and re-selecting the active scope never clears it. Connection filters (`flag`) cover `has_model_target` (at least one direct Model Target row), plus the retained `needs_target` and `single_truncated` states. Whether an upstream identity matches the client model name is not a filter; a retired `flag=upstream_decoupled` link falls back to the unfiltered list.
...
- `/observe` is one dashboard with four content-switcher values sharing the page time preset: Trend (`trend`), Errors (`errors`), Activity (`activity`), and Terminal Targets (`terminal_targets`). The Now strip and fixed-ingress Window KPI grid render once above that switcher; routing health is a separate `/observe/routing-health` route. Legacy `overview`/`analytics` links resolve to Trend, while legacy `events` links redirect to routing health. Observe fragments load independently and never synthesize zeros on failure. The fixed-ingress Window KPI grid has requests, final completion rate, TTFT P95, output rate, cache-read share, and known cost; final completion counts completed finalized ingress requests, including stream outcomes, rather than only HTTP status; cache-read share distinguishes genuine zero, no comparable rows, empty window, failed read, and partial coverage. Output rate is backend-authoritative measured evidence: only completed, progressive requests with trusted visible text/tool-output delivery evidence (at least two output events across at least 50ms) enter the per-request equal-weight mean, while GLM-style buffered bursts, non-streaming responses, Images, failures, and evidence-insufficient requests are unmeasured and never pollute the average. The Window KPI exposes the measured/unmeasurable/not_applicable/unknown census, series buckets expose measured sample/request counts with honest null gaps, and request details expose the persisted per-request state/reason; missing rates render as `—` and genuine zeros remain 0. Trend and Errors share a URL-backed scope and expose only that scope's authoritative metric/group catalog: ingress uses `requests|errors|ttft|output_rate|tokens|cache_read_share|cost` (default `requests`), final execution uses `requests|errors|final_attempt_latency|tokens|cache_read_share|cost` (default `requests`), and route attempt uses `attempts|errors|attempt_latency` (default `attempts`). URL and wire metrics use the same names with no implicit aliases. Output rate and cache-read share draw as lines from the same usage-series read, preserve absent buckets as gaps, and carry sample/coverage evidence. Invalid combinations fail with typed `422`; no alias silently substitutes a different metric. Every scoped fragment carries its own caliber, bounds, dataset coverage and sample/missing evidence, and the UI/export keep request counts distinct from attempt counts.
- Model creation supports one-step composite create: an optional `initial_terminal_target` (existing or inline endpoint) is created atomically with the model and enables the model by default. Non-OpenAI wire payloads omit OpenAI-only keys, while OpenAI text/image dimensions are validated independently and cross-mode capability values reject with `422 target_openai_mode_mismatch` and zero writes. Terminal Targets can be batch-copied to same-profile, same-family, same-mode destination models in one transaction only when text capability matches exactly (including `null`) and image capability contains the destination requirement; both enabled and disabled copies validate before any write, and copied targets default to not participating in routing.
- Model detail exposes read-only static routing diagnostics (`GET /api/models/{id}/routing-diagnostics`) and the models list embeds the authoritative compact `routing_summary`; both share one backend analyzer and the runtime-owned model-bound operation catalog across OpenAI, Anthropic, and Gemini. The analyzer is separate from the runtime planner, reads no Ban/retry/admission state, and applies the routing strategy to the same single mixed peer sequence, so `single` truncates that whole list rather than each target type. Route witnesses carry the real Endpoint identity, lowercase coverage, and a routing-schedule qualifier. Routing mutations refresh diagnostics and the authoritative model list, rather than preserving browser-recomputed routing or connection counts.
...
- Startup/bootstrap process settings are owned by the plaintext `config.json` bootstrap file; external edits require a Prism restart
...
- **Pricing read honesty**: list, setup-readiness, and currency-archive reads use the same base input/output readiness contract while the list separately reports missing specialty components. Broken kind/card/window/digest data is an error or explicit missing state, never an empty-history or zero-cost claim; edits show the existing impact preflight before save, and revision history preserves each version's role prices and concrete windows.
...
- **Cache-read share**: The Window KPI card and the per-request detail row report prompt tokens served from cache as `cache_read / (input + cache_read + cache_creation)`, computed once in the frontend over the disjoint components without re-deriving totals. Upstream ingestion already normalizes to disjoint semantics, so `input_tokens` never includes cache components; `cache_creation` is structurally absent for OpenAI/Gemini and coalesces to zero, while a null `input` or `cache_read` excludes the row (the two are not interchangeable). The window aggregate is computed backend-side in one SQL statement under a single `cache_basis_eligible` predicate that also requires a non-null `operation_name` outside `anthropic.count_tokens`, `gemini.count_tokens`, `openai.images.generations`, and `openai.images.edits` — token-count and image operations are excluded from this generation-comparison cohort, and a null operation name is indeterminate. Real zero, no comparable rows, an empty window, a failed read, and partial coverage are presented as mutually distinguishable states; none is shown as a fabricated `0%`.
...
- The `/observe` dashboard presents a rolling Now strip, fixed-ingress Window KPI cards, and the Trend, Errors, Activity, and Terminal Targets views described in §4.7 and §4.14. Its final-completion card uses finalized `completed_count / request_count`; HTTP success remains a separate backend statistic.
- Activity is a paginated finalized-ingress feed that links into the Requests chain view. Routing health has its own current-state and events page, linked from Observe and Ban Policies.
...
- Observe loads on entry and on relevant URL changes or explicit refresh/retry. It uses an ingress query context for Window KPIs and Activity, an independent selected-scope context for Trend and Errors, and a separate rolling Now read. Automatic refresh is off by default; the operator may choose 30 or 60 seconds for the mounted page. A hidden tab starts no scheduled read, slow reads prevent overlapping refresh cycles, and leaving the route or changing identity cancels reads and resets the interval. A refresh retains last-good data with an explicit stale marker on failure, while each fragment keeps its own sampling time and effective range. Recent Activity uses retained finalized ingress events, not an inferred all-time last-use value.
...
OpenAI image operations additionally replace inline and generated image bytes before audit storage, including each SSE event. Image audit bodies therefore preserve the redacted representation, not the original media payload; raw downloads still return the exact bytes that were stored.
...
- Deletion requests create durable management jobs; the Settings retention job center is a static browser-side snapshot of the global v2 queue — it lists once per scope/filter, extends by explicit load-more, refreshes on demand, and recalibrates serially with fresh cursors after a policy save, job creation, or cancellation (all-or-nothing commit). It opens durable checkpoint details and cancels only queued manual jobs. The same list/get/cancel routes remain available as API contracts.
...
- Every metric declares one scope with an authoritative ordered catalog owned by `backend/internal/domain/stats/scope.go`: `ingress` has `requests|errors|ttft|output_rate|tokens|cache_read_share|cost` (default `requests`); `final_execution` has `requests|errors|final_attempt_latency|tokens|cache_read_share|cost` (default `requests`); and `route_attempt` has `attempts|errors|attempt_latency` (default `attempts`). An omitted metric resolves to the scope default before validation; an explicit incompatible metric returns `422 metric_invalid` with no cross-scope alias. Ingress counts each finalized request once by its requested model, final execution uses the resolved leaf and actual winning Terminal Target, and route attempt counts each real upstream attempt by its attempt target and actual Terminal Target.
...
- Observe shares the URL-backed selected scope between Trend and Errors and keeps Activity explicitly ingress-scoped. An illegal or cross-scope URL metric is normalized with `replace` to that scope's default before the first series request, and the metric toggle shows only the current scope catalog. Query-context and series fragments are keyed by their complete preset/scope and token/metric/group/interval identities so stale ready data is withdrawn on a key change. Trend/Errors show scoped request-log coverage separately from the fixed usage KPI lane, and JSON export describes each fragment's own scope/bounds/coverage instead of assigning one mixed top-level window. Errors deep links use signed `view=attempts` selectors and route-attempt links additionally pin `row_kind=upstream`; Activity opens the exact ingress chain with the selected time range and no query context.
...
- The panel always distinguishes source values (refreshed only by explicit operator refreshes), per-field manual overrides (never touched by refreshes), and the effective merge. Every models.dev write locks the model row before the optional binding row. Bind re-verifies the confirmed Prism identity and catalog ETag; refresh commits against the exact catalog revision, binding coordinate, and `updated_at` token previewed; sparse override confirms the displayed coordinate before merging over the locked current row; bulk override clear and unbind carry the coordinate/`updated_at` snapshot the operator confirmed (unbind is idempotent once the row is gone). Every conflict (`409 models_dev_model_changed` / `models_dev_binding_stale` / `models_dev_catalog_stale`) writes nothing, so neither stale remote data nor a locally newer rebind/override can be clobbered and two operators editing different fields on the same offering never lose updates. Reading is honest too: a failed first read renders an error+retry surface and never a fabricated "未绑定", a failed re-read keeps the last good metadata under a single staleness badge with the last success stamp, `fetched_at` renders through the global timezone, and only a successful `bound:false` response may show the unbound conclusion. All `/api/models/{id}/catalog*` responses are `private, no-store`, and the model detail page composes models.dev and pi.dev under one 外部目录来源 section whose two panels load, fail, and recover independently.
...
- Both client source responses carry an independent backend readiness result from the same pure renderer used by export. Ready means the selected client can render from the sampled facts; it does not assert live upstream health. Blocking reasons link to the owning model editor. Catalog failures distinguish timeout, malformed format, unsupported version, failed content verification and unavailable transport; last-good evidence and saved bindings stay separate, and an explicit retry can recover without fabricating limits or prices.

- The standalone `/route/models/export` page defaults to Pi and offers OpenCode 1.18.27 as a separate target. Switching targets resets and cancels the previous source, selections, credential dialog and generated result; late responses cannot cross targets. The Pi target generates deterministic Pi 0.84.3 `prism-pi-models.json` (Pi `models.json` format) from Prism-managed model truth, behind management authentication, with the pi.dev structured directory (`https://pi.dev/api/models`) as the single template source and a persisted per-model binding as the render authority. Pi selection requires both structural eligibility and backend renderer readiness; missing/blocked readiness cannot be selected, including through a preset. Binding repair stays available without selection, and a repaired model is explicitly selected afterwards. The operator supplies an HTTP(S) Prism gateway origin and a provider id (default `prism`); the export never substitutes a reachable upstream endpoint URL and never uses pi.dev provider/api/baseUrl/cost/headers/samplingParams/fallback/routing to override Prism identity, protocol, gateway, credential selection or pricing truth.
- The backend pi.dev typed catalog uses restricted HTTPS transport (4 s timeout, same-origin redirects, 16 MiB budget, `Accept: application/json`, `ETag`/`If-None-Match` and `304` revalidation, `singleflight` coalescing, `last-known-good` retention, `X-Pi-Model-Catalog-Minimum-Version` validation against Pi 0.84.3, strict `Content-Type: application/json`, HTML rejection, and no browser fetch). The trusted `catalog_revision` is the verified SHA-256 of the response body (`sha256-…`); a missing/mismatched header fails closed and ETag remains a `304` validator only. Candidates use complete case-sensitive `model_id` equality plus final Pi API compatibility (`openai-responses` for `dual_native`/`responses_only`, `openai-completions` for `chat_completions_only`, `anthropic-messages` for Anthropic, `google-generative-ai` for Gemini), never names/slugs/contains/fuzzy. `0`/`1`/`many` states describe the fetched catalog revision, so candidate counts may change when that revision changes. Multiple coordinates—even with identical sanitized templates—always require an explicit operator choice. Separately, `POST /api/models/{model_config_id}/pi/search` is the bounded directory discovery entry for every model whose final Pi API is determinable, including `not_in_catalog` and `api_mismatch` rows: model-id-only case-insensitive literal containment (never provider, display name, or fuzzy), same-final-Pi-API only, ranked exact/prefix/substring then provider/id over the whole same-API hit set before the `offset` window is cut, default 20 and max 100 with append paging through the reported `total` (pages echo their window and `fetched_at`/`checked_at` separately), never preselected, and it writes nothing. `GET /api/models/{model_config_id}/pi` is the per-model Pi management read for the model detail panel: model identity including Prism's own final `pi_api`, one catalog evidence block (`fresh`/`stale` last-known-good/`unavailable`), live exact-candidate evidence, and the persisted binding truth from one read-only `REPEATABLE READ` snapshot — never export targets, pricing, source digests, credentials, or render results, and a catalog outage never manufactures an unbound binding. The model detail panel and the export row actions drive the same shared binding controller and dialogs, so both hosts issue byte-identical bind/refresh/override/unbind payloads and always reconcile through their own authoritative re-read. Each candidate's `compat` is sanitized through its concrete Pi 0.84.3 API allowlist; cross-API/unknown/routing/fallback fields, headers, sampling parameters, and unknown optional fields become sorted read-only `dropped_fields` evidence. Manual overrides reject rather than silently drop those keys.
- `model_pi_catalog_bindings` (`000027_model_pi_catalog_bindings.sql`, purely additive) freezes at most one binding per model: the selected coordinate, `bind_source`, trusted `catalog_revision`, stamps, seven safe `source_*`/`override_*` pairs, and sorted `source_dropped_fields`. It is independent of the models.dev table used by §4.19. `POST /api/models/{id}/pi/bind` requires `expected_catalog_revision` plus `expected_prism_model_id`/`expected_pi_api` and re-verifies all three under row lock, so a stale preview or a concurrent Prism rename can never commit. An explicit coordinate must be a real entry in the fetched revision carrying exactly the model's current final Pi API, which makes a deliberate cross-directory `catalog_model_id` bindable while a cross-API one rejects `422 pi_candidate_api_mismatch`; without coordinates the unchanged 0/1/many exact flow decides. Each bind freezes the confirmed Prism identity into `prism_model_id_at_bind` (`000028_model_pi_binding_identity.sql`, additive and data-preserving, backfilled from `catalog_model_id` so existing rename drift survives instead of being healed), and that snapshot — not the directory id — is what renderability compares against. Repeating the same coordinate with the same identity is a no-op; repeating it after a Prism rename advances the identity snapshot and `updated_at` CAS token while keeping frozen source, overrides, revision, `fetched_at` and `bind_source`; changing the coordinate clears overrides. Refresh preview returns source/dropped diffs plus coordinate, API, binding-update, and catalog-revision tokens; commit must match all five expected values under row lock before changing only source/dropped fields. `PUT` override is sparse across all seven leaves, validates `compat` against the bound API, and treats explicit null as per-field restore; delete clears all overrides, and unbind removes the row. The directory search plus the six bind/refresh/override/unbind routes are `Default-profile`, `M2`, planning-neutral, `private, no-store`.
- `GET /api/models/exports/pi/source` and `POST /api/models/exports/pi/render` share `Default-profile`, `M3`, `planning-neutral`, `private, no-store`; Pi and OpenCode have separate literal source/render routes. `source` combines one best-effort live fetch with one consistent DB snapshot and separates live candidate evidence from persisted binding evidence (`pi_selected`, `pi_binding_status`, `pi_binding_renderable`, source/override/effective metadata, dropped fields, and stamps). Live coordinate/template drift is visible and refreshable while a compatible frozen row stays renderable; changing the Prism full model ID or final Pi API makes it non-renderable until the identity is re-confirmed. `pi_binding_prism_model_id` publishes the frozen identity and `pi_api` the model's final Pi API, so one row can honestly read `not_in_catalog` while still being `bound` and renderable through a deliberate cross-directory bind. `render` performs no network I/O and reads no live or LKG catalog: it requires the source digest and exactly one matching coordinate assertion for every selected model, returns `422 candidate_unselected` for a missing binding/assertion and `422 candidate_invalid` for an extra/mismatched/incompatible coordinate (with `model_config_id`), and renders from the frozen effective template alone. The digest covers each binding's full coordinate, frozen Prism identity snapshot, revision, effective template, and dropped evidence but excludes the current live-fetch outcome and any directory search result, so switching between identical templates still changes it while catalog reachability does not.
- Output preserves only safe pi.dev leaves `name`/`reasoning`/`input`/`contextWindow`/`maxTokens`/`thinkingLevelMap`/`compat`; Prism's `model_id`, mapped Pi `api`, gateway-derived `baseUrl`, provider key, credential choice, and current pricing remain authoritative. When selected models share one derived client URL, only the provider definition carries `baseUrl` and every model inherits it; mixed URLs remain per-model and produce `mixed_base_urls`. Pricing stays `fail-closed` across all actually reachable Terminal Targets for every accepted operation (USD/`PER_1M`, five components, `reasoning_price == output_price`, lossless tier mapping for Pi's single strict positive tier; `pricing_component_missing` on any null component including `reasoning_price=null`, plus distinct warnings for no template, currency/unit mismatch, target conflict, peak/valley and tier unrepresentable; explicit `"0"` alone means free). Source headers/samplingParams/fallback/routing are never proxied into the file.
- Credentials have only `include=false` (omit) or `include=true` with a manually entered, trimmed, non-empty string; Pi 0.84.3 rejects an empty `apiKey`, so the dialog blocks it and the backend returns `422 credential_api_key_required` defensively. Export never reads stored endpoint keys and the typed value never enters source URLs, PostgreSQL, query caches, browser storage, logs, errors or warnings. All `source`/`render` and error responses are `private, no-store`; the rendered file ends with exactly one newline, `content_sha256` is the SHA-256 of those UTF-8 bytes, the response carries the fixed `prism-pi-models.json` name and `application/json;charset=utf-8` MIME, and full-content copy, Blob download and a true new-tab raw view reuse the same bytes. The browser may also copy a locally derived `{ "<provider_id>": { ...provider... } }\n` fragment for merging beneath an existing `models.json` `providers` object. Both copy actions use the shared clipboard helper, including the textarea fallback when Clipboard API is absent or rejects on a plain-HTTP LAN origin; the copied state appears only after success. Closing the result or leaving the route clears content and revokes its Blob URLs.
- The Pi target preserves its binding workflow without default-model or upload-enhancement controls, and honestly distinguishes live catalog `fresh`/`stale`/`unavailable`, `not_in_catalog`, `api_mismatch`, and `multiple` candidate evidence from persisted `unbound`/`bound`/`bound_drifted` binding status, shows the selected candidate's seven safe template fields and dropped paths before bind/rebind confirmation, surfaces refresh/override affordances per model, and keeps the provider-merge fragment. Bind and rebind share one 更换来源 dialog for every model with a determinable final Pi API, keeping the default exact-candidate layer and the bounded directory-search layer visibly separate; nothing is preselected, a one-hit search is still only evidence, the final export identity is shown before confirmation, and confirm is blocked unless the catalog is `fresh`. The two Provider ID domains are labelled distinctly — 导出 Provider 键名 for the operator-authored exported file key and 目录 Provider for a binding's pi.dev origin — and row actions beyond three collapse into an overflow menu.


- **OpenCode 1.18.27:** eligible enabled direct text entries export to `opencode-prism.json` using an independent `prism` or `prism-` custom provider key. Every model retains its complete Prism ID and carries its own SDK and gateway URL: Chat Completions uses `@ai-sdk/openai-compatible` with `/v1`, Responses and `dual_native` use `@ai-sdk/openai` with `/v1`, Anthropic uses `@ai-sdk/anthropic` with `/v1`, and Gemini uses `@ai-sdk/google` with `/v1beta`. There is no provider-level `baseURL` override. A primary conversation operation must be statically routable and its unchanged ID must be addressable by the existing runtime path grammar; Gemini IDs whose URL interpretation changes that identity are unavailable with a reason.
- OpenCode source and render read one read-only Default-profile snapshot each, using current reachable-target price facts and persisted models.dev source/override leaves. They never fetch either catalog, require Pi bindings, change metadata or invalidate runtime planning. Manual overrides take precedence for safe fields; names prefer Prism `display_name`, then a valid effective directory name, then the complete model ID. The page shows saved source, override, final value, provenance and missing/invalid reasons. Valid explicit false values, empty modality lists and expressible zero values remain distinct from absence. Context and output limits must be positive safe integers, and input/output individually cannot exceed context; an invalid limit group blocks that model. The existing metadata panel owns corrections.
- Only target-supported name/family/release date, attachment/reasoning/tool/temperature capability, modalities and limits are mapped. Custom variants, SDK options and interleaved reasoning are neither inferred nor edited. OpenCode may still apply model-ID/SDK-dependent defaults; automatic behavior is not a claim of manual reasoning adaptation. An explicit optional input limit of zero is preserved in configuration, but OpenCode can apply its own fallback budget when consuming it.
- OpenCode's four base costs are emitted only for identical standard USD/PER_1M current prices across all reachable targets, with all five Prism price components configured and reasoning price equal to output. Any tier, peak/valley schedule, missing component, currency/unit mismatch or conflict omits the entire cost object and explains why. No catalog price fills a gap, and a client zero estimate is not proof of free usage.
- `GET /api/models/exports/opencode/source` and `POST /api/models/exports/opencode/render` are `M3`, planning-neutral and `private, no-store`, including errors. The digest covers target version and actual consumed model, route/target, price and effective metadata/provenance facts; Pi bindings, live directory availability and unrelated or shadowed catalog fields do not affect it. Stale input returns `409 export_source_stale`; refresh and regenerate. Invalid selections or target parameters are rejected independently of frontend checks.
- OpenCode's no-key mode writes `env=[PRISM_API_KEY]` without `apiKey`; manual mode writes only the trimmed non-empty final key. Literal `{env:...}`/`{file:...}` inside supplied values are safely JSON-escaped to preserve their meaning through the client's configuration substitution. Credentials stay out of source responses, URLs, Prism persistence, caches and logs. The shared result sheet provides exact-byte full copy, download and new-tab viewing, plus a map to merge below singular `provider`; it clears content and Blob URLs on close, replacement, target switch or route exit. Loading and merging instructions are in [README](../README.md#client-configuration-export).

### 4.21 Personal Investigation and Maintenance
...
1. Render page chrome through `OperatorPageShell` and `OperatorPageHeader`.
...
4. Compose exact-request focus, filters, the shared toolbar, the selected ingress-chain or attempt table, and the overview detail sheet.
...
- Default attempt columns: time, status, TTFT, output rate, requested model, attempt target model, Terminal Target, total tokens, known cost, and pricing state. Optional columns include latency, client, reasoning effort, API family, endpoint, stream, and the persisted upstream model ID; column visibility is configurable.
- The ingress-chain view and the Observe Activity feed have no separate pricing-state column: a priced request shows only its known cost, and any other pricing state sits beside the amount.
...
- `columns.tsx` owns available columns, the `pricing_state` cell, and scoped status/duration helpers; `requestLogColumnPreferences.ts` selects the default visible set.
...
3. Retained browse filters update URL state with `replace: true`, clear every incompatible pagination anchor, and drive refreshed list/CSV cohorts directly without browser-side search or triage refinement. `client_rule_id` filters caller user agents only, `ingress_model_id` selects the requested model, `attempt_target_model_id` selects retained attempt targets, and signed `final_target_model_id` selects the finalized owner. `pricing_status` is the only pricing-state filter; `priced` is never generated or accepted.
...
The protected frontend route shell mounts observability, model configuration, endpoints, routing strategies, pricing, settings, and proxy-key workflows. Release and deployment facts belong to [Project Status](../STATUS.md) and the [startup guide](../README.md).
...
The [startup guide](../README.md) owns local launcher URLs and port selection. [Project Status](../STATUS.md) owns the deployed-instance inventory. Paths below are relative to the chosen Prism origin. The legacy `/models` and `/models/:id` paths redirect to `/route/models` and `/route/models/:id`; legacy Observe `overview`/`analytics` values open Trend, and `events` redirects to `/observe/routing-health`.

### Shared Scope Rules

- Public auth routes are `/auth/login`.
- Protected shell routes cover `/observe`, `/observe/routing-health`, `/observe/requests`, `/observe/requests/:requestId/audit`, `/route/models`, `/route/models/:id`, `/route/models/export`, `/route/endpoints`, `/route/ban-policies`, `/route/pricing`, `/system/settings`, and `/system/proxy-keys`; the canonical trend view is `/observe?tab=trend`.
...
1. The Now strip reads the rolling request count, RPM, TPM with token coverage, and enabled-model count. This rolling window is independent of the selected analysis preset.
2. Window KPI cards use one finalized-ingress query context and show request count, final completion rate, TTFT P95, measured output rate, cache-read share, and known cost with coverage and pricing evidence. Final completion includes stream/transport/client-disconnect outcomes; HTTP status success alone does not establish completion.
...
- `GET /api/stats/dashboard/now` for the rolling Now strip
- `GET /api/stats/query-context` for signed preset/scope bounds and coverage
- `GET /api/stats/usage-summary` for fixed-ingress Window KPIs
- `GET /api/stats/usage-series` and `/api/stats/usage-errors` for scoped analysis
- `GET /api/stats/observe-activity` for paginated finalized-ingress activity
- `GET /api/stats/endpoints/{endpoint_id}/terminal-targets` for the selected endpoint's scoped target statistics
- `POST /api/loadbalance/events/query-context`, `GET /api/loadbalance/events`, and `GET /api/loadbalance/current-state` for Routing Health
- Setup card existing-owner reads: endpoint, pricing-template, routing-policy, model and model-detail APIs, plus proxy-key setup readiness with the expected route-witness generation

The older dashboard aggregate, recent-activity, incident, usage-snapshot, and endpoint-model statistics endpoints remain API contracts; they are not the current Observe route's data sources.
...
OpenAI text routing is native-only and mode-strict. Operators set runtime support on each Terminal Target through `openai_text_capability`, using `responses_only`, `chat_completions_only`, or `dual_native`; each mode may connect only to the identical mode (3×3 equality matrix, diagonal only). The management UI filters target-model candidates and locks connection capabilities to the owner model's mode, and the backend rejects cross-mode authoring with `422 target_openai_mode_mismatch` and mode changes that break existing relations with `409`. Mode-incompatible Chat Completions/Responses attempts are skipped rather than translated. Startup runs the read-only persisted-relation scan after migration and upgrade work and refuses to serve when violations remain. Diagnostic operation-set coverage (`full|partial|none`) does not relax this authoring or runtime equality gate.
...
2. Provider adapters parse native payloads, rewrite the selected upstream model identity, build upstream requests, classify responses/streams, and extract usage. OpenAI Chat Completions and Responses retain their native operation shapes; Prism does not translate between them.
...
Priority validation commands belong to the [Contributing Guide](../CONTRIBUTING.md). Lane pressure, admission responses, scheduler behavior, durable telemetry, best-effort feedback, and cache-generation failure modes are defined in [Architecture §2.5](architecture.md#25-priority-enforcement-and-operator-facing-failure-modes). Use those owning contracts when diagnosing delayed observability or management overload; current runtime state and retained events are available through Routing Health.

### Cross-References

- [Product scope](#4-core-features)
- [API contracts](architecture.md#14-api-reference)
- [Request investigation details](#8-requests-page-specification)
- [Deployment and retained-data facts](../STATUS.md)
