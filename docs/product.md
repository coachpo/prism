...
- The backend pi.dev typed catalog uses restricted HTTPS transport (4 s timeout, same-origin redirects, 16 MiB budget, `Accept: application/json`, `ETag`/`If-None-Match` and `304` revalidation, `singleflight` coalescing, `last-known-good` retention, `X-Pi-Model-Catalog-Minimum-Version` validation against Pi 0.84.3, strict `Content-Type: application/json`, HTML rejection, and no browser fetch). The trusted `catalog_revision` is the verified SHA-256 of the response body (`sha256-…`); a missing/mismatched header fails closed and ETag remains a `304` validator only. Candidates use complete case-sensitive `model_id` equality plus final Pi API compatibility (`openai-responses` for `dual_native`/`responses_only`, `openai-completions` for `chat_completions_only`, `anthropic-messages` for Anthropic, `google-generative-ai` for Gemini), never names/slugs/contains/fuzzy. `0`/`1`/`many` states are explicit; the `qwen3.8-flash` examples remain two Chat candidates (`qwen-token-plan`, `qwen-token-plan-cn`), one Anthropic candidate (`opencode-go`), no Responses candidate, and no match for `openrouter/qwen/qwen3.8-flash`. Multiple coordinates—even with identical sanitized templates—always require an explicit operator choice. Each candidate's `compat` is sanitized through its concrete Pi 0.84.3 API allowlist; cross-API/unknown/routing/fallback fields, headers, sampling parameters, and unknown optional fields become sorted read-only `dropped_fields` evidence. Manual overrides reject rather than silently drop those keys.
- `model_pi_catalog_bindings` (`000027_model_pi_catalog_bindings.sql`, purely additive) freezes at most one binding per model: the selected coordinate, `bind_source`, trusted `catalog_revision`, stamps, seven safe `source_*`/`override_*` pairs, and sorted `source_dropped_fields`. It is independent of the models.dev table used by §4.19. `POST /api/models/{id}/pi/bind` accepts an explicit current candidate or the sole exact/API-compatible candidate; absent/ambiguous sets reject with `422`/`409`. Binding the same coordinate is idempotent; changing it clears overrides. Refresh preview returns source/dropped diffs plus coordinate, API, binding-update, and catalog-revision tokens; commit must match all five expected values under row lock before changing only source/dropped fields. `PUT` override is sparse across all seven leaves, validates `compat` against the bound API, and treats explicit null as per-field restore; delete clears all overrides, and unbind removes the row. All six routes are `Default-profile`, `M2`, planning-neutral.
- `GET /api/models/exports/pi/source` and `POST /api/models/exports/pi/render` share `Default-profile`, `M3`, `planning-neutral`, `private, no-store`; only the literal Pi route is mounted. `source` combines one best-effort live fetch with one consistent DB snapshot and separates live candidate evidence from persisted binding evidence (`pi_selected`, `pi_binding_status`, `pi_binding_renderable`, source/override/effective metadata, dropped fields, and stamps). Live coordinate/template drift is visible and refreshable while a compatible frozen row stays renderable; changing the Prism full model ID or final Pi API makes it non-renderable until rebind. `render` performs no network I/O and reads no live or LKG catalog: it requires the source digest and exactly one matching coordinate assertion for every selected model, returns `422 candidate_unselected` for a missing binding/assertion and `422 candidate_invalid` for an extra/mismatched/incompatible coordinate (with `model_config_id`), and renders from the frozen effective template alone. The digest covers each binding's full coordinate, revision, effective template, and dropped evidence but excludes the current live-fetch outcome, so switching between identical templates still changes it while catalog reachability does not.
...
- The Pi-only UI has no platform/default-model/upload-enhancement controls, honestly distinguishes live catalog `fresh`/`stale`/`unavailable`, `not_in_catalog`, `api_mismatch`, and `multiple` candidate evidence from persisted `unbound`/`bound`/`bound_drifted` binding status, shows the selected candidate's seven safe template fields and dropped paths before bind/rebind confirmation, surfaces refresh/override affordances per model, and keeps the provider-merge fragment.
