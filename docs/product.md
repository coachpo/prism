...
- The panel always distinguishes source values (refreshed only by explicit operator refreshes), per-field manual overrides (never touched by refreshes), and the effective merge. Every models.dev write locks the model row before the optional binding row. Bind re-verifies the confirmed Prism identity and catalog ETag; refresh commits against the exact catalog revision, binding coordinate, and `updated_at` token previewed; sparse override/clear merge over the locked current row; and unbind carries the coordinate/token snapshot the operator confirmed (idempotent once the row is gone). Every conflict (`409 models_dev_model_changed` / `models_dev_binding_stale` / `models_dev_catalog_stale`) writes nothing, so neither stale remote data nor a locally newer rebind/override can be clobbered and two operators editing different fields never lose updates. Reading is honest too: a failed first read renders an error+retry surface and never a fabricated "未绑定", a failed re-read keeps the last good metadata under a single staleness badge with the last success stamp, `fetched_at` renders through the global timezone, and only a successful `bound:false` response may show the unbound conclusion. All `/api/models/{id}/catalog*` responses are `private, no-store`, and the model detail page composes models.dev and pi.dev under one 外部目录来源 section whose two panels load, fail, and recover independently.
