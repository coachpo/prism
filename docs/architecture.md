...
The client model-export feature keeps source snapshot/query, selection reconciliation, directory search, and every Pi binding mutation (bind/refresh-preview/refresh-commit/override-write/override-clear/unbind) in `frontend/src/features/models/export/useModelExportSource.ts`, and destination validation, render request, key-decision handoff, stale handling, and result state in `useModelExportRender.ts`. `ModelExportDestinationPanel.tsx`, `ModelExportSelectionPanel.tsx`, `ModelExportSourcePanel.tsx`, and `ModelExportModelTable.tsx` own their concrete presentation boundaries; selection, source results, risk, and evidence are composed in that order. `PiBindingCell.tsx` composes one row's persisted-binding state, `PiBindingSourceDialog.tsx` owns the unified exact-candidate/directory-search bind and rebind flow, and `PiBindingRefreshDialog.tsx`, `PiBindingOverrideDialog.tsx`, `PiCandidateEvidence.tsx`, and `PiDroppedFieldsEvidence.tsx` own the remaining workflows and evidence views outside the table layout. `ModelExportPage.tsx` is only the page composition root, while `ExportKeyDialog.tsx` and `ExportResultSheet.tsx` retain their existing presentation ownership.
