...
│   │   ├── models/            # `/route/models` list/detail, client setup, and export
│   │   ├── endpoints/         # `/route/endpoints`
│   │   ├── loadbalance/       # `/route/ban-policies`
│   │   ├── pricing/           # `/route/pricing`
│   │   ├── proxy-keys/        # `/system/proxy-keys` credential lifecycle
│   │   ├── runtime-self-test/ # Shared direct verification and record reconciliation
│   │   ├── request-logs/      # `/observe/requests` and saved-content adapter
...
### 2.2A Personal Read and Preference Lifecycles

The Observe feature owns a page-local read-cycle counter shared by its query contexts, summary, Now, chart, setup, activity, errors, current-state summary and previously loaded Terminal Target details. A whole-page refresh starts only after those reads settle; signed-context consumers reload from the newly issued context. The mounted interval is `0`, `30` or `60` seconds, defaults to `0`, pauses while the document is hidden and resets on route exit or auth epoch change. Resource AbortControllers combine with the existing management epoch signal. Same-basis refresh failures retain last-good responses with their original timestamps; scope/preset changes withdraw incompatible data. Stable error-category selection survives a refresh only when the new server projection still contains it, and its request-filter conjunction and signed context are updated together.

Personal preferences are frontend-owned; no server account, background sync or new database table is introduced. Export preset storage uses `prism.export-presets.v1`, with file envelope `{format:"prism.export-presets",version:1,presets}`. Preset fields are `name`, `client`, semantic `modelIds`, `gatewayOrigin` and `providerId`; clients remain independent and render still validates the current source digest. Request views retain `prism.request-logs.saved-views.v1`, with transfer envelope `{format:"prism.request-views",version:1,views}` and allowlisted canonical non-transient query state. Both collections allow at most 20 names of 1–80 characters; file reads are bounded to 256 KiB. Strict transfer validation rejects unknown fields, unsupported versions and query combinations that would lose active filters during normalization. References are reconciled using the existing current-instance model/endpoint/target/key/client owners and bounded currency-segment paging. Storage write failure retains an explicit tab-only copy; it is not a successful persistent save.

The Requests comparison feature reuses the existing request-detail and audit-list contracts for metadata and an explicit selected-audit-detail read for payloads. Ordinary list browsing does not fetch audit bodies. It retains the audit source/direction, capture byte ledger and completeness, then compares only readable conversation text extracted from supported retained formats, with a bounded positional display. Tool arguments, images, attachments, and raw protocol envelopes stay outside the comparison. It never invokes runtime execution or manufactures missing content.
...
The canonical durable identity is created by `newRuntimeIngressContext` in `backend/internal/httpapi/runtime/ingress.go` after operation acceptance. Before request-body handling or provider execution, the response writer is bound to that same UUID, so `X-Prism-Ingress-Request-Id` identifies the retained request/usage/audit group when telemetry is recorded. Pre-operation rejection and the local models-list response retain a server-generated branch token and do not create durable request records. A response ID does not itself guarantee that telemetry has materialized; the frontend requires an exact match and labels any broader browsing fallback.
...
Client -> POST /v1/chat/completions {model: "gpt-4o"}
  ...
  -> Gateway assigns the accepted-operation UUID used for durable request/usage/audit grouping
...
Client -> POST /v1/chat/completions {model: "gpt-4o", stream: true}
  ...
  -> While a later candidate remains, a `2xx` SSE response is read up to its first event (at most 32 KiB) before selection; a provider error first event reroutes to the next candidate, otherwise the buffered head is replayed unchanged
...
Stream-start error reroute (`stream_start_inspection.go`): a provider error first event is an SSE `error` event, or a JSON payload that declares `"type": "error"` or carries a non-null top-level `error` member. Prism treats it as an in-band rejection of this request alone, so the executor closes that upstream and reroutes to the next candidate like a `reroute_status_codes` status: no runtime failure feedback (no retry-window write, no retry or ban counting), and the next launch carries `attempt_trigger = reroute`. No status code gates this, and unlike a reroute status the abandoned stream is not kept as the ingress's fallback response. The abandoned attempt persists as `attempt_result = stream_error`, `stream_outcome = provider_incomplete`, `failure_stage = stream`, and `success_flag = false`. Its error code is the recognized provider code, with `stream_provider_incomplete` as the fallback. The last candidate is not inspected, so its stream, including an error first event, passes through unchanged. While a later candidate remains, the client receives response headers only after the upstream's first event.
...
The model-management frontend keeps model CRUD form state in `frontend/src/pages/models/modelFormState.ts`, mixed access-target draft/order rules in `accessTargetFormState.ts`, and detail-side ownership/options/summary projection in `modelAccessTargetProjection.ts`. Connection collection patching is owned by `connectionCollectionState.ts`; `useModelDetailConnectionMutations.ts` only composes the connection submit, lifecycle, reconciliation, and mixed-target mutation owners. The management-only `ModelsDevCatalogPanel.tsx` owns the inset shell, honest read/action state, effective metadata projection, and snapshot-guarded unbind confirmation. `CatalogBindDialog.tsx`, `CatalogRefreshDialog.tsx`, and `CatalogOverrideDialog.tsx` own bind, refresh-preview/commit, and override workflows respectively; none of these facts enters runtime compatibility or routing.
...
The `/route/endpoints` feature keeps its lifecycle owners explicit: `useEndpointReferenceSummaries.ts` owns bounded batch summary hydration and fresh/stale/error replacement, `useEndpointReferenceDetails.ts` owns lazy per-Endpoint snapshots and opaque-cursor pagination, and `useEndpointReferences.ts` owns only their shared generation fence. `useEndpointList.ts` owns Endpoint cache load, visible-ID reference reconciliation, search, and reference-derived filter/sort; `useEndpointMutations.ts` composes the resource-specific form, delete, orphan-cleanup, duplicate, and attach lifecycles without owning their independent state. `EndpointDialog.tsx` separates saved state from optional verification; `AttachToModelDialog.tsx` offers an existing model or `/route/models?action=create&endpoint_id=...` for a preselected new-model flow. The endpoint route also accepts `endpoint_id` for a request-recovery edit entry.

The model flow keeps structured service request settings in `frontend/src/pages/model-detail/RequestSettingFields.tsx` and `structuredRequestSettings.ts`, composed by `ConnectionCustomRequestParametersEditor.tsx`; the existing validator and JSON wire contract remain authoritative. `frontend/src/features/models/detail/ModelClientConnectionCard.tsx` presents the effective client address, exact client model name, key setup, and model-preselected verification. `frontend/src/features/runtime-self-test/effectiveOrigin.ts` remains the common origin owner, and `/system/proxy-keys?action=verify&model_id=...` carries only the model identity, never a secret.
...
The loadbalance, pricing, and proxy-key feature data contracts keep independent state owners behind thin page coordinators.

`frontend/src/features/loadbalance/useBanPolicyStrategyCollection.ts` owns strategy collection/detail reads and revision-backed fragments, `useBanPolicyMutations.ts` owns CRUD/default mutation state, and `useStrategyImpactPager.ts` owns per-strategy cursor merge and stale/error state.

Pricing uses `usePricingTemplateCollection.ts` for the revision cache/list, `usePricingTemplateMutations.ts` for editor impact preflight and create/update/delete CAS workflows, `usePricingTemplateDetailReads.ts` for usage/history reads, and `usePricingImportProtocol.ts` for preview/hash-commit; `usePricingFeatureData.ts` only composes them. `PricingTemplateImportDialog.tsx` reads a selected price file and keeps read/preview failures recoverable; `PricingImportPreview.tsx` presents the explicit commit step. File selection replaces raw JSON authoring in the UI without changing import schemas or hash validation. Price forms use reporting-currency readiness from `ReportingCurrencyContext`; optional blank prices serialize to null and explicit zero remains distinct.

Proxy-key ledger queries/capacity live in `useProxyKeyLedger.ts`, and visible-key usage in `useProxyKeyUsage.ts`; `useProxyKeyCreateMutation.ts`, `useProxyKeyEditMutation.ts`, `useProxyKeyRotateMutation.ts`, and `useProxyKeyDeleteMutation.ts` own the four credential mutation lifecycles, while `useProxyKeyMutations.ts` only composes their page-facing values. `proxyKeyMutationReconciliation.ts` is the explicit cache-patch/invalidation owner; the individual mutation hooks own safe inline error state so failed operations retain their dialog and draft. `useProxyKeySecretSession.ts` remains the only post-handoff owner of a one-time raw secret. The standing access-verification dialog open state remains page presentation state in `ProxyKeysFeaturePage.tsx`.

`frontend/src/pages/proxy-api-keys/ProxyKeyVerifyAccessDialog.tsx` composes model/key selection with the shared `frontend/src/features/runtime-self-test/RuntimeSelfTestPanel.tsx`; the generated-secret dialog reuses that panel without stacking a second self-test dialog. `useRuntimeSelfTest.ts` owns direct-send, cancellation, and independent record-retry state. `selfTestRunner.ts` sends the existing operation-specific request, consumes the response without retaining diagnostic bodies, then reads the exact ingress chain. Only an authoritative finalized summary with a matching detail record supplies execution, final target, and trusted cost; an arbitrary attempt row or HTTP 200 alone cannot establish completion. Received-response, interrupted-receive, cancelled-wait, and finalized outcomes remain separate. Both chain and detail reads forward cancellation. The panel renders localized outcomes and missing-data recovery without raw responses or headers; known micro-unit fees are converted to currency units, and missing fees are never zero-filled.

The request-log frontend keeps attempts and ingress chains in separate read owners: `frontend/src/pages/request-logs/useRequestLogAttempts.ts` owns attempt-list query/replace/generation/filter state, while `useRequestLogIngressChains.ts` owns chain-page replacement, signed cursor navigation, nested row append fencing, and chain-specific state. `requestLogChainProjection.ts` owns the chain-envelope to attempt-row projection and stable-ID dedupe; `useRequestLogsPageData.ts` only combines the active view for `RequestLogsPage.tsx`. `RequestLogsTable.tsx` owns the virtualized attempt table and `ColumnToggleMenu.tsx` owns column visibility presentation.

The dedicated request-log audit page keeps request detail, audit-list cursor pages, and selected audit detail in `useRequestLogAuditRequest.ts`, `useRequestLogAuditList.ts`, and `useRequestLogAuditDetail.ts`; `useDedicatedRequestLogAudit.ts` only composes their public lane states and retry commands. `useRequestLogProxyApiKeyOptions.ts` owns the filter query key/parameters and selected-option prepend, while `FiltersBarPrimaryFilters.tsx` retains filter markup order. `requestLogMetricPresentation.ts` owns the shared cost, token, TTFT, and token-rate value formatting consumed by both `columns.tsx` and the detail overview; table-only status, pricing, and stream semantics remain in `columns.tsx`. `RequestLogAuditPage.tsx` renders the saved-content page and validates its request-list `return_to` navigation. `detail/RequestLogPayloadBlock.tsx` renders only readable conversation and tool-activity notices from the internal protocol parsers. `detail/RequestFailureRecovery.tsx` is shared by the sheet and content page, backed by `requestFailurePresentation.ts` and exact current-model lookup in `useRequestRecoveryModel.ts`; labels live in `frontend/src/i18n/messages/requestRecovery.ts`.

Routing-health events use `useRoutingHealthQueryContext.ts` for the signed window context lifecycle, `useRoutingHealthEventPage.ts` for URL-backed event filters/cursor/detail selection and page reads, and `LoadbalanceEventsPresentation.tsx` plus `LoadbalanceEventRow.tsx` for the event card and row projection. Global Current State uses `useRoutingHealthCurrentStateRead.ts` for read/pagination, `useRoutingHealthCurrentStateReset.ts` for reset commands and post-reset calibration, and `GlobalCurrentStatePresentation.tsx` plus `GlobalCurrentStateRow.tsx` for filters, table, and confirmation dialog. `RoutingHealthTab.tsx` only composes these owners.

The Settings shell uses canonical public URLs with `scope=global|instance` and a section allowlist (`billing-currency`, `timezone`, `audit-privacy`, `header-blocklist`, `client-rules`, `authentication`, `retention`, `manual-cleanup`, `retention-jobs`); the legacy `tab` query value is dropped during canonicalization. `frontend/src/pages/settings/settingsNavigation.ts` owns those scope and section facts. The visible `显示与记录` group (`scope=global`) keeps billing and reporting currency, timezone, audit & privacy, and config-rule flows; the visible `访问与数据` group (`scope=instance`) owns authentication and operator account, automatic retention policy with actual coverage, manual cleanup, and the retention job center. Audit resource state is split across API-family audit settings, Header Blocklist, and User-Agent Client Rules owners; retention state is split across policy, manual-cleanup, and job lifecycles, with `retentionProtocol.ts` owning their shared protocol invariants. `costing/costingForm.ts`, `currencyMigrationProtocol.ts`, `manualCleanup.ts`, `sections/authentication/authenticationPassword.ts`, and `frontend/src/lib/timezone.ts` own their respective form, migration, cleanup, password, and timezone projections. Normal log retention applies across all profiles; list and detail APIs are pinned to Default id `1`. `retentionPresentation.ts` maps dataset and warning values into user-facing descriptions, while the job and currency components present outcomes and limitations rather than generation/epoch or internal checkpoint fields. UTC cutoff, irreversible deletion, estimated coverage, delayed reclamation, and the effect on active queries remain visible; auth, retention, and currency-migration protocols are unchanged.
...
build_upstream_headers():
  1. Start with the caller's headers, withholding only hop-by-hop headers and host, transport-controlled headers (accept-encoding, content-length), the caller's own credential headers (authorization, x-api-key, x-goog-api-key), and names the provider auth profile controls
...
Outbound client headers are governed by a blocklist, and the Header Blocklist is part of it. The caller's headers reach the upstream as sent, except for four categories that are wrong to copy rather than merely revealing:

| Category | Headers | Why |
| --- | --- | --- |
| Hop-by-hop | `connection`, `keep-alive`, `te`, `trailers`, `transfer-encoding`, `upgrade`, `proxy-authenticate`, `proxy-authorization`, `host` | They describe this connection, not the next one (RFC 7230 §6.1) |
| Transport-controlled | `accept-encoding`, `content-length` | Prism recomputes them; the runtime transport sets `DisableCompression`, so a forwarded `gzip` would return a body the streaming parsers cannot read, breaking usage, token, and output-rate extraction |
| Caller credentials | `authorization`, `x-api-key`, `x-goog-api-key` | Prism authenticates with the connection's own key; forwarding these leaks a client credential to a third party and can collide with the header Prism sets. Checked as a full set, because the auth profile's controlled names only cover the active api family |
| Provider-controlled | The active auth profile's `AuthHeader` and `ExtraHeaders` | Prism sets them downstream in the same pipeline |

A blocklist rather than an allowlist because the goal is that the upstream cannot tell a gateway is there at all. A request stripped down to a handful of protocol headers is itself an anomaly — no real client sends an empty User-Agent and nothing else — so reproducing what the caller sent is what keeps Prism from announcing itself. Which of the remaining headers are undesirable is a deployment question, not a protocol one, and it is answered by the Header Blocklist at runtime rather than by this list at compile time.

`cookie` is withheld by a seeded system rule rather than by the code above, because it is the one caller header whose forwarding is a question about the deployment rather than the protocol. On a shared instance several callers authenticate through one upstream key, and a forwarded `Cookie` would carry one caller's session into requests the others also make. A deployment where the upstream genuinely needs it can disable the rule: system rules cannot be deleted or repatterned, but `enabled` is mutable, so the safe default keeps its escape hatch.

`user-agent` is forwarded, and is worth calling out because it was previously withheld. Withholding it was a negative trade for the gateway-invisibility goal: an empty User-Agent is a stronger signal that a proxy is present than any real client string is, and it made acceptance depend on Prism rather than on the caller. An upstream that only accepts particular User-Agents is still stating a fact about that endpoint, so `connection.custom_headers` remains the way to pin one value for that endpoint; it overrides the forwarded one, and `request_logs.user_agent_overridden` records that the upstream value differed from the caller's. Only when neither the caller nor `custom_headers` supplies one does Prism send an empty User-Agent, to keep net/http from substituting its `Go-http-client` default and naming the gateway's implementation outright.

Withholding a header does not erase it. The audit trail records the headers the caller actually sent unioned with the headers Prism actually forwarded, so an operator can answer both "did my client leak something" and "what did Prism send" from the same record. The Header Blocklist applies twice over: it stops a matching header from being forwarded, and it is fed to the audit scrubber as extra sensitive-name rules so the value is redacted in diagnostics and audit as well. It can only make either stricter — it never weakens the fixed `safediag` bottom line.

Custom headers are a power-user feature and the supported way to pin a header for one upstream regardless of what the caller sent. They can override ordinary forwarded headers, but they cannot override Prism-controlled authentication or provider-version headers and cannot re-add headers blocked by the Header Blocklist. This is enforced by skipping proxy-controlled custom header names and applying the blocklist last in the header construction pipeline.
...
The request-content view is reached from request investigation at `/observe/requests/:requestId/audit`, retaining the existing audit API and route identity. It shows the request outcome, readable messages extracted from recognized stored request/response formats, natural-language tool activity, and capture limitations. Raw headers, JSON/SSE envelopes, tool arguments/results, and raw-download controls are absent from the UI. Invalid-UTF-8, binary, unsupported, missing, and truncated bodies keep explicit explanations and byte/capture evidence instead of manufactured conversation text. The backend still retains and serves its documented audit byte representation; recognized captured text may contain user-provided sensitive content. The overview-only sheet does not load bodies, and the content page preserves the original request-list state through a validated local `return_to` target.
...
The visible Settings `访问与数据` group (canonical `scope=instance&section=retention`) owns `/api/settings/log-retention` with a day count per retained dataset, the owner actual-coverage projection, fresh destructive preflight dialogs, and the durable job center (`scope=instance&section=retention-jobs`). Operators start a one-time cleanup through a fresh manual-cleanup preflight and a sealed manual job; manual queued jobs are cancellable, running manual purges are not.
...
```
Operator -> Settings `访问与数据` group -> 自动保留策略
...
Operator -> Settings `访问与数据` group -> 手动清理
...
Log retention controls live on the visible Settings `访问与数据` group (canonical `scope=instance`), with automatic policy + actual coverage, manual cleanup, and the retention job center. The job center is a static browser-side snapshot over the durable queue: it never polls; manual refresh and post-mutation calibration re-read the loaded pages serially with fresh cursors and swap atomically, while the management-jobs worker remains the only background processor. The visible `显示与记录` group (canonical `scope=global`) manages billing and reporting currency, timezone, audit & privacy, config rules, and other Default-profile state.
...
Prism exposes no standalone global catalog management surface (no catalog CRUD/navigation page); catalog management lives inside the per-model detail page (models.dev metadata, source-linked pricing, and the pi.dev template panel). Model compatibility is carried by each model's required `api_family`; catalog metadata does not participate in runtime routing.
...
Request: `{ provider_id?, catalog_model_id?, expected_catalog_revision, expected_prism_model_id, expected_api_family }`. With explicit coordinates both fields are required and must exist in the fetched catalog (`422 models_dev_offering_unknown` otherwise); without them the unique exact match is applied, while ambiguity or absence rejects with `409` plus the candidate list. A revision mismatch returns `409 models_dev_catalog_stale`. The write transaction takes the model row lock, re-verifies the confirmed Prism identity (`409 models_dev_model_changed` on a rename or family edit), then takes the binding row lock and re-resolves the carry-forward from the locked current row: a different offering clears prior manual overrides, the same offering keeps overrides and the original match source. After binding, refreshes never re-guess the coordinates.
...
Preview fetches the current catalog revision, diffs it against the stored source snapshot field by field (`changes[]` with `added`/`removed`/`changed`), and publishes the binding CAS snapshot: `provider_id`, `catalog_model_id`, `binding_updated_at`, `catalog_revision`, `fetched_at`. Commit requires `expected_provider_id`, `expected_catalog_model_id`, `expected_binding_updated_at`, and `expected_catalog_revision`, locks the model and binding rows, rejects with `409 models_dev_binding_stale` when the coordinate or token no longer matches, then replaces only the source columns plus revision/fetch stamp (`409 models_dev_not_bound` when unbound; `409 models_dev_offering_missing` when the bound offering vanished from the catalog — fail closed, nothing is wiped).
...
`PUT` accepts `{expected_provider_id, expected_catalog_model_id, override}`, where `override` is a sparse object of known metadata fields; each present key writes that override, and an explicit JSON `null` restores the field to its source value (per-field restore). Unknown fields, wrong types, out-of-range numbers, or non-enum statuses return `422`. The write confirms that the displayed coordinate is still bound, then merges over the locked current row and rewrites only the override columns plus the CAS token, so a stale draft cannot cross a rebind while concurrent same-offering sparse overrides of different fields never lose updates. `DELETE /override` clears every manual override only when the confirmed `{expected_provider_id, expected_catalog_model_id, expected_binding_updated_at}` snapshot still matches. `DELETE /catalog` uses the same snapshot to unbind (remove the binding row) without touching runtime identity, rejects `409 models_dev_binding_stale` when a concurrent rebind/refresh/override intervened, and stays idempotent once the row is gone. All are planning-neutral.
...
Response `200`: `{ items[] (provider_id, provider_name, model_id, name), total, limit, offset, scope, catalog_revision, fetched_at }` — every page publishes the snapshot revision and fetch stamp it was computed from, without fabricating a freshness enum. `limit` clamps to 100; `scope=family` restricts to the api_family's canonical providers and `all` covers every catalog provider for manual binding. All `/api/models/{id}/catalog*` responses — including this one, the match preview, and every error — carry `Cache-Control: private, no-store`; the model-detail `GET /api/models/{id}` response also carries it while the legacy embedded models.dev catalog field remains.
...
##### Client Readiness and Repair

Each Pi/OpenCode source model includes `readiness: {status: "ready"|"blocked", blocking_reasons: string[], repair_path: string}`. The management projection invokes that client's existing pure renderer for one model with a placeholder local origin and no credentials; it does no network I/O. This keeps source readiness and render gates identical, including required metadata, static operation reachability and the client's independent binding rules. `repair_path` points to `/route/models/{id}`; the OpenCode export page may mount the existing metadata editor in place and re-read source after a saved correction. Pi initial selection, toggles, bulk selection, refresh reconciliation and preset restoration require both structural `selectable` and `readiness.status=ready`; absent readiness fails closed. Unselected rows keep binding repair actions, and a newly repaired row is explicitly checked. The backend structural `selectable` field retains its separate meaning. The sampled readiness is not a runtime-health assertion. Price omission remains a separate warning, never zero-price evidence.

Pi catalog blocks in source, search and the per-model read expose `failure_code` on failure, with `fetched_at` and `checked_at` when available. Safe classifications are `timeout`, `format`, `version`, `checksum`, `too_large`, `cancelled`, and `unavailable`; successful reads clear the failure. Source readiness uses retained binding facts even when live catalog evidence is stale or unavailable. models.dev and Pi remote errors are sanitized inside their catalog owners and never echo response bodies, headers or credential-bearing URLs.

##### Pi Catalog, Transport and Candidate Matching

The pi.dev typed catalog is fetched only outside any DB transaction via restricted HTTPS (`4 s` timeout, same-origin `https` redirects, `16 MiB` budget, `Accept: application/json`, `ETag`/`If-None-Match` and `304` revalidation, `singleflight` coalescing, `last-known-good` retention, `X-Pi-Model-Catalog-Minimum-Version` validation against `0.84.3`, strict `Content-Type: application/json`, and HTML rejection; browsers never fetch pi.dev directly). The trusted `catalog_revision` is validated as the response body's own `SHA-256` (`sha256-…`): a missing or mismatched `X-Pi-Model-Catalog-Revision` header fails the fetch closed and is never satisfied by falling back to the `ETag`, which is a `304` validator only and carries no trust of its own. `200` with a matching revision returns a fresh catalog, `304` revalidates the existing `LKG`, transport/timeout/illegal-schema/version-too-new/revision-mismatch keep `LKG` as `stale`, and no `LKG` fails. Candidates are derived by complete `model_id` case-sensitive exact equality plus final Pi API compatibility (`openai-responses` for `dual_native`/`responses_only`, `openai-completions` for `chat_completions_only`, `anthropic-messages` for Anthropic, `google-generative-ai` for Gemini); no name/slug/contains/fuzzy. `0`/`1`/`many` states are explicit, special characters are preserved verbatim, `qwen3.8-flash` Chat has two `openai-completions` candidates (`qwen-token-plan`, `qwen-token-plan-cn`), Anthropic has one `opencode-go` (`anthropic-messages`), Responses has none, and `openrouter/qwen/qwen3.8-flash` does not match `qwen3.8-flash` — this fixture is `TestCatalogCandidatesExactCaseSensitiveIDPlusAPIOnly` in `backend/internal/domain/pidev/client_test.go`. A single candidate may be proposed for one-click bind; multiple candidates — even sharing one template — never auto-merge/first/lex/provider-preference and always require an explicit `provider_id`/`catalog_model_id` bind. `Catalog.SearchModelIDs` is the separate bounded directory discovery rule behind `POST .../pi/search`: model-id-only case-insensitive literal containment (provider id, display name, `baseUrl`, and every other field are never searched), no fuzzy/slug/token/edit-distance/wildcard expansion, restricted to entries whose `api` equals the model's current final Pi API, ranked exact then prefix then substring then provider id then model id byte-wise on the original case-sensitive values over the whole same-API hit set before the `offset` window is cut, defaulting to a 20-row page, clamping to 100, echoing the requested `offset` and the full match total, and returning equally-unselected hits — a one-hit search is still not a selection. The search response also carries `checked_at` next to `fetched_at`, keeping the content-download stamp and the last revalidation (304-included) stamp distinguishable. The case-sensitive `Candidates` rule above is untouched by it. Catalog `compat` is sanitized through an API-specific Pi 0.84.3 allowlist before persistence; unknown/cross-API keys and provider-routing/fallback directives are removed with sorted `dropped_fields` evidence, while manual overrides reject the same keys. `provider`/`baseUrl`/`cost` are validation-only and never retained downstream; `headers`, `samplingParams`, and unknown optional fields are likewise dropped as evidence, so none can override Prism identity, protocol, gateway, credential or pricing truth.

##### Persisted Pi Catalog Binding

`model_pi_catalog_bindings` (`000027_model_pi_catalog_bindings.sql`, purely additive, independent of the models.dev `model_catalog_bindings` table in §1.3A) holds at most one row per model: the frozen coordinate (`provider_id`, `catalog_model_id`, `api`), the frozen Prism identity `prism_model_id_at_bind` added by `000028_model_pi_binding_identity.sql`, `bind_source` (`single_candidate`/`manual`), the trusted `catalog_revision`, `fetched_at`/`updated_at`, `source_*`/`override_*` column pairs for the seven safe leaves `name`/`reasoning`/`input`/`context_window`/`max_tokens`/`thinking_level_map`/`compat`, and sorted `source_dropped_fields` evidence. It is the render authority: `source`/`render` read it directly and never re-derive a selection from a live fetch.

```text
GET /api/models/{model_config_id}/pi
POST /api/models/{model_config_id}/pi/bind
POST /api/models/{model_config_id}/pi/search
POST /api/models/{model_config_id}/pi/refresh/preview
POST /api/models/{model_config_id}/pi/refresh/commit
PUT /api/models/{model_config_id}/pi/override
DELETE /api/models/{model_config_id}/pi/override
DELETE /api/models/{model_config_id}/pi
```

`POST .../pi/search` accepts `{model_id_query, limit?, offset?}` (a negative `offset` is a `422`) and returns catalog status/revision, `fetched_at` and `checked_at`, Prism export identity, and bounded same-API candidate evidence that echoes the requested `offset` and the full `total`; a `stale` response is read-only evidence and never a bind authority. `GET /api/models/{model_config_id}/pi` is the single-model Pi management read: one best-effort `piCatalogForRead` fetch outside any transaction, then one profile-scoped read-only `REPEATABLE READ` transaction over the model row and its persisted binding, then the pure projection. It returns `{model (model_config_id, model_id, api_family, pi_api), catalog (status/revision/minimum_version/etag/fetched_at/checked_at), candidate_status, candidates, binding_status, binding_renderable, binding}` where a catalog outage degrades only the live evidence block (`status=unavailable`, `candidate_status=catalog_unavailable`) and never manufactures an unbound or non-renderable binding. It never loads export targets, pricing plans, source digests, credentials, render results, or any runtime graph, and it shares the live-candidate/binding-health projections with the export source so both surfaces compute identical axes. `bind` requires `expected_catalog_revision`, `expected_prism_model_id`, and `expected_pi_api`; it fetches and revision-checks the catalog outside the write transaction, then re-verifies those expectations and re-resolves the coordinate inside it under row lock, so every rejection writes nothing. With explicit `provider_id`/`catalog_model_id`, both fields are required and the pair must be a real entry in the fetched revision whose `api` equals the model's current final Pi API — a deliberate cross-directory `catalog_model_id` is therefore bindable, while an unknown pair rejects `422 pi_candidate_unknown` and a different-API pair rejects `422 pi_candidate_api_mismatch`. Without them the unchanged default flow decides: a unique complete-id-plus-API candidate applies automatically (`bind_source=single_candidate`), zero candidates reject with `422 pi_candidate_not_in_catalog`/`pi_candidate_api_mismatch`, and multiple candidates reject with `409 pi_candidate_ambiguous` plus the complete candidate list. A mismatched identity/API expectation rejects `409 pi_model_changed`. Every accepted bind also freezes the confirmed Prism model id into `prism_model_id_at_bind`. Rebinding the same coordinate with the same confirmed identity is idempotent and leaves its source, revision, overrides, stamps, and original `bind_source` unchanged; rebinding the same coordinate after a Prism rename preserves frozen source, overrides, revision, `fetched_at`, dropped evidence and `bind_source` while advancing the identity snapshot and `updated_at` CAS token; rebinding a different coordinate freezes that candidate and clears overrides. `refresh/preview` diffs the seven source fields plus dropped-path evidence and returns `provider_id`, `catalog_model_id`, `api`, `binding_updated_at`, and `catalog_revision`; `refresh/commit` requires the five matching `expected_*` values, locks and revalidates the current model/binding, re-resolves the coordinate against that revision, then replaces only source/dropped fields and stamps. A coordinate/version/revision change or missing/API-changed candidate returns `409` with no write. `override` `PUT` accepts a sparse object of the seven fields, each validated against the bound API through the same Pi 0.84.3 schema `RenderPi` enforces, with explicit JSON `null` restoring only that field to source; `DELETE /override` clears every override; `DELETE /pi` unbinds without touching runtime identity. All eight model-level routes are `Default-profile`, `M2`, planning-neutral.
...
The UI mounts the existing metadata editor in the export context for corrections, preserves valid selections after an authoritative source re-read, and shows all final values, provenance and missing/invalid reasons. OpenCode can apply SDK/model-ID-based inference defaults after loading; the exporter does not claim authored custom reasoning adaptation. [README](../README.md#client-configuration-export) owns loading and provider-merge instructions. Package-local renderer tests, management contract tests and the existing browser journey cover their respective boundaries; `tests/runtime/opencode_*_test.go` provides the separately invoked real-client round-trip boundary documented in [CONTRIBUTING](../CONTRIBUTING.md#real-opencode-export-acceptance).

---

#### 1.3C Bounded Batch Maintenance

```text
POST /api/models/batch/model_limits/preview
POST /api/models/batch/model_limits/apply
POST /api/models/batch/model_strategy/preview
POST /api/models/batch/model_strategy/apply
POST /api/models/batch/target_pricing/preview
POST /api/models/batch/target_pricing/apply
```

All routes use Default profile `1`, management `M2`, and `private, no-store`. A request contains `items` (1–20 unique positive ids), `confirm_manual_overrides`, optional `preview_token` and action-specific inputs. `model_limits` uses model ids with explicit positive safe-integer `context_limit`/`output_limit` (`output <= context`); it accepts only omitted or `opencode` client and writes the existing models.dev overrides without changing Pi bindings. `model_strategy` uses model ids and a positive strategy `reference_id`. `target_pricing` uses Terminal Target ids and a positive pricing-template `reference_id`. Mixed-action fields reject.

Responses contain `action`, `preview_token`, `can_apply`, `applied`, and per-item `id`, `label`, `before`, `after`, `manual_override`, optional `error`. Preview hashes the normalized input and locked authoring facts, including owner identity, binding/update tokens and referenced policy/template revision. Apply revalidates the whole batch under deterministic owner locks, checks the token and explicit replacement confirmation, then executes all writes in one transaction. A stale preview is `409`; an invalid matching preview or unconfirmed manual replacement is `422`; neither partially writes. The returned applied values are authoritative transaction readback, and consumers invalidate related collections before continuing.

Pricing assignment shares the existing profile serialization lock before model/target locks, preserving the pricing writer lock order. It retains native text equality/image containment validation even for disabled targets, and increments pricing-reference generation in the same transaction. Strategy application invalidates planning and route-witness generations; pricing application invalidates planning; metadata-only limits are planning-neutral. No schema migration or history rewrite is involved.
...
#### 1.5B Sampled Route Explanation

`GET /api/models/{model_config_id}/route-explanation?operation={native-operation-name}` is a Default-profile `M1`, `private, no-store` observation. The operation must be a model-bound entry in the runtime operation registry and match the requested model. The handler uses the already-published runtime snapshot without triggering a refresh. `generation` and `published_at` identify that process-local snapshot, not the latest database commit or route-witness generation.

The response carries `observed_at`, `sample_completed_at`, `model_config_id`, `operation`, `completeness` (`sampled` or `partial`), `boundary`, optional `planner_error`, `candidates` and `exclusions`. Candidates include `path`, owner `strategy`/`model_config_id`/`model_id`, `terminal_target_id`, nullable `planner_position`, `schedule`, `runtime_observed`, `reason`, and `capacity`. Exclusions retain the path, model identity, target type and reason. Missing hot-state observations do not become zero occupancy or known availability.

The runtime owner copies existing state under read locks and samples cursor values into an independent store. The actual recursive planner runs on that private copy; its normal cursor operations never touch the live store. Schedule and admission-rejection rules evaluate the sampled input, with no capacity reservation. Work is bounded to 256 expanded graph edges, depth 32 and 4,096 existing state rows/cursors; unavailable or over-budget reads fail visibly. Independent state fragments are not an atomic live snapshot, and no next-request target is guaranteed. No upstream request, live registration, round-robin advance, ban change or audit/request record is created by explanation.
...
Both routes answer `Cache-Control: private, no-store` with the auth/profile `Vary` set because a preview carries state-bound catalog, linked-template, and Terminal Target evidence that intermediaries must not retain. Any relevant pricing-state change invalidates the hash. Unchanged template-only reuse is deliberately idempotent with respect to template and target state, so the preview is not described as a server-enforced one-time token.

Source-linked pricing import from the fixed official models.dev catalog. Preview resolves the offering from a bound `model_config_id` or an explicit `provider_id` + `catalog_model_id` pair, builds the price plan outside any transaction, finds the single **live** source-linked template for that offering (partial unique index that excludes `deleted_at`), flags drift when the template's current shape differs from the planned shape, captures each requested Terminal Target's CAS state, and returns `preview_hash` over offering + revision + plan + template identity + target CAS columns. An optional `model_config_id` may accompany explicit coordinates; it is echoed as display-only `model` evidence and deliberately stays out of the hash because it cannot change what the commit writes. `connection_ids` may be empty, which previews and commits a template-only import that assigns nothing. The response also fixes the unit contract explicitly as `catalog_currency: "USD"` plus `pricing_unit: "PER_1M"`, never converted. The plan fails closed with stable reasons (`reporting_currency_not_usd`, `cost_missing`, `price_not_representable`, `audio_cost_present`, `multiple_tiers`, `tier_not_supported`, `legacy_tier_shape`, `tier_evidence_conflict`, `specialty_shape_mismatch`) and zero writes whenever prices are not losslessly representable; only an OpenAI single `context` tier maps its `size` verbatim onto `input_tokens_above`. A row without `cost.reasoning` plans that card's reasoning price at the row's output price, the models.dev consumer convention, while an explicit value, including zero, is kept. Representation, legacy-tier, and specialty-parity checks judge the raw rows before this fallback, so reasoning evidence on only one tiered row still fails closed as `specialty_shape_mismatch`. Commit replays everything inside one transaction against the cached snapshot (no remote I/O): a stale revision or moved state rejects with `409`; incompatible plans reject with `422` and zero writes; drift requires explicit `confirm_drift`; creating a linked template records its first `revision_source='catalog'` revision, confirmed price drift appends another carrying `catalog_revision`, and unchanged reuse creates no artificial revision. Terminal Target assignment enforces the existing double CAS (`updated_at` + `pricing_template_id`) under sorted row locks, rolling back any opened revision, mutation ledger, generation update, and earlier assignment when a later target conflicts. Template creation or drift advances `pricing_template_generation`; unchanged reuse does not. Independently, the platform route contract advances runtime planning-cache generations and schedules a refresh after every successful catalog commit, including a template-only reuse that leaves pricing state unchanged.

The template and revision read surfaces project that provenance instead of hiding it: `GET /api/pricing-templates`, `GET /api/pricing-templates/{id}`, and the bounded list page carry `catalog_provider_id` + `catalog_model_id` (both null on a manual template, and a half-populated pair fails closed as `pricing_template_catalog_evidence_incomplete`) plus the current revision's `revision_source`/`catalog_revision`, and `GET /api/pricing-templates/{id}/revisions` carries `revision_source`/`catalog_revision` per append-only revision. Because uniqueness is scoped to live rows, soft-deleting a source-linked template releases its coordinates: re-importing the same offering creates a **new** live template while the retired row, its revisions, cards, and historical references survive unchanged.

Successful catalog commits return both `template_id` and the authoritative stored `template_name`, including any collision suffix chosen during creation, so callers never announce an inferred coordinate as the saved template name.
...
| `gemini.count_tokens` | `{"totalTokens": N}` or `{"total_tokens": N}`, with optional `cachedContentTokenCount` or `cache_read_input_tokens` | Top-level count as base `input_tokens` and `total_tokens`; optional cached count is retained as cache-read input; `output_tokens` = null. Pricing selects the applicable input card and charges observed input/cache components only; absent output is zero for this operation. |
...
This is the retained REST analytics snapshot contract for API callers and debugging. The current `/observe?tab=trend` UI uses the signed query-context summary/series fragments described above rather than this snapshot polling path.
...
| `sort_by` | string | `created_at` | `created_at`, `elapsed_ms`, or `total_cost_user_currency_micros` |
| `sort_order` | string | `desc` | Metric direction within a comparable group; missing values remain last in either direction |
...
Ranked chains use the same retained cohort as totals and chain CSV. `elapsed_ms` is the floored nonnegative millisecond difference between the retained finalized event's ingress start and completion, never the sum of attempt durations. Cost is the latest retained finalized event's known `priced`/`trusted` report-currency cost, once per ingress. Comparable cost groups are the canonical `cost_segment_key` plus currency code; group ordering is an identity order, not a comparison of amounts across currencies or epochs. Each chain carries `ranking: {metric,value,group,state}`, where missing-finalized, missing-elapsed, untrusted-cost and unknown-currency states remain explicit.

The envelope's `ranking` contains `metric`, `rankable_ingress_count`, `unrankable_ingress_count` and `unrankable_reasons` (`missing_finalized`, `missing_elapsed`, `untrusted_cost`, `unknown_currency`) for the **entire filtered cohort**, including on an empty outer page. It is not a count of rows currently rendered. Cursor identity includes the metric, nullable value, comparable group and deterministic time/ingress tie-breakers alongside the existing window/filter/retention identity. Chain CSV selects that same cohort and follows its group/metric order before retained-row order, without adding synthetic parent costs or making per-attempt charges additive.
...
Every audited attempt may retain its request body; only the final attempt retains a response body. `is_stream` does not prevent response capture. Body capture disabled at request time leaves both base64 fields `null`; binary or invalid-UTF-8 prefixes remain representable without loss. The browser decodes eligible UTF-8 for its readable-conversation projection; binary and unsupported content are explained without a raw-data UI. The raw-download routes below remain backend API capabilities with their existing exact-byte contract.
...
#### 2.2B `model_pi_catalog_bindings` (management-only pi.dev export binding)

One optional binding row per model freezing the selected pi.dev candidate coordinate for the Pi client export (§1.3B) plus two independent metadata projections of its seven safe leaves. Independent of `model_catalog_bindings` above: neither table reads, rewrites, or reuses the other. This binding never participates in `api_family` compatibility truth, capability gating, routing, or the runtime snapshot; writes never invalidate planning.

| Column group | Contents | Notes |
| --- | --- | --- |
| Coordinates | `provider_id`, `catalog_model_id`, `api`, `prism_model_id_at_bind`, `bind_source`, `catalog_revision` (`sha256-…`, verified against the response body), `fetched_at` | `bind_source` is `single_candidate` or `manual`. One row per `model_config_id` (PK, FK -> model_configs ON DELETE CASCADE). `api` is CHECKed against the four Pi API literals. `prism_model_id_at_bind` is the non-null, non-empty Prism identity frozen at bind time; it is deliberately not CHECKed equal to `catalog_model_id`, because an explicit cross-directory bind makes them differ on purpose |
| `source_*` | `name` (text), `reasoning` (bool), `input` (JSONB array), `context_window`/`max_tokens` (bigint), `thinking_level_map` (JSONB object), `compat` (JSONB object), plus non-null JSONB-array `source_dropped_fields` | Replaced wholesale only by `pi/refresh/commit`; absent values stay NULL, while dropped paths remain sorted evidence and never render |
| `override_*` | The same seven fields, all nullable | Written only by `PUT .../pi/override`, validated through the same Pi 0.84.3 schema `RenderPi` enforces; refreshes never touch them; an explicit null override restores that field to its source value |
| Stamp | `updated_at` | Monotonic optimistic-lock token advanced on every actual write and required by refresh commit together with coordinate/API/revision |

Constraints: CHECKs restrict `api` to the four Pi API literals and `bind_source` to `single_candidate`/`manual`, require the metadata JSONB columns to be arrays/objects as applicable, require `source_dropped_fields` to be an array, require `prism_model_id_at_bind` to be non-empty, and keep every non-null `context_window`/`max_tokens` value positive as required by Pi 0.84.3. `000028_model_pi_binding_identity.sql` adds that snapshot column, backfills it from `catalog_model_id`, then makes it `NOT NULL`; it adds no unique constraint, so several Prism models may still reference one directory coordinate.
