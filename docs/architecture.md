...
The Requests comparison feature reuses the existing request-detail and audit-list contracts for metadata and an explicit selected-audit-detail read for payloads. Ordinary list browsing does not fetch audit bodies. It retains the audit source/direction, capture byte ledger and completeness, then compares only readable retained text with a bounded positional display. It never invokes runtime execution or reconstructs missing tool, SSE or binary content.
...
The loadbalance, pricing, and proxy-key feature data contracts keep independent state owners behind thin page coordinators. `frontend/src/features/loadbalance/useBanPolicyStrategyCollection.ts` owns strategy collection/detail reads and revision-backed fragments, `useBanPolicyMutations.ts` owns CRUD/default mutation state, and `useStrategyImpactPager.ts` owns per-strategy cursor merge and stale/error state. Pricing uses `usePricingTemplateCollection.ts` for the revision cache/list, `usePricingTemplateMutations.ts` for editor impact preflight and create/update/delete CAS workflows, `usePricingTemplateDetailReads.ts` for usage/history reads, and `usePricingImportProtocol.ts` for preview/hash-commit; `usePricingFeatureData.ts` only composes them. Proxy-key ledger queries/capacity and visible-key usage live in `useProxyKeyLedger.ts`; `useProxyKeyCreateMutation.ts`, `useProxyKeyEditMutation.ts`, `useProxyKeyRotateMutation.ts`, and `useProxyKeyDeleteMutation.ts` own the four credential mutation lifecycles, while `useProxyKeyMutations.ts` only composes their page-facing values. `proxyKeyMutationReconciliation.ts` is the explicit cache-patch/invalidation owner, `proxyKeyMutationErrors.ts` is the shared error-to-toast mapping owner, and `useProxyKeySecretSession.ts` is the only post-handoff owner of a one-time raw secret. The standing access-verification dialog open state remains page presentation state in `ProxyKeysFeaturePage.tsx`.
