...
An omitted `metric` resolves to the scope default before validation; an explicit disallowed combination (e.g., `route_attempt`+`requests`/`output_rate`/`tokens`) returns `422 metric_invalid` with no compatibility aliases. `ingress` reads one `usage_request_events` row per finalized ingress and uses requested-model identity. `final_execution` uses the finalized event's resolved leaf model and actual winning Terminal Target, with `final_attempt_latency` joined to the retained `final_attempt_number`. `route_attempt` reads only upstream `request_logs` rows and uses attempt-result semantics with `attempt_latency` per attempt and `attempts` as a single-bar count. Ingress and final-execution cost are separate, non-additive projections of the same `priced` plus `trusted` served-final fact; route attempts expose no aggregate cost. Latency P50/P95 for `ttft`, `final_attempt_latency`, and `attempt_latency` share the same percentile fields with correct Chinese semantics, and the UI keeps missing, zero, partial-sample and read-failure states distinct. Every metric envelope identifies its caliber, datasets, coverage, and sample/missing counts, while invalid or scope-incompatible group/filter keys fail with typed `422`. The Models table and Observe surfaces render these scopes without assigning health from absent samples.
...
`bind` fetches and revision-checks the catalog outside the write transaction, then re-resolves inside it: with explicit `provider_id`/`catalog_model_id` both fields are required and must be a current exact-id-plus-API candidate (`422 pi_candidate_unknown` otherwise); without them, a unique candidate applies automatically (`bind_source=single_candidate`), zero candidates reject with `422 pi_candidate_not_in_catalog`/`pi_candidate_api_mismatch`, and multiple candidates reject with `409 pi_candidate_ambiguous` plus the candidate list. Rebinding the same coordinate keeps prior overrides and the original `bind_source`; rebinding a different one clears overrides, since they described another candidate's metadata. `refresh/preview` diffs the bound coordinate's live catalog entry against the stored source snapshot field by field (`changes[]` with `added`/`removed`/`changed`) without writing; `refresh/commit` requires `expected_catalog_revision`, re-resolves the coordinate inside its own transaction (so a concurrent rebind cannot smuggle a foreign candidate's values into the row), replaces only the source columns plus revision/fetch stamp, and fails `409 pi_catalog_stale` on a revision mismatch or `409 pi_candidate_missing` when the coordinate vanished or changed API — fail closed, nothing is wiped. `override` `PUT` accepts a flat object of the seven fields, each validated through `modelexport.ValidatePiSourceField` (the same schema `RenderPi` enforces), with an explicit JSON `null` restoring that field to its source value; `DELETE /override` clears every override; `DELETE /pi` unbinds (removes the row) without touching runtime identity. All six routes are `Default-profile`, `M2`, planning-neutral.

##### Source, Render Fact Chain

Two static `Default-profile` routes share `M3`, `planning-neutral`, `private, no-store` and have no compatibility shell; the old `/api/models/exports/{platform}/*` and the interim `resolve` step are no longer mounted.

```text
GET /api/models/export/source
POST /api/models/export/render
```

`source` does one best-effort pi.dev fetch outside any transaction and one `REPEATABLE READ` DB snapshot, returning every model with two independently-computed evidence layers: live catalog evidence (`pi_candidates`/`candidate_status`: `not_in_catalog`/`api_mismatch`/`single`/`multiple`/`catalog_unavailable`) and persisted binding evidence (`pi_selected`/`pi_binding_status`: `unbound`/`bound`/`bound_drifted`, plus `pi_bind_source`/`pi_binding_catalog_revision`/`pi_binding_fetched_at`/`pi_binding_updated_at`), alongside layered `prism`/`merged` metadata and provenance, pricing risk and warnings, the pinned `0.84.3` target version, and a clock-free `source_digest`. `pi_binding_status` compares the bound coordinate against this request's live fetch only when that fetch actually succeeded; a failed or absent live fetch never asserts `bound_drifted` against a healthy binding, since it has no positive evidence of drift. `render` performs no network I/O and reads no live pi.dev state at all — no availability gate, no `LKG` dependency: it accepts only `expected_source_digest`, `model_config_ids`, `base_url`, `provider_id`, `credential`, and a `selections` coordinate assertion that must exactly equal each selected model's persisted binding, and renders exclusively from the binding's effective (source-with-override) fields. Every selected model without a matching assertion or without any binding at all fails `422` (`candidate_unselected`/`candidate_invalid`); any digest mismatch fails `409 export_source_stale` before rendering.

The `source_digest` is `SHA-256` over canonical JSON of the sorted, clock-free fact set: target version, models, targets, pricing, prism metadata, each model's bound `pi_selected` coordinate plus its `catalog_revision`, and the resulting effective candidate metadata. It deliberately excludes the current live pi.dev fetch's own revision/status/candidate evidence (`json:"-"` on `SourceFacts.PiCatalog` and `ModelFact.PiCandidates`/`PiCandidateStatus`/`PiBindingStatus` in `backend/internal/domain/modelexport/digest.go`): that transient, per-request fetch outcome never changes what gets rendered for an already-bound model, and `source`'s live fetch and `render`'s cached-only read are not guaranteed to observe the same fetch outcome even when nothing render-relevant changed — folding it into the digest would make `render` spuriously `409` after every successful `bind`. A `bind`, `rebind`, or `refresh/commit` does move the digest, because each rewrites the bound coordinate, its revision, or its effective metadata.
...
| `row_kind` | repeated string | — | Attempt-row kind filter (`planning`, `admission`, `upstream`, `legacy_unknown`); Observe route-attempt deep links pin `upstream` |
...
The list route is the slim browse contract used by `/observe/requests` and other row-summary consumers. It keeps one row per upstream attempt, returns `filter_options.endpoints` for the endpoint dropdown, `filter_options.ingress_models` for the requested-model dropdown, `filter_options.clients` for caller client filtering, and `filter_options.attempt_target_models` for retained-attempt filtering. It includes requested-model labels, final-target labels, `stream_outcome`, `stream_error_kind`, and the failure preview for display. The current request-log page uses page sizes `100`, `300`, and `500`, with `100` as the frontend default. This retained-history route is the operator drill-in surface for investigation, not a dashboard aggregate or metrics endpoint.

`filter_options` always includes `endpoints`, `ingress_models`, `clients`, and `attempt_target_models`. `filter_options.ingress_models` contains `{ ingress_model_id, model_label }` entries for requested-model filtering. `filter_options.clients` contains `{ client_rule_id, client_label }` entries built from enabled User-Agent Client Rules. `client_rule_id` filtering is caller-only and matches `caller_user_agent`; it never matches `upstream_user_agent`. `filter_options.attempt_target_models` contains `{ attempt_target_model_id, model_label }` entries for retained-attempt filtering. Empty option sets are returned as empty arrays instead of omitted fields. `ingress_request_id` groups multiple attempt rows that belong to one incoming runtime request. Row payload `model_id` stays the requested model and `resolved_target_model_id` captures the target model for that attempt, while request-log row and detail payloads use `resolved_target_model_label` for the matching display label. Row IDs are decimal strings (`request_log_id`), never JS numbers.
...
Server-side full filtered CSV export (Requests SPEC §6.8). Query parameters mirror the attempt-view filters above (`view`, `from_time`/`to_time`, `pricing_status`, `unpriced_reason`, `status_family`, `status_code`, `error_text`, `ingress_request_id`, `ingress_model_id`, `endpoint_id`, `client_rule_id`, `attempt_target_model_id`, `api_family`, `row_kind`) plus signed `query_context`/final selectors or optional `exact_request_log_ids`. The export:
...
| Column group | Contents | Notes |
| --- | --- | --- |
| Coordinates | `provider_id`, `catalog_model_id`, `api`, `bind_source`, `catalog_revision` (`sha256-…`, verified against the response body), `fetched_at` | `bind_source` is `single_candidate` or `manual`. One row per `model_config_id` (PK, FK -> model_configs ON DELETE CASCADE). `api` is CHECKed against the four Pi API literals |
| `source_*` | `name` (text), `reasoning` (bool), `input` (JSONB array), `context_window`/`max_tokens` (bigint), `thinking_level_map` (JSONB object), `compat` (JSONB object) | Replaced wholesale only by `pi/refresh/commit`; absent values stay NULL |
...
Constraints: CHECKs restrict `api` to the four Pi API literals and `bind_source` to `single_candidate`/`manual`, require the JSONB columns to be arrays/objects (as applicable) when present, and keep `context_window`/`max_tokens` non-negative.
