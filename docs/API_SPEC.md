...
Sidecar routes are global management routes. They omit `X-Profile-Id` and operate on instance-wide CLIProxyAPI registrations. Prism stores sidecar metadata, normalized inventory snapshots, watchdog policy and holds, one authoritative parent sweep per sidecar, mandatory child sweep items, pending repair actions, latest observed quota state, projection-only quota scan history, and redacted retained action history; CLIProxyAPI remains the source of truth for live auth/provider state.
...
```
`GET /quota-states` returns `{ "items": SidecarAuthQuotaState[] }`, one latest observed row per known auth file when quota inventory is enabled. The public item contract includes `auth_id`, optional `auth_name`, optional `provider`, `auth_index_present`, `disabled`, optional `current_priority`, `priority_state`, `quota_band`, optional `probe_status`, optional `reason_code`, optional `quota_reset_at`, optional `blocking_window`, optional `last_snapshot_at`, optional `last_probed_at`, optional `last_error_code`, and `active_hold`. Public quota bands are only `using`, `quota_exceeded`, and `error`; the detail fields are metadata that explain or time-bound the band without adding more public quota states. `priority_state` is a separate watchdog priority classification with four values: `working`, `empty-quota`, `initial`, and `error`. Responses do not expose raw probe payloads, provider identity payloads, cooldown timestamps, or scan position.

`GET /quota-scans` returns retained projection/history rows for initial, manual, and scheduled scan sources. `GET /quota-scans/current` normally returns `204` because executable work is no longer owned by quota scan rows. `POST /quota-scans` records a completed manual projection row and, when there is work to do, queues manual-scan child items under the authoritative watchdog parent sweep; `replace_active` supersedes the current parent sweep before replacement work is materialized. Scan responses include `scan_type`, terminal `status`, `planned_count`, `attempted_count`, `using_count`, `quota_exceeded_count`, `error_count`, `skipped_count`, optional cancellation and completion timestamps, optional `last_error_code`, and timestamps. `POST /quota-scans/{scan_id}/cancel` is idempotent for terminal projection rows and returns the stored run.
...
Watchdog policy responses expose controller metadata, `active_revision`, optional `pending_revision`, `has_pending_changes`, optional `active_sweep`, and editable fields including `enabled`, `failure_threshold`, `failure_window_seconds`, `fallback_cooldown_seconds`, `watchdog_sweep_interval_seconds`, `using_priority`, `quota_exceeded_priority`, `working_priority`, `empty_quota_priority`, `initial_priority`, `error_priority`, `manual_override_pause_seconds`, `probe_concurrency`, `probe_timeout_seconds`, `probe_batch_cooldown_seconds`, `probe_jitter_min_ms`, `probe_jitter_max_ms`, `cooldown_jitter_percent`, `quota_inventory_enabled`, `initial_scan_enabled`, `rolling_refresh_enabled`, and `rolling_refresh_after_seconds`. `active_sweep` includes `sweep_id`, `status`, `policy_revision_id`, `started_at`, and `progress`; `progress` includes `total_items`, `pending_items`, `active_items`, `succeeded_items`, `failed_items`, `cancelled_items`, `superseded_items`, and `terminal_items` from mandatory child sweep items.

`PUT` replaces editable fields and `PATCH` accepts a partial update, but both only save a pending immutable revision. `POST /watchdog-policy/apply` promotes the expected pending revision to active for future sweeps and leaves an already-running parent sweep pinned to its starting revision with unchanged child work. `POST /watchdog-policy/apply-and-restart` promotes the expected pending revision and supersedes current parent and child work so replacement work starts under a new parent pinned to the promoted revision. Stale `expected_revision_id` values return `409 Conflict` on save and apply routes.

Positive integer thresholds, windows, cooldowns, concurrency budgets, timeouts, and refresh windows are required when present. Priority validation keeps the four priority bands ordered so `working_priority` is highest, `empty_quota_priority` is next, `initial_priority` is next, and `error_priority` is lowest; legacy `using_priority` and `quota_exceeded_priority` remain response aliases for the working and empty-quota bands. Jitter validation requires non-negative probe jitter bounds, `probe_jitter_max_ms >= probe_jitter_min_ms`, and `cooldown_jitter_percent` between `0` and `100`. Probe budget validation keeps `probe_concurrency` between 1 and 8 and `probe_timeout_seconds` between 1 and 120 seconds; the bounded watchdog worker uses a 125-second timeout with a fixed 5-second safety margin. The frontend label for `probe_concurrency` is `Probe batch size`; it limits a single sweep batch. `probe_batch_cooldown_seconds` paces batches inside an active sweep, while `watchdog_sweep_interval_seconds` is the rest interval before another completed sweep can be followed by a new one.

The sidecar watchdog uses probe-first quota authority. Recovery and quota decisions come from active CLIProxyAPI `/api-call` probes, not from stored snapshot `quota_*` fields. Auto initial inventory, manual scan, rolling refresh, and due-hold probe sources are materialized as child sweep items under the one authoritative parent sweep, not as executable quota scan rows. In v1, Prism only builds quota probes for `codex` and `chatgpt` provider keys, using a wrapped `GET https://chatgpt.com/backend-api/wham/usage` request through CLIProxyAPI with token substitution handled by the sidecar. Unsupported providers are skipped with an explicit probe action outcome.

Action-history responses read retained `sidecar_watchdog_actions` rows and redact token, secret, password, and authorization-like text. Pending repairs live separately in `sidecar_watchdog_pending_actions` until workers claim and finish them. Action rows preserve raw `previous_priority` and `target_priority`, add derived `previous_priority_state` and `target_priority_state`, and expose `mutation_outcome` values such as `patched`, `already_at_target`, `skipped`, and `failed` so clients do not infer no-op behavior from raw priorities. Probe action types include `probe_succeeded`, `probe_failed_timeout`, `probe_failed_management_auth`, `probe_failed_token`, `probe_failed_status`, `probe_failed_parse`, `probe_failed_transport`, and `probe_skipped_unsupported_provider`. When a probe confirms quota is still blocking an auth file, the public action type is `quota_hold_extended`. Probe observations remain an internal sanitized store record and public API responses do not expose raw probe payloads, provider identity payloads, cooldown timestamps, or scan position.
