...
- Eligible Chat Completions and Responses requests, both streaming and non-stream, can promote before dispatch when tokenizer-backed estimation is present, the estimate proves the request exceeds the source target's usable context window, the source model has an explicit `context_overflow_promotion_target_id`, the same estimate fits the promoted target, and native or sibling OpenAI text translation compatibility is valid. Prism builds the ordinary source plan but opens zero source upstream requests, preserves the request streaming mode, executes one promoted attempt, and records `trigger_phase=pre_dispatch_estimate`, `source_attempt_count=0`, and `final_attempt_count=1`.
