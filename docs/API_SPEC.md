...
Watchdog policy responses expose controller metadata, `active_revision`, optional `pending_revision`, `has_pending_changes`, optional `active_sweep`, and editable fields including `enabled`, `failure_threshold`, `failure_window_seconds`, `fallback_cooldown_seconds`, `watchdog_sweep_interval_seconds`, `using_priority`, `quota_exceeded_priority`, `working_priority`, `empty_quota_priority`, `initial_priority`, `error_priority`, `manual_override_pause_seconds`, `probe_concurrency`, `probe_timeout_seconds`, `probe_batch_cooldown_seconds`, `probe_jitter_min_ms`, `probe_jitter_max_ms`, `cooldown_jitter_percent`, `quota_inventory_enabled`, `initial_scan_enabled`, `rolling_refresh_enabled`, and `rolling_refresh_after_seconds`. `PUT` replaces editable fields and `PATCH` accepts a partial update, but both create a pending immutable revision instead of changing the active runtime policy. `POST /watchdog-policy/apply` promotes the expected pending revision to active; stale `expected_revision_id` values return `409 Conflict`. Active sweeps stay pinned to their starting revision, and the response marks a restart request when a newer active revision waits for the next sweep.
...
The sidecar watchdog uses probe-first quota authority. Recovery and quota decisions come from active CLIProxyAPI `/api-call` probes, not from stored snapshot `quota_*` fields. Auto initial scans and rolling refresh remain enabled by policy fields and run as quota scan work, separate from sweep interval and batch cooldown timing. In v1, Prism only builds quota probes for `codex` and `chatgpt` provider keys, using a wrapped `GET https://chatgpt.com/backend-api/wham/usage` request through CLIProxyAPI with token substitution handled by the sidecar. Unsupported providers are skipped with an explicit probe action outcome.
