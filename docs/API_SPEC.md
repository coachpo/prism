...
Positive integer thresholds, windows, cooldowns, concurrency budgets, timeouts, and refresh windows are required when present. Priority validation keeps the four priority bands ordered so `working_priority` is highest, `empty_quota_priority` is next, `initial_priority` is next, and `error_priority` is lowest; legacy `using_priority` and `quota_exceeded_priority` remain response aliases for the working and empty-quota bands. Jitter validation requires non-negative probe jitter bounds, `probe_jitter_max_ms >= probe_jitter_min_ms`, and `cooldown_jitter_percent` between `0` and `100`. Probe budget validation keeps `probe_concurrency` and `probe_timeout_seconds` within the bounded watchdog worker budget. The frontend label for `probe_concurrency` is `Probe batch size`; it limits a single sweep batch. `probe_batch_cooldown_seconds` paces batches inside an active sweep, while `watchdog_sweep_interval_seconds` is the rest interval before another completed sweep can be followed by a new one.
