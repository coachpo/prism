...
- `POST /api/models/{model_config_id}/pi/bind` freezes a coordinate (explicit, or the unique candidate when exactly one exists) after checking the caller's `expected_catalog_revision` against a fresh fetch; rebinding to a different coordinate clears prior overrides, while repeating the same coordinate is idempotent so only refresh may advance its frozen source. `POST .../pi/refresh/preview` returns a field/dropped-path diff plus coordinate, API, `binding_updated_at`, and catalog-revision tokens; `.../pi/refresh/commit` must match all of them under row lock before replacing only the source fields and dropped evidence, leaving overrides untouched. `PUT`/`DELETE .../pi/override` write and clear sparse per-field manual overrides validated against the bound API's same Pi 0.84.3 schema; `DELETE .../pi` unbinds. All six are `M2`, planning-neutral.
- `source` does one best-effort live pi.dev fetch for discovery evidence only (`pi_candidates`/`candidate_status`: `not_in_catalog`/`api_mismatch`/`single`/`multiple`/`catalog_unavailable`) and separately reports each model's persisted binding (`pi_selected`, `pi_binding_status`: `unbound`/`bound`/`bound_drifted`, `pi_binding_renderable`, source/override/effective metadata, and dropped-field evidence). Live coordinate/template drift is visible but does not invalidate a compatible frozen binding; a Prism full-model-ID or final-Pi-API mismatch sets `pi_binding_renderable=false`. `render` performs no network I/O and reads no live or LKG catalog state: it accepts `expected_source_digest`, `model_config_ids`, `base_url`, `provider_id`, `credential`, and exactly one `selections` coordinate assertion per selected model, each matching its persisted binding, then renders only from the effective frozen fields. The clock-free `source_digest` covers the bound coordinate, its revision, effective template, and dropped evidence per model—never the current live-fetch outcome. Failures are `409 export_source_stale` on digest drift, `422` on an unbound/incompatible/non-matching selection, and `422`/`409` on bind/refresh conflicts.
