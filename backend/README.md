...
- Startup fields include listener host and port, CORS origins, auth TTL and cookie metadata, M2/M3 management admission limits, database URL and pool budgets, runtime side-effects attempt timeout, runtime secret encryption key, auth JWT signing key, and parse-compatible telemetry fields. Legacy `mail` fields remain parse-compatible only. The `runtime.transport` section was removed outright: outbound provider requests carry no connection or timeout limits, and a leftover `runtime.transport` block fails startup with a readable migration error.
...
- Raw bootstrap files require `runtime.sideEffects.attemptTimeout` as a Go duration string. Fresh seeds set it to `"10s"`. The `runtime.transport` section no longer exists; a leftover `runtime.transport` block fails startup with a readable migration error.
- `runtime.sideEffects.attemptTimeout` is the runtime side-effect enqueue attempt budget. It changes only after editing `config.json` and restarting Prism.
