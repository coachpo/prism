...
- Config export/import now uses split bundles: profile bundles are `version: 3` with `bundle_kind: profile_config`, `vendor_refs`, `profile_settings`, encrypted `secret_payload`, and nullable `api_key_secret_ref`; vendor-catalog bundles remain `version: 2` with authoritative vendor metadata.
