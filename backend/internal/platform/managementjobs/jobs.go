...
func NewStore(options Options) *Store {
	...
	return &Store{pool: options.Pool, scheduler: options.Scheduler, workerID: workerID, now: now, batchSize: defaultBatchSize}
}
...
func (s *Store) processAuditDelete(ctx context.Context, job Job) error {
	deleted, lastID, err := s.deleteAuditChunk(ctx, job)
	if err != nil {
		_, _ = s.pool.Exec(ctx, `UPDATE management_jobs SET attempt_count = LEAST(attempt_count + 1, max_attempts), state = CASE WHEN attempt_count + 1 >= max_attempts THEN 'failed' ELSE 'queued' END, error_code = 'transient_delete_error', error_message = $2, next_attempt_at = CASE WHEN attempt_count + 1 >= max_attempts THEN next_attempt_at ELSE now() + interval '5 seconds' END, finished_at = CASE WHEN attempt_count + 1 >= max_attempts THEN now() ELSE finished_at END, locked_by = NULL, locked_until = NULL, updated_at = now() WHERE id = $1`, job.ID, err.Error())
		return err
	}
	if deleted > 0 {
		if _, err := s.pool.Exec(ctx, `UPDATE management_jobs SET rows_deleted = rows_deleted + $2, batches_completed = batches_completed + 1, progress_json = jsonb_build_object('last_cursor', $3::text), last_heartbeat_at = now(), locked_until = now() + interval '2 minutes', updated_at = now(), state = CASE WHEN cancel_requested THEN 'cancelled' ELSE 'queued' END, finished_at = CASE WHEN cancel_requested THEN now() ELSE finished_at END WHERE id = $1`, job.ID, deleted, fmt.Sprintf("%d", lastID)); err != nil {
			return fmt.Errorf("update management job chunk progress: %w", err)
		}
		_ = s.appendEvent(ctx, job.ID, "chunk", "audit delete chunk completed", deleted)
		_ = s.Wake(context.Background())
		return nil
	}
	_, err = s.pool.Exec(ctx, `UPDATE management_jobs SET state = CASE WHEN cancel_requested THEN 'cancelled' ELSE 'succeeded' END, finished_at = now(), locked_by = NULL, locked_until = NULL, last_heartbeat_at = now(), updated_at = now() WHERE id = $1`, job.ID)
	_ = s.appendEvent(ctx, job.ID, "finished", "audit delete job finished", 0)
	return err
}

func (s *Store) deleteAuditChunk(ctx context.Context, job Job) (int64, int64, error) {
	var deleted int64
	var lastID int64
	where := "profile_id = $1"
	args := []any{jobScopeProfile(job)}
	if !job.Scope.DeleteAll && job.Scope.Before != nil {
		args = append(args, job.Scope.Before.UTC())
		where += fmt.Sprintf(" AND created_at < $%d", len(args))
	}
	args = append(args, s.batchSize)
	query := `WITH doomed AS (SELECT id FROM audit_logs WHERE ` + where + ` ORDER BY created_at ASC, id ASC LIMIT $` + fmt.Sprintf("%d", len(args)) + `) DELETE FROM audit_logs USING doomed WHERE audit_logs.id = doomed.id RETURNING audit_logs.id`
	...
	return deleted, lastID, rows.Err()
}
...
func jobScopeProfile(job Job) int { return job.ProfileID }
