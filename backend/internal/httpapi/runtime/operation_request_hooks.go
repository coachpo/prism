...
var operationRequestHooksByCollectionID = map[string]operationRequestHooks{
	...
	// The image operations opt into streaming through the body's `stream` flag
	// and emit partial-image events until their terminal event. They carry no
	// text generation parameters (no temperature, top_p or max output tokens),
	// so no buffered generation-params extractor is registered.
