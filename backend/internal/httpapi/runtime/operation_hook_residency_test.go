...
func TestRuntimeOperationHookResidency(t *testing.T) {
	tests := []struct {
		...
		{
			name:                 "openai image generations streaming",
			requestPath:          "/v1/images/generations",
			hookCollectionID:     runtimeHookCollectionOpenAIImagesGeneration,
			provider:             "openai",
			rawBody:              []byte(`{"model":"gpt-image-2","prompt":"hidden","stream":true,"partial_images":2}`),
			wantRequestStream:    true,
			wantGenerationStatus: requestGenerationParamsStatusMissing,
			wantResponseKind:     operationResponseKindImageGeneration,
			wantUsageRule:        runtimeUsageRuleOpenAIImages,
			wantStreamHooks:      true,
		},
		{
			name:                 "openai image edits non streaming",
			requestPath:          "/v1/images/edits",
			hookCollectionID:     runtimeHookCollectionOpenAIImagesEdit,
			provider:             "openai",
			rawBody:              []byte(`{"model":"gpt-image-2","prompt":"hidden","images":[{"file_id":"file-1"}]}`),
			wantRequestStream:    false,
			wantGenerationStatus: requestGenerationParamsStatusMissing,
			wantResponseKind:     operationResponseKindImageGeneration,
			wantUsageRule:        runtimeUsageRuleOpenAIImages,
			// Edits register stream hooks even though this body does not opt in:
			// the caller chooses per request through the body's `stream` flag.
			wantStreamHooks: true,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assertStreamHookResidency(t, operation, test.provider, test.wantResponseKind, test.wantRequestStream, test.wantStreamHooks, test.wantUsageRule)
		})
	}
...
func assertResponseHookResidency(t *testing.T, operation RuntimeOperation, provider string, wantKind operationResponseKind, wantUsageRule runtimeUsageNormalizationRule) {
	...
	// Usage normalization belongs to the generation seams only. Text generation
	// and image generation both produce provider usage; token-count operations
	// must keep usage normalization out of their response hooks.
	if wantKind == operationResponseKindTextGeneration || wantKind == operationResponseKindImageGeneration {
...
func assertStreamHookResidency(t *testing.T, operation RuntimeOperation, provider string, wantKind operationResponseKind, isStreamingRequest bool, wantStream bool, wantUsageRule runtimeUsageNormalizationRule) {
	t.Helper()
	...
	if selected.Provider != provider || selected.Kind != wantKind {
		t.Fatalf("expected %s/%s stream hooks for %s, got %+v", provider, wantKind, operation.Name, selected)
	}
