...
func validateRuntimeRoutingPlanTerminalTargetIssues(issues []runtimeRoutingPlanValidationIssue, path string, connectionID int, connection runtimeConnection) []runtimeRoutingPlanValidationIssue {
	...
	if providerauth.IsOpenAI(connection.APIFamily) {
		issues = validateRuntimeRoutingPlanOpenAICapabilities(connection, path, issues)
	}
	return issues
}

// validateRuntimeRoutingPlanOpenAICapabilities checks both OpenAI dimensions of
// a Terminal Target. Each dimension is optional on its own but must be a
// supported value when present, and an OpenAI target that declares neither
// dimension can serve nothing and is rejected.
func validateRuntimeRoutingPlanOpenAICapabilities(connection runtimeConnection, path string, issues []runtimeRoutingPlanValidationIssue) []runtimeRoutingPlanValidationIssue {
	if connection.OpenAITextCapability != nil && !providerauth.IsSupportedOpenAITextCapability(*connection.OpenAITextCapability) {
		issues = appendRuntimeRoutingPlanValidationIssue(issues, "terminal_target_openai_text_capability_invalid", path+".openai_text_capability", fmt.Sprintf("terminal target %d has unsupported OpenAI text capability %q", connection.ID, *connection.OpenAITextCapability))
	}
	if connection.OpenAIImageCapability != nil && !providerauth.IsSupportedOpenAIImageCapability(*connection.OpenAIImageCapability) {
		issues = appendRuntimeRoutingPlanValidationIssue(issues, "terminal_target_openai_image_capability_invalid", path+".openai_image_capability", fmt.Sprintf("terminal target %d has unsupported OpenAI image capability %q", connection.ID, *connection.OpenAIImageCapability))
	}
	if connection.OpenAITextCapability == nil && connection.OpenAIImageCapability == nil {
		issues = appendRuntimeRoutingPlanValidationIssue(issues, "terminal_target_openai_capabilities_absent", path+".openai_text_capability", fmt.Sprintf("terminal target %d declares neither an OpenAI text nor an OpenAI image capability", connection.ID))
	}
	return issues
}
