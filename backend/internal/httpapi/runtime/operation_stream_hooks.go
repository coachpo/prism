...
var operationStreamHooksByCollectionID = map[string]operationStreamHooks{
	...
	runtimeHookCollectionOpenAIImagesGeneration: {
		...
		ClassifyTerminalSignal: classifyOpenAIImagesGenerationStreamTerminal,
		MergeUsage:             mergeOpenAIImagesStreamUsage,
	},
	runtimeHookCollectionOpenAIImagesEdit: {
		Provider:               "openai",
		Kind:                   operationResponseKindImageGeneration,
		UsageRule:              runtimeUsageRuleOpenAIImages,
		ClassifyTerminalSignal: classifyOpenAIImagesEditStreamTerminal,
		MergeUsage:             mergeOpenAIImagesStreamUsage,
	},
	"anthropic.messages": {
		Provider:               "anthropic",
...
// Image streams emit `*.partial_image` events until a single terminal
// `*.completed` event that carries the final image and the usage object. There
// is no incomplete or failed terminal event in the image stream contract, so
// only the completed signal is classified; an aborted stream is classified by
// the shared SSE outcome logic instead.
const (
	openAIImagesGenerationCompletedEvent = "image_generation.completed"
	openAIImagesEditCompletedEvent       = "image_edit.completed"
)

func classifyOpenAIImagesGenerationStreamTerminal(event string, payload map[string]any) sseTerminalSignal {
	return classifyOpenAIImagesStreamTerminal(openAIImagesGenerationCompletedEvent, event, payload)
}

func classifyOpenAIImagesEditStreamTerminal(event string, payload map[string]any) sseTerminalSignal {
	return classifyOpenAIImagesStreamTerminal(openAIImagesEditCompletedEvent, event, payload)
}

func classifyOpenAIImagesStreamTerminal(completedEvent string, event string, payload map[string]any) sseTerminalSignal {
	if isOpenAIImagesCompletedStreamEvent(completedEvent, event, payload) {
		return sseTerminalSignalCompleted
	}
	return sseTerminalSignalNone
}

// The terminal event is recognised from the SSE `event:` name or, when the
// upstream omits it, from the payload's own `type` discriminator.
func isOpenAIImagesCompletedStreamEvent(completedEvent string, event string, payload map[string]any) bool {
	if event == completedEvent {
		return true
	}
	payloadType, _ := payload["type"].(string)
	return payloadType == completedEvent
}

// mergeOpenAIImagesStreamUsage reads usage from either image terminal event.
// Both operations share one merger because ImagesUsage is identical in shape
// and only the event name differs; partial-image events carry no usage.
func mergeOpenAIImagesStreamUsage(rule runtimeUsageNormalizationRule, usage *responseUsage, event string, payload map[string]any) {
	if !isOpenAIImagesCompletedStreamEvent(openAIImagesGenerationCompletedEvent, event, payload) &&
		!isOpenAIImagesCompletedStreamEvent(openAIImagesEditCompletedEvent, event, payload) {
		return
	}
