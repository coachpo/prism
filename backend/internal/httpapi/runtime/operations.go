...
const (
	...
	runtimeHookCollectionOpenAIImagesGeneration     = "image.openai_images_generation"
	runtimeHookCollectionOpenAIImagesEdit           = "image.openai_images_edit"
)
...
var runtimeOperationCatalog = []RuntimeOperation{
	...
	// Image operations bind their model from the JSON body like the text
	// operations do. Streaming stays false at the registry level because the
	// caller opts in per request through the body's `stream` flag, exactly as
	// Chat Completions and Responses do; only Gemini declares streaming in the
	// path itself.
