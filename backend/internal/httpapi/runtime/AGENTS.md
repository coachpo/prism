...
- Frozen Terminal Target `upstream_model_id` (migration `000031`): the planning snapshot loads only owner-backed active connections and requires a non-blank identity on each runtime connection (`planning_terminal_target_adapter.go`, `planning_snapshot_records.go`); a missing value rejects the snapshot so hot refresh keeps the last-good generation, while orphan rows never enter runtime. `attemptUpstreamModelID` in `runtime_planning.go` and the provider bridges plus default body/path rewrite consume the explicit value verbatim. Logical attribution (`ResolvedTargetModelID`, pricing model id), capability checks, and routing stay on `attempt.TargetModel`; telemetry snapshots freeze `attempt.Connection.UpstreamModelID` onto every upstream row and the winner usage event (`request_log_rows.go`, `usage_event_row.go`, `telemetry_failure_envelopes.go`)
...
- Its exported route-witness projection is read-only catalog data for management analyzers; it must not alter matching or let management widen the runtime operation allowlist.
