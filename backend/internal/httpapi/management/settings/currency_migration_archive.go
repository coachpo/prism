...
func (s *Service) loadArchiveCurrencyMigrationProjection(ctx context.Context, tx pgx.Tx, profileID int, request currencyMigrationDraftPreviewRequest, forUpdate bool) (archiveCurrencyMigrationProjection, error) {
	...
	var nonReady int
	if err := tx.QueryRow(ctx, `SELECT count(*) FROM pricing_templates AS templates LEFT JOIN pricing_template_revisions AS revisions ON revisions.id = templates.current_revision_id WHERE templates.profile_id = $1 AND templates.deleted_at IS NULL AND (revisions.id IS NULL OR revisions.currency_code IS DISTINCT FROM $2 OR revisions.reporting_currency_epoch_id IS DISTINCT FROM $3 OR revisions.currency_attribution <> 'active_epoch' OR revisions.template_kind NOT IN ('standard','tiered','peak_valley') OR NOT EXISTS (SELECT 1 FROM unnest(CASE revisions.template_kind WHEN 'standard' THEN ARRAY['standard']::text[] WHEN 'tiered' THEN ARRAY['tier_base','tier_above']::text[] ELSE ARRAY['peak','offpeak']::text[] END) AS required(role) WHERE NOT EXISTS (SELECT 1 FROM pricing_template_cards cards WHERE cards.revision_id = revisions.id AND cards.card_role = required.role AND cards.input_price IS NOT NULL AND cards.output_price IS NOT NULL AND cards.cached_input_price IS NOT NULL AND cards.cache_creation_price IS NOT NULL AND cards.reasoning_price IS NOT NULL)) OR (revisions.template_kind = 'peak_valley' AND (revisions.pricing_schedule_timezone IS NULL OR revisions.pricing_schedule_digest IS NULL OR NOT EXISTS (SELECT 1 FROM pricing_template_windows windows WHERE windows.revision_id = revisions.id))))`, profileID, settings.ReportCurrencyCode, activeEpochID).Scan(&nonReady); err != nil {
		return archiveCurrencyMigrationProjection{}, err
	}
	if nonReady != 0 {
