...
type Service struct {
	...
	corsOriginProvider platformcors.OriginProvider
	instanceID         string
	eventsContextCodec *eventsQueryContextCodec
}
...
func NewService(settings config.Settings, options Options) (*Service, error) {
	...
	return &Service{pool: pool, ownsPool: ownsPool, now: now, runtimeState: options.RuntimeState, corsOriginProvider: corsOriginProvider, instanceID: randomInstanceID(), eventsContextCodec: newEventsQueryContextCodec(settings.SecretEncryptionKey, now)}, nil
}
...
func (s *Service) MountManagementRoutes(api chi.Router) {
	api.Route("/loadbalance", func(router chi.Router) {
		...
		router.Post("/strategies/preview", s.handlePreviewStrategy)
		router.Get("/strategies/{strategy_id}", s.handleGetStrategy)
		router.Put("/strategies/{strategy_id}", s.handleUpdateStrategy)
		router.Put("/strategies/{strategy_id}/default", s.handleSetStrategyDefault)
		router.Get("/strategies/{strategy_id}/models", s.handleListStrategyModels)
		...
		router.Post("/events/query-context", s.handleIssueEventsQueryContext)
	})
}
