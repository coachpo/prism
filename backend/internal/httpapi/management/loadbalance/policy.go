...
func canonicalDefaultStrategySpecs() []loadbalancedomain.CanonicalStrategySpec {
	return loadbalancedomain.CanonicalDefaultStrategySpecs()
}

func defaultStrategyPayload(spec loadbalancedomain.CanonicalStrategySpec) loadbalancedomain.RuntimeStrategy {
	return loadbalancedomain.DefaultStrategyPayload(spec)
}

func strategyPayloadFromRuntimeStrategy(strategy loadbalancedomain.RuntimeStrategy) strategyPersistedPayload {
	legacyStrategyType := ""
...
func canonicalizeStrategyRequest(requestBody loadbalanceStrategyRequest) (strategyPersistedPayload, error) {
	...
	return canonicalizeStrategyPolicyFields(requestBody)
}

// canonicalizeStrategyPolicyFields applies the exact same validation and
// normalization used by create/update, without the name presence requirement
// (the preview endpoint accepts an unsaved draft whose name may be omitted).
func canonicalizeStrategyPolicyFields(requestBody loadbalanceStrategyRequest) (strategyPersistedPayload, error) {
...
func strategyMatchesCanonicalDefault(response loadbalanceStrategyResponse, expected loadbalancedomain.CanonicalStrategySpec) bool {
	strategy := loadbalancedomain.RuntimeStrategy{
		...
		RetryJitterRatio:                   response.RetryJitterRatio,
		RetryMaxDelayMS:                    response.RetryMaxDelayMS,
		CycleRetryAttemptLimit:             response.CycleRetryAttemptLimit,
		BanCumulativeRetryAttemptThreshold: response.BanCumulativeRetryAttemptThreshold,
		BanDurationSeconds:                 response.BanDurationSeconds,
	}
	return loadbalancedomain.StrategyMatchesCanonical(strategy, expected)
}
