...
- `export_opencode_*` reads shared model/routing/price facts plus only supported persisted models.dev source/override leaves in one read-only REPEATABLE READ snapshot. Source and render never fetch catalogs or read Pi bindings. Keep its digest independent of Pi and unused catalog facts; require valid positive context/output limits and use the runtime matcher for path-bound model identity eligibility.
...
Use `routes_test.go` and `export_pi_binding_boundaries_test.go` for local seams. Database-backed graph, catalog CAS, and export behavior belong in [model](../../../../tests/contract/model_contract_test.go), [catalog concurrency](../../../../tests/contract/catalog_concurrency_contract_test.go), [Pi export](../../../../tests/contract/model_export_contract_test.go), and [OpenCode export](../../../../tests/contract/model_export_opencode_contract_test.go) contract tests.
