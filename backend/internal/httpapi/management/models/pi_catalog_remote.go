...
func (s *Service) piCatalogForRead(ctx context.Context) (*pidev.Catalog, string, string) {
	if s.piCatalog == nil {
	...
	if snap := s.piCatalog.Snapshot(); snap != nil {
		return snap, "stale", pidev.FailureCode(err)
	}
	return nil, "unavailable", pidev.FailureCode(err)
}
