...
func listEndpointDirectReferences(ctx context.Context, tx pgx.Tx, profileID int, endpointIDs []int) (map[int][]EndpointDirectReference, error) {
	...
	rows, err := tx.Query(ctx, `WITH ranked AS (
		SELECT connections.endpoint_id, connections.id AS connection_id, model_access_targets.id AS access_target_id, connections.name,
			...
			connections.custom_request_parameters
		FROM connections
	...
	SELECT endpoint_id, connection_id, access_target_id, name, model_config_id, model_id, display_name, api_family,
		...
		pricing_template_id, pricing_name, custom_headers, custom_request_parameters
	FROM ranked
	...
	for rows.Next() {
		...
		var customRequestParametersRaw sql.NullString
		if err := rows.Scan(&endpointID, &connectionID, &accessTargetID, &terminalTargetName, &reference.ModelConfigID, &reference.ModelID, &modelDisplayName, &reference.APIFamily, &reference.AuthoredStagePosition, &reference.IsEnabled, &reference.IsActive, &openAITextCapability, &pricingTemplateID, &pricingName, &customHeadersRaw, &customRequestParametersRaw); err != nil {
		...
		reference.CustomRequestParameterCount = customHeaderCount(customRequestParametersRaw)
