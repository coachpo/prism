...
func endpointsRouteSeedRichConnectionUsage(t *testing.T, ctx context.Context, conn *pgx.Conn, profileID int, endpointID int, now time.Time) (int, int) {
	...
	if err := conn.QueryRow(ctx, `INSERT INTO connections (profile_id, api_family, endpoint_id, pricing_template_id, qps_limit, max_in_flight_non_stream, max_in_flight_stream, openai_text_capability, is_active, priority, name, auth_type, custom_headers, health_status, health_detail, last_health_check, created_at, updated_at) VALUES ($1, 'openai', $2, $3, 7, 3, 2, 'responses_only', TRUE, 42, 'boundary-terminal', 'bearer', '{"X-Boundary":"kept"}', 'degraded', 'sticky health detail', $4, $5, $5) RETURNING id`, profileID, endpointID, pricingID, lastHealthAt, now).Scan(&connectionID); err != nil {
