...
- Source uniqueness is scoped to live rows (`uq_pricing_templates_catalog_offering ... AND deleted_at IS NULL`, migration `000028`), so a soft-deleted source template releases its coordinates for a new live import while the retired row and its history survive. Template and revision reads project that provenance (`catalog_provider_id`/`catalog_model_id`, `revision_source`/`catalog_revision`) and fail closed on a half-populated coordinate pair as `pricing_template_catalog_evidence_incomplete`.
