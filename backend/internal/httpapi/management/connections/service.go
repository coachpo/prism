...
// domainError is kept as a package-local alias for pricing handlers that use
// the historical lower-case spelling. It is the same HTTP error contract.
...
func (s *Service) MountManagementRoutes(api chi.Router) {
	...
	api.Route("/pricing-templates", func(router chi.Router) {
		...
		router.Post("/catalog/preview", catalogPricingPrivateNoStore(s.handleCatalogPricingPreview))
		router.Post("/catalog/commit", catalogPricingPrivateNoStore(s.handleCatalogPricingCommit))
		...
		router.Get("/{template_id}/revisions", s.handleListPricingTemplateRevisions)
		router.Get("/{template_id}/impact", s.handleGetPricingTemplateImpact)
