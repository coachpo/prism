...
```
auth/
├── runtime_probe.go               # Runtime probe and request IP extraction
├── request_helpers.go             # JSON decoding and no-store response helpers
...
- Keep auth-control writes transactional and replayable: the operation result is persisted with the initiating browser operation id, the effective pointer update remains the transaction's linearization point, and a lost response is recovered by replay rather than a second mutation.
