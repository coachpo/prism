...
func GenerateOpenAIWarningsForTarget(ownerAcceptedFormat string, targetCapability string, path string, modelConfigID int, accessTargetID int, connectionID int) []ConfigurationWarning {
	return GenerateOpenAIWarningsForTargetDimensions(&ownerAcceptedFormat, nil, &targetCapability, nil, path, modelConfigID, accessTargetID, connectionID)
}

// GenerateOpenAIWarningsForTargetDimensions is the both-dimension form. Text
// and image operations are classified together against one target so a single
// warning reports every accepted operation the target leaves unserved.
func GenerateOpenAIWarningsForTargetDimensions(ownerAcceptedFormat *string, ownerImageOperations *string, targetCapability *string, targetImageCapability *string, path string, modelConfigID int, accessTargetID int, connectionID int) []ConfigurationWarning {
	accepted := OpenAIAcceptedOperationSetForDimensions(ownerAcceptedFormat, ownerImageOperations)
	if len(accepted) == 0 {
		return nil
	}
	supported := OpenAITargetSupportedOperationSetForDimensions(targetCapability, targetImageCapability)
...
func BuildRoutingSummary(graph *DiagnosticsGraph, root DiagnosticsModel, result DiagnosticsResult) RoutingSummary {
	...
	for _, group := range OpenAIOperationGroupOrder() {
		groupStatus[group] = GroupStatusNotAccepted
	}
	routableCount := 0
	acceptedCount := 0
	for _, coverage := range result.OperationCoverage {
