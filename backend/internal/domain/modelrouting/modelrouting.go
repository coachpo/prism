...
const (
	...
	// OpenAIImageUncoveredIssueCode is emitted when an OpenAI relation would
	// leave an accepted image operation unserved by the target.
	OpenAIImageUncoveredIssueCode = "target_openai_image_uncovered"
)
...
func ResolveAuthoredAccessTargets(targets []AuthoredAccessTarget, options ResolveOptions) ([]ResolvedAccessTarget, []ValidationIssue) {
	...
	for _, target := range orderedTargets {
		...
		case IsModelTargetType(target.TargetType):
			...
			if openAIImageCapabilityUncovered(options.Source.OpenAIImageOperations, model.OpenAIImageOperations) {
				return nil, appendResolveIssue(issues, options, OpenAIImageUncoveredIssueCode, "target_model_id", target)
			}
		...
		case IsTerminalTargetType(target.TargetType):
			...
			if openAIImageCapabilityUncovered(options.Source.OpenAIImageOperations, terminal.OpenAIImageCapability) {
				return nil, appendResolveIssue(issues, options, OpenAIImageUncoveredIssueCode, terminalTargetFieldFromResolve(options), target)
			}
...
// openAIImageCapabilityUncovered reports whether a relation would leave one of
// the source's accepted image operations unserved. A source that accepts no
// image operation imposes no image requirement, and a target that serves more
// image operations than the source accepts stays valid: image coverage is
// containment, not the strict equality the text dimension requires.
func openAIImageCapabilityUncovered(sourceImageOperations *string, targetImageCapability *string) bool {
	if sourceImageOperations == nil || strings.TrimSpace(*sourceImageOperations) == "" {
		return false
	}
	return !providerauth.OpenAIImageCapabilitiesCover(sourceImageOperations, targetImageCapability)
}
