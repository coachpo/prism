...
// ResolveQueryBounds resolves a preset or custom window against the owning
// usage retention source. A nil floor means the policy does not impose a
// logical cutoff; it must not be replaced with a product default.
func ResolveQueryBounds(preset string, fromTime *time.Time, toTime *time.Time, referenceNow time.Time, usageRetentionFrom *time.Time) (QueryBounds, error) {
	normalizedPreset := strings.TrimSpace(preset)
	if normalizedPreset == "" {
		normalizedPreset = "24h"
	}
	...
	case "1h", "6h", "24h", "7d", "30d":
		duration := presetDuration(normalizedPreset)
		start := referenceNow.UTC().Add(-duration)
		requestedFrom = &start
		end := referenceNow.UTC()
		requestedTo = &end
	case "all":
		// The current raw read model has a bounded 30-day query surface. This
		// bound is an owner capability, not a retention default; a nil policy
		// floor remains nil for coverage and clipping.
		start := referenceNow.UTC().Add(-30 * 24 * time.Hour)
		if usageRetentionFrom != nil {
			start = *usageRetentionFrom
		}
		requestedFrom = &start
		end := referenceNow.UTC()
		requestedTo = &end
	case "custom":
		if fromTime == nil || toTime == nil {
			return QueryBounds{}, &HTTPError{StatusCode: 422, Detail: "preset=custom requires from_time and to_time"}
		}
		start := fromTime.UTC()
		end := toTime.UTC()
		if !end.After(start) {
			return QueryBounds{}, &HTTPError{StatusCode: 422, Detail: "invalid_time_range"}
		}
		if end.Sub(start) > 30*24*time.Hour {
			return QueryBounds{}, &HTTPError{StatusCode: 422, Detail: "custom range cannot exceed 30 days"}
		}
		requestedFrom = &start
		requestedTo = &end
	default:
		return QueryBounds{}, &HTTPError{StatusCode: 422, Detail: fmt.Sprintf("unknown preset %q", normalizedPreset)}
	}

	usageFrom := *requestedFrom
	usageTo := *requestedTo
	gaps := make([]CoverageGap, 0, 1)
	retentionFrom := usageRetentionFrom
	complete := true
	if retentionFrom != nil && usageFrom.Before(*retentionFrom) {
		gaps = append(gaps, CoverageGap{FromTime: usageFrom, ToTime: *retentionFrom, Reason: "retention_deleted"})
		usageFrom = *retentionFrom
		complete = false
	}
	if !usageFrom.Before(usageTo) {
		return QueryBounds{}, &HTTPError{StatusCode: 422, Detail: "invalid_time_range"}
	}
	return QueryBounds{
		RequestedPreset:    normalizedPreset,
