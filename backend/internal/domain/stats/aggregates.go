...
func GetConnectionSuccessRates(ctx context.Context, exec queryExecutor, params ConnectionSuccessRateParams) ([]ConnectionSuccessRate, error) {
	...
	for rows.Next() {
		...
		var statusCode int
		if err := rows.Scan(&connectionID, &statusCode); err != nil {
...
func buildConnectionSuccessRatesQuery(params ConnectionSuccessRateParams) (string, []any) {
	...
	return `SELECT connection_id, status_code FROM request_logs WHERE ` + strings.Join(clauses, " AND "), args
}
...
func loadSummaryRequestLogRows(ctx context.Context, exec queryExecutor, params StatsSummaryParams) ([]summaryRequestLogRow, error) {
	...
	rows, err := exec.Query(ctx, `SELECT created_at, model_id, api_family, endpoint_base_url, endpoint_id, connection_id, status_code, response_time_ms, input_tokens, output_tokens, total_tokens FROM request_logs WHERE `+strings.Join(clauses, " AND "), args...)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.CreatedAt, &item.ModelID, &item.APIFamily, &endpointBaseURL, &endpointID, &connectionID, &item.StatusCode, &item.ResponseTimeMS, &inputTokens, &outputTokens, &totalTokens); err != nil {
...
func loadModelMetricSummaryRows(ctx context.Context, exec queryExecutor, profileID int, modelIDs []string, fromTime time.Time) ([]summaryRequestLogRow, error) {
	...
	rows, err := exec.Query(ctx, `SELECT created_at, model_id, api_family, endpoint_base_url, endpoint_id, connection_id, status_code, response_time_ms, input_tokens, output_tokens, total_tokens FROM request_logs WHERE profile_id = $1 AND created_at >= $2 AND model_id IN (`+strings.Join(placeholders, ", ")+`)`, args...)
	if err != nil {
