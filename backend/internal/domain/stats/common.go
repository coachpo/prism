...
func resolveSpendingEndpointLabel(current endpointRecord, currentFound bool, endpointID *int) string {
	if currentFound {
		return resolveEndpointLabel(current.Name, current.BaseURL, nil, endpointID, "unknown_endpoint")
	}
	if endpointID != nil {
		return fmt.Sprintf("Endpoint %d", *endpointID)
	}
...
func loadCompiledUserAgentRules(ctx context.Context, exec queryExecutor, profileID int) ([]compiledUserAgentRule, error) {
	rows, err := exec.Query(ctx, `SELECT name, pattern FROM user_agent_client_rules WHERE enabled = TRUE AND (profile_id = $1 OR is_system = TRUE) ORDER BY is_system ASC, id ASC`, profileID)
	if err != nil {
	...
	for rows.Next() {
		...
		items = append(items, compiledUserAgentRule{Name: name, Pattern: compiled})
	}
	if err := rows.Err(); err != nil {
