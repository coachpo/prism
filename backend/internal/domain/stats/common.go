...
// scopedRequestLogDurationSQL resolves the row-scoped duration: real attempt
// wall-clock for upstream rows, the legacy mixed projection otherwise.
const scopedRequestLogDurationSQL = `CASE WHEN row_kind = 'upstream' THEN attempt_duration_ms ELSE legacy_duration_ms END`
