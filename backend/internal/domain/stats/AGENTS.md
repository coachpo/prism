...
├── observe_attempt_errors.go     # Route-attempt error aggregation and Top-N/Other
├── observe_attempt_error_filters.go # Route-attempt SQL/replay-filter projection
├── observe_attempt_outcome.go    # Shared retained-attempt outcome classifier
...
- Keep request-log filtering aligned with current product semantics: `ingress_model_id` selects the requested entry model, `attempt_target_model_id` selects the retained attempt target, and signed `final_target_model_id` selects the finalized owner. Same-key lists are OR, cross-key predicates are AND, and `__null__` explicitly matches unattributed values. `client_rule_id` matches caller User-Agent Client Rules only; `row_kind` preserves the planning/admission/upstream/legacy boundary and route-attempt deep links pin `upstream`. `pricing_status` is the four-state pricing filter, while the retired `priced` alias rejects as an unknown query key.
...
- Bind chain/row cursors to the complete normalized cohort/window as well as profile, ordering, page size and retention generation. Chain items, totals and CSV must select the same ingress cohort and count complete retained chains.
- Keep CSV export server-side from a single `READ ONLY REPEATABLE READ` snapshot with typed rejection before any file bytes. Attempts and chain views own distinct cohort projections; exactly 31 days is export-only, while exact ingress is retained-all.
...
- Route-attempt Errors and series classify upstream attempts through the shared attempt outcome owner, preserve non-null replay filters, calculate Top-N/Other from the full denominator, and resolve labels without publishing unexplained raw IDs. Unsupported endpoint/scope group or filter combinations reject instead of degrading to all/Other.
