...
- Keep request-log filtering aligned with current product semantics: `ingress_model_id` is requested identity, `attempt_target_model_id` is retained-attempt identity, and signed `final_target_model_id` is finalized-owner identity. Same-key lists are OR, cross-key predicates are AND, and `__null__` is explicit unattributed matching. `client_rule_id` matches caller User-Agent only; the retired `priced` alias rejects.
