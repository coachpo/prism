...
├── pidev/          # HTTP-neutral pi.dev catalog client/schema: trusted revision/LKG, exact candidates, bounded model-id search
...
- Keep the model-routing graph complete for recursive Model Targets while exposing a separate direct-entry witness projection. `direct_request_enabled` gates client roots and representatives only; it never removes a configured node from graph resolution or diagnostics.
