...
func (preset presetPolicyFields) strategy() RuntimeStrategy {
	return RuntimeStrategy{
		...
		FailureStatusCodes:                 append([]int(nil), 403, 422, 429, 500, 502, 503, 504, 529),
...
var threePresets = []presetPolicyFields{
	{
		name: "conservative", baseDelayMS: 120000, backoffMultiplier: 2.0, jitterRatio: 0.2,
		maxDelayMS: 1800000, cycleLimit: 2, banMode: "temporary", banThreshold: 4, banDurationSeconds: 3600,
	},
	{
		name: "balanced", baseDelayMS: 60000, backoffMultiplier: 2.0, jitterRatio: 0.2,
		maxDelayMS: 900000, cycleLimit: 3, banMode: "off", banThreshold: 0, banDurationSeconds: 0,
	},
	{
		name: "aggressive", baseDelayMS: 10000, backoffMultiplier: 1.5, jitterRatio: 0.2,
...
func TestThreePresetsGoldenPayloads(t *testing.T) {
	...
	for _, preset := range threePresets {
		...
		if !equalIntSlicesForTest(strategy.FailureStatusCodes, []int{403, 422, 429, 500, 502, 503, 504, 529}) {
...
func TestThreePresetsPreviewGolden(t *testing.T) {
	// Conservative: 2-minute base, 2x backoff, 20% jitter, 30-minute cap,
	...
	if conservative.Steps[0].NominalDelayMS != 120000 || conservative.Steps[0].JitterMinDelayMS != 96000 || conservative.Steps[0].JitterMaxDelayMS != 144000 {
		t.Fatalf("unexpected conservative first step %+v", conservative.Steps[0])
	}
	if conservative.Steps[1].NominalDelayMS != 240000 || conservative.Steps[1].JitterMaxDelayMS != 288000 {
	...
	// Aggressive: 10-second base, 1.5x backoff, 20% jitter, 2-minute cap,
	...
	wantNominal := []int{10000, 15000, 22500, 33750, 50625}
