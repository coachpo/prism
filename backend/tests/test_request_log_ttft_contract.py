...
def test_nullable_historical_ttft_fields() -> None:
    request_log_ttft = RequestLog.__table__.c["ttft_ms"]
    usage_event_ttft = UsageRequestEvent.__table__.c["ttft_ms"]

    assert request_log_ttft.nullable is True
    assert usage_event_ttft.nullable is True

    request_log_payload = RequestLogResponse.model_validate(
        _make_request_log(ttft_ms=None, completion_duration_ms=654)
    ).model_dump()
    detail_payload = RequestLogDetailResponse.from_request_log(
        _make_request_log(ttft_ms=None, completion_duration_ms=654)
    ).model_dump()
    detail_payload_with_null_duration = RequestLogDetailResponse.from_request_log(
        _make_request_log(ttft_ms=None, completion_duration_ms=None)
    ).model_dump()

    assert request_log_payload["ttft_ms"] is None
    assert request_log_payload["response_time_ms"] == 321
    assert request_log_payload["completion_duration_ms"] == 654
    assert detail_payload["summary"]["ttft_ms"] is None
    assert detail_payload["summary"]["response_time_ms"] == 321
    assert detail_payload["summary"]["completion_duration_ms"] == 654
    assert (
        detail_payload_with_null_duration["summary"]["completion_duration_ms"] is None
    )


def test_list_contract_exposes_ttft() -> None:
    payload = RequestLogListItemResponse.model_validate(
        _make_request_log(ttft_ms=123, completion_duration_ms=987)
    ).model_dump()
...
def test_detail_contract_exposes_ttft() -> None:
    payload = RequestLogDetailResponse.from_request_log(
        _make_request_log(ttft_ms=123, completion_duration_ms=987)
    ).model_dump()

    assert payload["summary"]["ttft_ms"] == 123
