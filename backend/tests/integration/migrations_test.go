...
var expectedPrismMigrationVersions = []string{
	...
	"000002_runtime_latency_semantics",
}
...
func TestRuntimeLatencySemanticsMigrationPreservesValues(t *testing.T) {
	...
	// rename the column back to its historical name and un-stamp 000002 so the
	...
	if _, err := conn.Exec(testContext, `DELETE FROM prism_schema_migrations WHERE version = '000002_runtime_latency_semantics'`); err != nil {
		t.Fatalf("un-stamp 000002 history for legacy simulation: %v", err)
	}
	...
	assertMigrationVersions(t, "upgraded versions", upgradeResult.Versions, []string{"000002_runtime_latency_semantics"})
