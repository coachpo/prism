...
var expectedPrismMigrationVersions = []string{
	...
	"000003_endpoint_reference_metadata",
}
...
func revertEndpointReferenceMetadataMigration(t *testing.T, ctx context.Context, conn *pgx.Conn) {
	t.Helper()
	if _, err := conn.Exec(ctx, `DELETE FROM prism_schema_migrations WHERE version = '000003_endpoint_reference_metadata'`); err != nil {
		t.Fatalf("un-stamp 000003: %v", err)
	}
	if _, err := conn.Exec(ctx, `ALTER TABLE endpoints DROP COLUMN api_key_fingerprint, DROP COLUMN api_key_updated_at, DROP COLUMN config_revision`); err != nil {
		t.Fatalf("revert 000003 metadata columns: %v", err)
	}
...
func TestEndpointReferenceMetadataUpgradePath(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("apply 000003 upgrade: %v", err)
	}
	if upgradeResult.Outcome != migrate.OutcomeApply {
		t.Fatalf("expected upgrade run to apply 000003, got %q", upgradeResult.Outcome)
	}
...
func TestEndpointNameNarrowingPreflightRejectsOverlongNames(t *testing.T) {
	...
	// The 000003 preflight must fail the upgrade and report the violation
