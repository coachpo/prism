...
// catalog-import revision. It must survive the 000028 upgrade byte-for-byte.
...
// 000028_pricing_catalog_source_live_uniqueness is data-preserving: it rewrites
...
func TestPricingCatalogSourceIndexUpgradePreservesRetainedData(t *testing.T) {
	...
	// Re-create the exact pre-000028 retained shape: the uniqueness guard only
	// required both coordinates to be present, so a soft-deleted template kept
	// claiming its offering forever.
	if _, err := conn.Exec(testContext, `DELETE FROM prism_schema_migrations WHERE version = '000028_pricing_catalog_source_live_uniqueness'`); err != nil {
	...
	if upgradeResult.Outcome != migrate.OutcomeApply {
		t.Fatalf("expected 000028 to apply, got %q", upgradeResult.Outcome)
	}
