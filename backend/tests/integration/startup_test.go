...
func assertObservedStepOrder(t *testing.T, steps []startup.Step) {
	...
	want := []startup.Step{
		...
		startup.StepSettingsSchemaFinalize,
	}
...
func snapshotStartupState(t *testing.T, ctx context.Context, conn *pgx.Conn) string {
	...
	return dumpRows(t, ctx, conn, []rowDumpQuery{
		...
		{"app_auth_settings", `SELECT singleton_key, email_verification_attempt_count, must_change_password, created_at, updated_at FROM app_auth_settings ORDER BY id ASC`},
