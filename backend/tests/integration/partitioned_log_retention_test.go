...
func TestScheduledGlobalLogRetentionProcessesStoredSettings(t *testing.T) {
	...
	// clock past the protection deadline.
	processingStore := managementjobs.NewStore(managementjobs.Options{Pool: pool, LogRetention: retentionStore, Now: func() time.Time { return retentionNow.Add(72 * time.Hour) }})
	for range logretention.ManagedTables() {
		if err := processingStore.ProcessDue(testContext); err != nil {
	...
	dayAlignedCutoff := time.Date(2026, time.February, 18, 0, 0, 0, 0, time.UTC)
	for _, tableName := range logretention.ManagedTables() {
		task9AssertPostCutoffRetentionWithFutureRows(t, testContext, pool, tableName, baseDay, dayAlignedCutoff, 1, 2)
...
func task9InsertManagedLogRow(t *testing.T, ctx context.Context, exec task9Exec, tableName string, profileID int, marker string, rowIndex int, createdAt time.Time) {
	...
	switch tableName {
	case "request_logs":
		_, err := exec.Exec(ctx, `INSERT INTO request_logs (profile_id, model_id, api_family, row_kind, url_scrub_provenance, upstream_status_code, attempt_duration_ms, is_stream, success_flag, pricing_status, pricing_evidence_trust, request_path, created_at) VALUES ($1, 'task9-model', 'openai', 'upstream', 'runtime_scrubbed', 200, 25, FALSE, TRUE, 'ineligible', 'trusted', $2, $3)`, profileID, "/v1/task9/"+marker, createdAt.UTC())
		if err != nil {
	...
	case "usage_request_events":
		_, err := exec.Exec(ctx, `INSERT INTO usage_request_events (profile_id, ingress_request_id, model_id, api_family, endpoint_label_snapshot, status_code, success_flag, pricing_status, pricing_evidence_trust, attempt_count, request_path, created_at) VALUES ($1, $2, 'task9-model', 'openai', 'task9 retained endpoint', 200, TRUE, 'ineligible', 'trusted', 1, '/v1/task9', $3)`, profileID, marker, createdAt.UTC())
		if err != nil {
...
func task9RunLogRetentionJob(t *testing.T, ctx context.Context, store *managementjobs.Store, tableName string, cutoff *time.Time, deleteAll bool, expectedRowsDeleted int64) managementjobs.RetentionJobSummaryDTO {
	...
	completed, err := store.GetGlobalRetentionJobDetailDTO(ctx, job.ID)
	if err != nil {
		t.Fatalf("load completed log retention job %s: %v", job.ID, err)
	}
	if completed.Job.State != "succeeded" {
		t.Fatalf("expected %s log retention job %s to succeed, got %s", tableName, job.ID, completed.Job.State)
	}
	if completed.Job.Progress.BoundaryRowsDeleted != fmt.Sprintf("%d", expectedRowsDeleted) {
		t.Fatalf("expected %s job %s boundary_rows_deleted=%d, got %s", tableName, job.ID, expectedRowsDeleted, completed.Job.Progress.BoundaryRowsDeleted)
	}
...
func task9AssertScheduledRetentionJobSucceeded(t *testing.T, ctx context.Context, queryer task9QueryRower, tableName string) {
	...
	err := queryer.QueryRow(ctx, `
		...
		WHERE type = 'log_retention'
		  ...
		  AND idempotency_key LIKE $1 || ':2026-02-18:%'
...
func task9AssertPostCutoffRetention(t *testing.T, ctx context.Context, pool *pgxpool.Pool, tableName string, baseDay time.Time, cutoff time.Time) {
	t.Helper()
	task9AssertPostCutoffRetentionWithFutureRows(t, ctx, pool, tableName, baseDay, cutoff, task9ExpectedRuntimeRows(tableName)+1, 1)
}

func task9AssertPostCutoffRetentionWithFutureRows(t *testing.T, ctx context.Context, pool *pgxpool.Pool, tableName string, baseDay time.Time, cutoff time.Time, futureRetainedRows int, boundaryRetainedRows int) {
	t.Helper()
...
func task9AssertDeleteAllRebootstrap(t *testing.T, ctx context.Context, pool *pgxpool.Pool, retentionStore *logretention.Store, tableName string, retentionNow time.Time) {
	...
	if len(partitions) != logretention.HorizonDays() {
		t.Fatalf("expected %d %s partitions after delete-all (current/future horizon intact), got %+v", logretention.HorizonDays(), tableName, partitions)
	}
	// v2 delete-all drops only complete past partitions (canonical time before
	// the frozen purge_to_time) and never rebuilds the horizon: the surviving
	// horizon still starts at the app bootstrap day (real clock), not the
	// frozen test clock.
	expectedStart := task9UTCDay(time.Now().UTC())
...
func task9InsertRuntimeEndpoint(t *testing.T, ctx context.Context, exec task9QueryRower, profileID int, name string, baseURL string, apiKey string, now time.Time) int {
	t.Helper()
	var endpointID int
	if err := exec.QueryRow(ctx, `INSERT INTO endpoints (profile_id, name, base_url, api_key, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $5) RETURNING id`, profileID, name, baseURL, apiKey, now).Scan(&endpointID); err != nil {
