...
// The text-mode preflight is strict equality over the text dimension only.
// Relaxing openai_accepted_format to nullable for image-only models must not
// make those models look like violations: two rows that both carry no text mode
// are equal, not distinct.
func TestOpenAIModePreflightIgnoresImageOnlyRows(t *testing.T) {
	testContext, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()

	harness := newPostgresHarness(t)
	databaseName := "openai_mode_preflight_image_only"
	conn := harness.openEmptyDatabase(t, testContext, databaseName)
	defer func() { _ = conn.Close(testContext) }()
	...
	strategyID := preflightInsertStrategy(t, testContext, conn, profileID)
	sourceID := preflightInsertImageOnlyModel(t, testContext, conn, profileID, "preflight-image-source", "generations_and_edits", strategyID)
	targetID := preflightInsertImageOnlyModel(t, testContext, conn, profileID, "preflight-image-target", "generations_and_edits", strategyID)
	preflightInsertModelTarget(t, testContext, conn, profileID, sourceID, targetID, 0, true)
	endpointID := preflightInsertEndpoint(t, testContext, conn, profileID, "preflight-image-endpoint")
	connectionID := preflightInsertImageOnlyConnection(t, testContext, conn, profileID, sourceID, endpointID, "generations_and_edits", "preflight-image-endpoint")
	preflightInsertConnectionTarget(t, testContext, conn, profileID, sourceID, connectionID, 1, true)

	report, err := openaimodecheck.Check(testContext, conn, profileID)
	if err != nil {
		t.Fatalf("run openai mode preflight check on image-only state: %v", err)
	}
	if len(report.Violations) != 0 {
		t.Fatalf("expected image-only rows to produce no text-mode violations, got %+v", report)
	}

	if _, err := service.RunWithConn(testContext, conn); err != nil {
		t.Fatalf("expected image-only startup to pass the text-mode fail-fast, got %v", err)
	}
}

func preflightInsertImageOnlyModel(t *testing.T, ctx context.Context, conn *pgx.Conn, profileID int, modelID string, imageOperations string, strategyID int) int {
	t.Helper()
	now := time.Now().UTC()
	var modelConfigID int
	if err := conn.QueryRow(ctx, `INSERT INTO model_configs (profile_id, api_family, model_id, display_name, loadbalance_strategy_id, openai_accepted_format, openai_image_operations, is_enabled, created_at, updated_at) VALUES ($1, 'openai', $2, $2, $3, NULL, $4, TRUE, $5, $5) RETURNING id`, profileID, modelID, strategyID, imageOperations, now).Scan(&modelConfigID); err != nil {
		t.Fatalf("insert preflight image-only model %q: %v", modelID, err)
	}
	return modelConfigID
}

func preflightInsertImageOnlyConnection(t *testing.T, ctx context.Context, conn *pgx.Conn, profileID int, ownerModelConfigID int, endpointID int, imageCapability string, name string) int {
	t.Helper()
	now := time.Now().UTC()
	var connectionID int
	if err := conn.QueryRow(ctx, `INSERT INTO connections (profile_id, api_family, endpoint_id, openai_text_capability, openai_image_capability, is_active, priority, name, health_status, created_at, updated_at) VALUES ($1, 'openai', $2, NULL, $3, TRUE, 0, $4, 'unknown', $5, $5) RETURNING id`, profileID, endpointID, imageCapability, name, now).Scan(&connectionID); err != nil {
		t.Fatalf("insert preflight image-only connection %q: %v", name, err)
	}
	return connectionID
}
