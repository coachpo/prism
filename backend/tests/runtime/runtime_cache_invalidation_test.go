...
func runtimeAuthCacheInvalidationAfterAuthDisable(t *testing.T) {
	...
	authSettingsResponse := harness.requestJSON(t, http.MethodGet, "/api/settings/auth", nil, nil)
	assertStatus(t, authSettingsResponse, http.StatusOK)
	var authSettings map[string]any
	decodeJSONResponse(t, authSettingsResponse, &authSettings)
	disableResponse := harness.requestJSON(
		t,
		http.MethodPut,
		"/api/settings/auth",
		map[string]any{
			"operation_id":         "runtime-disable-" + randomSuffix(),
			"expected_revision":    fmt.Sprint(authSettings["revision"]),
			"desired_auth_enabled": false,
			"account_change":       map[string]any{"kind": "preserve"},
			"acknowledgements":     map[string]any{"disable_to_permissive_access": true},
		},
		nil,
	)
...
func runtimePlanningCacheInvalidationAfterAuditSettingsWrite(t *testing.T) {
	...
	generation := harness.runtimeCache.PublishedGeneration()
	auditGetResponse := harness.requestJSON(t, http.MethodGet, "/api/settings/audit", nil, runtimeModelHeader(profileID))
	assertStatus(t, auditGetResponse, http.StatusOK)
	var auditSettings map[string]any
	decodeJSONResponse(t, auditGetResponse, &auditSettings)
	updateResponse := harness.requestJSON(t, http.MethodPut, "/api/settings/audit", map[string]any{
		"operation_id":      "runtime-audit-" + randomSuffix(),
		"expected_revision": fmt.Sprint(auditSettings["revision"]),
		"policies": []map[string]any{
			{"family": "openai", "mode": "metadata_only"},
...
func runtimePlanningCacheInvalidationAfterOwnerScopedConnectionRoutes(t *testing.T) {
	...
	generation := harness.runtimeCache.PublishedGeneration()
	createResponse := harness.requestJSON(t, http.MethodPost, fmt.Sprintf("/api/models/%d/connections", ownerModelConfigID), map[string]any{"endpoint_id": createEndpointID, "name": "cache owner created", "is_active": true, "openai_text_capability": "dual_native"}, runtimeModelHeader(profileID))
	assertStatus(t, createResponse, http.StatusCreated)
	harness.waitForRuntimeSnapshotGeneration(t, generation)
	createPayload := decodeConnectionMutationConnection(t, createResponse)
...
func seedRuntimeVerifiedAuthSettings(t *testing.T, harness *runtimeHarness, username string, password string, _ string) {
	t.Helper()
	harness.seedRuntimeAuthConfig(t, username, password)
}
