...
func TestRuntimeAdmissionUsesLocalStateOnly(t *testing.T) {
	...
	rejectedEndpointID := harness.seedEndpoint(t, profileID, "phase2-admission-rejected-"+suffix, harness.upstream.baseURL("/phase2/admission/rejected"), "phase2-admission-rejected-key")
	eligibleEndpointID := harness.seedEndpoint(t, profileID, "phase2-admission-eligible-"+suffix, harness.upstream.baseURL("/phase2/admission/eligible"), "phase2-admission-eligible-key")
...
func TestRuntimeRoundRobinUsesLocalCursorOnly(t *testing.T) {
	...
	harness.seedProxyTarget(t, publicModelConfigID, targetModelConfigID)
	firstEndpointID := harness.seedEndpoint(t, profileID, "phase2-round-robin-first-"+suffix, harness.upstream.baseURL("/phase2/round-robin/first"), "phase2-round-robin-first-key")
	secondEndpointID := harness.seedEndpoint(t, profileID, "phase2-round-robin-second-"+suffix, harness.upstream.baseURL("/phase2/round-robin/second"), "phase2-round-robin-second-key")
...
func TestRuntimeCircuitRecoveryUsesLocalStateOnly(t *testing.T) {
	...
	publicModelConfigID := harness.seedModel(t, profileID, "openai", publicModelID, "proxy", nil)
	harness.seedProxyTarget(t, publicModelConfigID, targetModelConfigID)
	endpointID := harness.seedEndpoint(t, profileID, "phase2-circuit-endpoint-"+suffix, harness.upstream.baseURL("/phase2/circuit/recovery"), "phase2-circuit-key")
