...
func TestOpenAIModelsAPIListsActiveEnabledOpenAIModelsOnly(t *testing.T) {
	...
	for _, path := range []string{"/v1/models", "/v1/models?client_version=0.143.0"} {
		t.Run(path, func(t *testing.T) {
			headers := map[string]string(nil)
			if path != "/v1/models" {
				headers = map[string]string{"If-None-Match": `W/"stale-codex-etag"`}
			}
			response := harness.requestJSON(t, http.MethodGet, path, nil, headers)
			...
			if etag := response.Header.Get("ETag"); etag != "" {
				t.Fatalf("expected /v1/models not to emit an ETag, got %q", etag)
			}
			var payload struct {
