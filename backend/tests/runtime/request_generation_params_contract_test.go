...
func TestRuntimeRequestGenerationParamsMalformedAndFailoverClone(t *testing.T) {
	...
	t.Run("FailoverAttemptsCarryEquivalentParams", func(t *testing.T) {
		...
		primaryEndpointID := harness.seedEndpoint(t, profileID, "generation-primary", primary.baseURL("/primary"), "primary-key")
		secondaryEndpointID := harness.seedEndpoint(t, profileID, "generation-secondary", secondary.baseURL("/secondary"), "secondary-key")
...
func seedGeminiMultiConnectionRoute(t *testing.T, harness *runtimeHarness, profileID int, primaryURL string, secondaryURL string) seededRuntimeRoute {
	...
	primaryEndpointID := harness.seedEndpoint(t, profileID, "generation-gemini-buffered-primary-"+suffix, primaryURL, "generation-gemini-buffered-primary-key")
	secondaryEndpointID := harness.seedEndpoint(t, profileID, "generation-gemini-buffered-secondary-"+suffix, secondaryURL, "generation-gemini-buffered-secondary-key")
...
func seedOpenAIProxyRoute(t *testing.T, harness *runtimeHarness, profileID int, publicModelPrefix string, targetModelPrefix string, endpointBaseURL string, endpointAPIKey string, openAITextCapability string) seededRuntimeRoute {
	...
	endpointID := harness.seedEndpoint(t, profileID, publicModelPrefix+"-endpoint-"+suffix, endpointBaseURL, endpointAPIKey)
