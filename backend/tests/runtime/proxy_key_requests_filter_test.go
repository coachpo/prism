...
func seedProxyKeyAttributedRequests(t *testing.T, harness *requestLogContractHarness, profileID int) {
	t.Helper()
	// requestLogContractHarness fixes the stats reference clock at this instant;
	// keep seeded rows in that same deterministic window instead of using the
	// wall clock, which makes the filter contract depend on the day the suite
	// happens to run.
	...
	// Ensure partitions for every UTC day spanned by the seeded rows so the
	// suite stays green across day boundaries (rows up to now-6h may fall on
	// the previous calendar day).
	seenDays := map[string]struct{}{}
	for _, row := range rows {
		dayKey := row.createdAt.UTC().Format("20060102")
		if _, ok := seenDays[dayKey]; ok {
			continue
		}
		seenDays[dayKey] = struct{}{}
		ensureRuntimeTestLogPartitions(t, harness.databaseName, runtimeTestLogPartitionFor("request_logs", row.createdAt))
	}

	base := `INSERT INTO request_logs (id, profile_id, model_id, api_family, status_code, response_time_ms, row_kind, url_scrub_provenance, upstream_status_code, attempt_duration_ms, is_stream, success_flag, request_path, created_at, ingress_request_id, attempt_number, proxy_api_key_id_snapshot, proxy_api_key_name_snapshot, proxy_api_key_attribution_state, proxy_api_key_auth_enforced_at_request, pricing_status, pricing_evidence_trust) VALUES ($1, $2, 'model-a', 'openai', 200, 10, 'upstream', 'runtime_scrubbed', 200, 10, FALSE, TRUE, '/v1/chat/completions', $3, $4, 1, $5, $6, $7, $8, 'ineligible', 'trusted')`
	for _, row := range rows {
...
func TestRequestLogsProxyAPIKeyFilterOptionsContract(t *testing.T) {
	harness := newRequestLogContractHarness(t)
	profileID := loadRuntimeDefaultProfileID(t, harness)
	if _, err := harness.conn.Exec(context.Background(), `INSERT INTO proxy_api_keys (id, name, key_prefix, key_hash, last_four, is_active, created_at, updated_at) VALUES (200, 'always-current', 'pm-current', $1, '0000', TRUE, $2, $2)`, strings.Repeat("c", 64), time.Date(2026, 4, 19, 12, 0, 0, 0, time.UTC)); err != nil {
...
func legacyRequestLogsIngressChainViewEXISTSSemantics(t *testing.T) {
	harness := newRequestLogContractHarness(t)
	profileID := loadRuntimeDefaultProfileID(t, harness)
	seedProxyKeyAttributedRequests(t, harness, profileID)

	// The retained ingress-chain view is window-scoped; proxy-key attribution
	// itself is covered by the flat attempts view above.
	now := time.Date(2026, 4, 19, 12, 0, 0, 0, time.UTC)
	chainWindow := "&from_time=" + url.QueryEscape(now.Add(-150*time.Minute).Format(time.RFC3339)) + "&to_time=" + url.QueryEscape(now.Add(time.Hour).Format(time.RFC3339))
	response := harness.requestJSON(t, http.MethodGet, "/api/stats/requests?view=ingress_chains&chain_limit=50"+chainWindow, nil, runtimeModelHeader(profileID))
	assertStatus(t, response, http.StatusOK)
	var payload map[string]any
	decodeJSONResponse(t, response, &payload)
	if payload["retained_ingress_total"] != float64(2) {
		t.Fatalf("expected retained_ingress_total=2 for selected time window, got %+v", payload)
	}
	...
	for _, ingress := range []string{"ingress-current-1", "ingress-current-2"} {
		...
		rows := chain["retained_rows"].([]any)
		if len(rows) != 1 || jsonInt(t, chain["retained_request_log_row_count"]) != 1 {
			t.Fatalf("expected one retained row in chain %s, got %+v", ingress, chain)
		}
		if asMapRuntime(t, rows[0])["row_kind"] != "upstream" {
			t.Fatalf("expected upstream retained row, got %+v", rows[0])
		}
	}

	// A chain containing a second retained sibling remains a full chain.
	if _, err := harness.conn.Exec(context.Background(), `INSERT INTO request_logs (id, profile_id, model_id, api_family, status_code, upstream_status_code, response_time_ms, is_stream, success_flag, request_path, url_scrub_provenance, created_at, ingress_request_id, attempt_number, proxy_api_key_id_snapshot, proxy_api_key_name_snapshot, proxy_api_key_attribution_state, proxy_api_key_auth_enforced_at_request, pricing_status, pricing_evidence_trust) VALUES (306, $1, 'model-a', 'openai', 500, 500, 10, FALSE, FALSE, '/v1/chat/completions', 'runtime_scrubbed', $2, 'ingress-current-1', 2, NULL, NULL, 'none', FALSE, 'ineligible', 'trusted')`, profileID, now.Add(-30*time.Minute)); err != nil {
		t.Fatalf("seed sibling chain row: %v", err)
	}
	siblingResponse := harness.requestJSON(t, http.MethodGet, "/api/stats/requests?view=ingress_chains&chain_limit=50"+chainWindow, nil, runtimeModelHeader(profileID))
	...
	rows := siblingChain["retained_rows"].([]any)
	if len(rows) != 2 || jsonInt(t, siblingChain["retained_request_log_row_count"]) != 2 {
	...
	pageOneResponse := harness.requestJSON(t, http.MethodGet, "/api/stats/requests?view=ingress_chains&chain_limit=1"+chainWindow, nil, runtimeModelHeader(profileID))
	...
	pageTwoResponse := harness.requestJSON(t, http.MethodGet, "/api/stats/requests?view=ingress_chains&chain_limit=1&chain_cursor="+url.QueryEscape(chainCursor)+chainWindow, nil, runtimeModelHeader(profileID))
...
func TestRequestLogsDeepLinkRoundTripsProxyAPIKeyID(t *testing.T) {
	...
	referenceNow := time.Date(2026, 4, 19, 12, 0, 0, 0, time.UTC)
	response := harness.requestJSON(t, http.MethodGet, fmt.Sprintf("/api/stats/requests?proxy_api_key_id=%d&from_time=%s&to_time=%s", 201, referenceNow.Add(-24*time.Hour).Format(time.RFC3339), referenceNow.Add(time.Hour).Format(time.RFC3339)), nil, runtimeModelHeader(profileID))
	assertStatus(t, response, http.StatusOK)
	var payload map[string]any
	decodeJSONResponse(t, response, &payload)
	if payload["total"] != float64(2) {
