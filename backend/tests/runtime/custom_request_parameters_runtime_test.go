...
func TestCustomRequestParametersFailoverPerAttemptIsolation(t *testing.T) {
	...
	primaryEndpointID := harness.seedEndpoint(t, profileID, "custom-params-primary-endpoint-"+suffix, primaryUpstream.baseURL("/custom-params/failover/primary"), "primary-key")
	secondaryEndpointID := harness.seedEndpoint(t, profileID, "custom-params-secondary-endpoint-"+suffix, secondaryUpstream.baseURL("/custom-params/failover/secondary"), "secondary-key")
...
func TestCustomRequestParametersSequentialCandidateIsolation(t *testing.T) {
	...
	publicModelConfigID := harness.seedModel(t, profileID, "openai", publicModelID, "proxy", nil)
	harness.seedProxyTarget(t, publicModelConfigID, targetModelConfigID)
	firstEndpointID := harness.seedEndpoint(t, profileID, "custom-params-candidate-first-endpoint-"+suffix, firstUpstream.baseURL("/custom-params/candidate/first"), "first-key")
	secondEndpointID := harness.seedEndpoint(t, profileID, "custom-params-candidate-second-endpoint-"+suffix, secondUpstream.baseURL("/custom-params/candidate/second"), "second-key")
...
func waitForRuntimeAuditRequestBody(t *testing.T, harness *runtimeHarness, profileID int, target *sqlNullStringForTest) {
	...
	var auditEnabled, auditCapture bool
	var auditCount int
	_ = harness.conn.QueryRow(context.Background(), `SELECT audit_enabled_at_request, audit_capture_bodies_at_request FROM request_logs WHERE profile_id = $1 ORDER BY id DESC LIMIT 1`, profileID).Scan(&auditEnabled, &auditCapture)
	_ = harness.conn.QueryRow(context.Background(), `SELECT COUNT(*) FROM audit_logs WHERE profile_id = $1`, profileID).Scan(&auditCount)
	t.Fatalf("audit request body never became available for profile %d (request snapshot enabled=%v capture=%v audit_rows=%d)", profileID, auditEnabled, auditCapture, auditCount)
}
...
func TestCustomRequestParametersGenerationParamsPerAttempt(t *testing.T) {
	...
	publicModelConfigID := harness.seedModel(t, profileID, "openai", publicModelID, "proxy", nil)
	harness.seedProxyTarget(t, publicModelConfigID, targetModelConfigID)
	primaryEndpointID := harness.seedEndpoint(t, profileID, "custom-params-genparams-primary-endpoint-"+suffix, primaryUpstream.baseURL("/custom-params/genparams/primary"), "primary-key")
	secondaryEndpointID := harness.seedEndpoint(t, profileID, "custom-params-genparams-secondary-endpoint-"+suffix, secondaryUpstream.baseURL("/custom-params/genparams/secondary"), "secondary-key")
