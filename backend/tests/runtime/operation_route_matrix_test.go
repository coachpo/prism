...
func TestRuntimeOperationRouteMatrixSupportedOperations(t *testing.T) {
	tests := []runtimeOperationRouteMatrixCase{
		...
		{
			name:          "OpenAIImagesGenerations",
			apiFamily:     "openai",
			operationName: "openai.images.generations",
			responsePayload: map[string]any{
				"created": 1713833628,
				"data":    []map[string]any{{"b64_json": "cm91dGUtbWF0cml4LWltYWdl"}},
				"size":    "1024x1024",
				"usage":   map[string]any{"input_tokens": 11, "output_tokens": 41, "total_tokens": 52},
			},
			requestPath: routeMatrixStaticRequestPath("/v1/images/generations"),
			requestBody: func(route seededRuntimeRoute, _ string) any {
				return map[string]any{"model": route.PublicModelID, "prompt": "route matrix image", "size": "1024x1024"}
			},
			wantUpstreamPath:  routeMatrixStaticUpstreamPath("/v1/images/generations"),
			assertModelSource: assertRouteMatrixBodyModelBinding,
			generationParams:  routeMatrixGenerationParamsExpectation{status: "missing"},
			usage:             routeMatrixUsageExpectation{streamOutcome: "not_streaming", inputTokens: routeMatrixInt64(11), outputTokens: routeMatrixInt64(41), totalTokens: routeMatrixInt64(52)},
			persistedAttribution: &routeMatrixPersistedAttributionExpectation{
				upstreamOperationName: "openai.images.generations",
				translationMode:       "none",
				upstreamRequestPath:   "/v1/images/generations",
			},
			responseContains: "1024x1024",
		},
		{
			name:          "OpenAIImagesEdits",
			apiFamily:     "openai",
			operationName: "openai.images.edits",
			responsePayload: map[string]any{
				"created": 1713833629,
				"data":    []map[string]any{{"b64_json": "cm91dGUtbWF0cml4LWVkaXQ="}},
				"quality": "high",
				"usage":   map[string]any{"input_tokens": 13, "output_tokens": 29, "total_tokens": 42},
			},
			requestPath: routeMatrixStaticRequestPath("/v1/images/edits"),
			requestBody: func(route seededRuntimeRoute, _ string) any {
				return map[string]any{"model": route.PublicModelID, "prompt": "route matrix edit", "images": []map[string]any{{"file_id": "file-route-matrix"}}}
			},
			wantUpstreamPath:  routeMatrixStaticUpstreamPath("/v1/images/edits"),
			assertModelSource: assertRouteMatrixBodyModelBinding,
			generationParams:  routeMatrixGenerationParamsExpectation{status: "missing"},
			usage:             routeMatrixUsageExpectation{streamOutcome: "not_streaming", inputTokens: routeMatrixInt64(13), outputTokens: routeMatrixInt64(29), totalTokens: routeMatrixInt64(42)},
			persistedAttribution: &routeMatrixPersistedAttributionExpectation{
				upstreamOperationName: "openai.images.edits",
				translationMode:       "none",
				upstreamRequestPath:   "/v1/images/edits",
			},
			responseContains: "high",
		},
		{
...
// routeMatrixOpenAIImageOperations seeds the image dimension for the image
// routes only. Each image route is seeded with exactly the operation it
// exercises, so the matrix also proves the narrow capability is sufficient.
func routeMatrixOpenAIImageOperations(operationName string) *string {
	switch operationName {
	case "openai.images.generations":
		return runtimeStringPtr("generations")
	case "openai.images.edits":
		return runtimeStringPtr("edits")
	default:
		return nil
	}
}
