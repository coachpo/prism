...
func TestRuntimeHotPathAvoidsPlanningAndCoordinationSQL(t *testing.T) {
	t.Run("OpenAIAdmissionSkip", func(t *testing.T) {
		...
		rejectedEndpointID := harness.seedEndpoint(t, profileID, "phase3-admission-rejected-"+suffix, harness.upstream.baseURL("/phase3/admission/rejected"), "phase3-admission-rejected-key", 0)
		eligibleEndpointID := harness.seedEndpoint(t, profileID, "phase3-admission-eligible-"+suffix, harness.upstream.baseURL("/phase3/admission/eligible"), "phase3-admission-eligible-key", 1)
	...
	t.Run("GeminiRoundRobin", func(t *testing.T) {
		...
		harness.seedProxyTarget(t, publicModelConfigID, targetModelConfigID)
		firstEndpointID := harness.seedEndpoint(t, profileID, "phase3-round-robin-first-"+suffix, harness.upstream.baseURL("/phase3/round-robin/first"), "phase3-round-robin-first-key", 0)
		secondEndpointID := harness.seedEndpoint(t, profileID, "phase3-round-robin-second-"+suffix, harness.upstream.baseURL("/phase3/round-robin/second"), "phase3-round-robin-second-key", 1)
