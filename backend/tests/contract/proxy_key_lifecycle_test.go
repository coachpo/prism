...
// succeeds: in-place rotation replaces the secret on the counted row, so the
// counted key set never changes and no headroom is required.
func TestProxyKeyRotateAtFullCapacityIsNetNeutral(t *testing.T) {
	...
	// The rotated key stays usable on the same row; no successor row is added.
	rotated := loadProxyKeyByID(t, harness, keyID)
	if !rotated.IsActive {
		t.Fatal("expected the rotated key to stay active")
	}
	if rotated.ExpiresAt != nil {
		t.Fatalf("expected rotation to leave the never-expiring key unexpired, got %+v", rotated)
	}
	if rotated.RotationCount != 1 {
		t.Fatalf("expected the rotated key to count one rotation, got %+v", rotated)
	}
	var rowCount int
	if err := harness.conn.QueryRow(context.Background(), `SELECT COUNT(*) FROM proxy_api_keys`).Scan(&rowCount); err != nil {
		t.Fatalf("count proxy key rows after rotation: %v", err)
	}
	if rowCount != 100 {
		t.Fatalf("expected rotation at the limit to leave 100 rows, got %d", rowCount)
	}
}
...
func TestProxyKeyImmutableHistoryAfterDeleteRenameRotate(t *testing.T) {
	...
	if _, err := harness.conn.Exec(context.Background(), `INSERT INTO request_logs (id, profile_id, model_id, api_family, row_kind, url_scrub_provenance, legacy_status_code, legacy_duration_ms, status_code, response_time_ms, is_stream, success_flag, request_path, created_at, proxy_api_key_id_snapshot, proxy_api_key_name_snapshot, proxy_api_key_attribution_state, proxy_api_key_auth_enforced_at_request, ingress_request_id, attempt_number, pricing_status, pricing_evidence_trust) VALUES (1, $1, 'model-a', 'openai', 'legacy_unknown', 'legacy_unknown', 200, 10, 200, 10, FALSE, TRUE, '/v1/chat/completions', $2, $3, 'history-key', 'identified', TRUE, 'ingress-hist-1', NULL, 'ineligible', 'trusted')`, modelLoadDefaultProfileID(t, harness), now, keyID); err != nil {
		t.Fatalf("seed historical request log: %v", err)
	}
	if _, err := harness.conn.Exec(context.Background(), `INSERT INTO usage_request_events (id, profile_id, ingress_request_id, model_id, api_family, status_code, success_flag, attempt_count, request_path, created_at, endpoint_label_snapshot, proxy_api_key_id_snapshot, proxy_api_key_name_snapshot, proxy_api_key_attribution_state, proxy_api_key_auth_enforced_at_request, pricing_status, pricing_evidence_trust) VALUES (1, $1, 'ingress-hist-1', 'model-a', 'openai', 200, TRUE, 1, '/v1/chat/completions', $2, 'ep', $3, 'history-key', 'identified', TRUE, 'ineligible', 'trusted')`, modelLoadDefaultProfileID(t, harness), now, keyID); err != nil {
	...
	// Rotate: the row keeps its id, and history keeps the name it was written with.
	rotateResponse := harness.requestJSON(t, harness.client, http.MethodPost, fmt.Sprintf("/api/settings/auth/proxy-keys/%d/rotate", keyID), nil, nil)
	assertStatus(t, rotateResponse, http.StatusOK)
	assertHistoricalSnapshotIdentity(t, harness, keyID, "history-key")
