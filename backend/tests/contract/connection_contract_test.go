...
func TestConnectionStandaloneMutationRejections(t *testing.T) {
	...
	// The legacy pricing-template assignment routes are hard-unmounted: direct
	// requests reach only the canonical router unknown-path 404/405 handling.
	legacyPricingResponse := harness.requestJSON(t, harness.client, http.MethodPut, fmt.Sprintf("/api/connections/%d/pricing-template", connectionID), map[string]any{"pricing_template_id": pricingTemplateID}, modelHeader(defaultProfileID))
	assertStatus(t, legacyPricingResponse, http.StatusNotFound)
...
func TestTargetRouteCRUD(t *testing.T) {
	...
	connectionPayload := connectionMutationConnection(t, createConnection)
	...
	targets := accessTargetMutationTargets(t, createModelTarget)
	...
	updatedConnection := connectionMutationConnection(t, updateConnection)
	...
	targets = accessTargetMutationTargets(t, reorderResponse)
	...
	targets = accessTargetMutationTargets(t, toggleConnectionTarget)
	...
	secondConnectionPayload := connectionMutationConnection(t, createSecondConnection)
	...
	targets = accessTargetMutationTargets(t, createSecondModelTarget)
	...
	targets = accessTargetMutationTargets(t, moveTerminalFront)
	assertTargetRouteOrder(t, targets, []expectedAccessTarget{
	...
	targets = accessTargetMutationTargets(t, moveModelBetweenTerminals)
	assertTargetRouteOrder(t, targets, []expectedAccessTarget{
		{TargetType: "connection", ConnectionID: secondConnectionID, Position: 0, IsEnabled: true},
	...
	targets = accessTargetMutationTargets(t, toggleModelTarget)
	assertTargetRouteOrder(t, targets, []expectedAccessTarget{
		{TargetType: "connection", ConnectionID: secondConnectionID, Position: 0, IsEnabled: true},
	...
	targets = accessTargetMutationTargets(t, deleteFirstTerminal)
	assertTargetRouteOrder(t, targets, []expectedAccessTarget{
...
func TestDeleteReferencedConnection(t *testing.T) {
	...
	created := connectionMutationConnection(t, createResponse)
...
func TestTargetDeleteOwnedConnectionDeletesConnectionNoOrphan(t *testing.T) {
	...
	assertStatus(t, createConnection, http.StatusCreated)
	created := connectionMutationConnection(t, createConnection)
	connectionID := jsonInt(t, created["id"])
	...
	targets := accessTargetMutationTargets(t, deleteConnectionTarget)
...
func TestModelScopedConnectionCreateCreatesOwnerTarget(t *testing.T) {
	...
	otherCreated := connectionMutationConnection(t, otherCreateResponse)
...
func TestLegacyModelConnectionAuxiliaryRoutesRejectWithOwnerGuidance(t *testing.T) {
	...
	// The legacy pricing-template assignment routes are hard-unmounted: direct
	// requests reach only the canonical router unknown-path 404/405 handling.
	legacyPricingResponse := harness.requestJSON(t, harness.client, http.MethodPut, fmt.Sprintf("/api/models/%d/connections/%d/pricing-template", modelConfigID, connectionID), map[string]any{"pricing_template_id": nil}, modelHeader(defaultProfileID))
	assertStatus(t, legacyPricingResponse, http.StatusNotFound)
}
...
func insertContractPricingTemplate(t *testing.T, harness *contractHarness, profileID int, name string) int {
	t.Helper()
	return insertContractPricingTemplateWithPrices(t, harness, profileID, name, "1", "2", "0", "0", "0")
}

// ensureContractProfileCurrencyEpoch creates the active epoch 1 + settings
// row for contract profiles inserted directly via SQL (the bootstrap seed
// only covers the Default profile).
func ensureContractProfileCurrencyEpoch(t *testing.T, harness *contractHarness, profileID int) {
	t.Helper()
	now := time.Now().UTC()
	var settingsCount int
	if err := harness.conn.QueryRow(context.Background(), `SELECT COUNT(*) FROM user_settings WHERE profile_id = $1`, profileID).Scan(&settingsCount); err != nil {
		t.Fatalf("count user settings for profile %d: %v", profileID, err)
	}
	if settingsCount == 0 {
		var epochID int64
		if err := harness.conn.QueryRow(context.Background(), `INSERT INTO reporting_currency_epochs (profile_id, epoch, currency_code, currency_symbol, effective_at, superseded_at, created_at, updated_at) VALUES ($1, 1, 'USD', '$', NULL, NULL, $2, $2) RETURNING id`, profileID, now).Scan(&epochID); err != nil {
			t.Fatalf("insert contract epoch 1 for profile %d: %v", profileID, err)
		}
		if _, err := harness.conn.Exec(context.Background(), `INSERT INTO user_settings (profile_id, report_currency_code, report_currency_symbol, timezone_preference, current_reporting_currency_epoch_id, pricing_migration_state, pricing_template_generation, pricing_reference_generation, created_at, updated_at) VALUES ($1, 'USD', '$', NULL, $2, 'ready', 0, 0, $3, $3)`, profileID, epochID, now); err != nil {
			t.Fatalf("insert contract user settings for profile %d: %v", profileID, err)
		}
	}
}

func insertContractPricingTemplateWithPrices(t *testing.T, harness *contractHarness, profileID int, name string, inputPrice string, outputPrice string, cachedInputPrice string, cacheCreationPrice string, reasoningPrice string) int {
	t.Helper()
	ensureContractProfileCurrencyEpoch(t, harness, profileID)
	now := time.Now().UTC()
	// The logical-row -> revision -> pointer sequence is one transaction: the
	// deferred pointer invariant closes only at commit.
	tx, err := harness.conn.Begin(context.Background())
	if err != nil {
		t.Fatalf("begin contract pricing template insert: %v", err)
	}
	...
	if err := tx.Commit(context.Background()); err != nil {
		t.Fatalf("commit contract pricing template insert %q: %v", name, err)
	}
	return templateID
}
...
func assertPricingTemplateStoredPrices(t *testing.T, harness *contractHarness, profileID int, name string, inputPrice string, outputPrice string, cachedInputPrice any, cacheCreationPrice any, reasoningPrice any) {
	t.Helper()
	...
	if gotInputPrice != inputPrice || gotOutputPrice != outputPrice || !nullableTestStringEqual(gotCachedInputPrice, cachedInputPrice) || !nullableTestStringEqual(gotCacheCreationPrice, cacheCreationPrice) || !nullableTestStringEqual(gotReasoningPrice, reasoningPrice) {
		t.Fatalf("expected stored pricing fields input=%q output=%q cached_input=%q cache_creation=%q reasoning=%q, got input=%q output=%q cached_input=%v cache_creation=%v reasoning=%v", inputPrice, outputPrice, cachedInputPrice, cacheCreationPrice, reasoningPrice, gotInputPrice, gotOutputPrice, gotCachedInputPrice, gotCacheCreationPrice, gotReasoningPrice)
	}
}

func nullableTestStringEqual(got *string, want any) bool {
	if want == nil {
		return got == nil
	}
...
func TestConnectionCapabilitySnapshotsExposeOpenAITextCapability(t *testing.T) {
	...
	// The text capability is no longer required on its own — an image-only
	// Terminal Target has none — so omitting it under a text owner is rejected by
	// the owner-mode equality rule instead, which names the actual conflict.
	...
	assertErrorResponse(t, missingCapability, http.StatusUnprocessableEntity, "openai_text_capability must equal the owner model openai_accepted_format")
	...
	// Strict mode equality keeps the capability locked to the owner mode; an
	// idempotent same-mode update still round-trips the snapshot value.
	sameModeUpdateResponse := harness.requestJSON(t, harness.client, http.MethodPatch, fmt.Sprintf("/api/models/%d/connections/%d", ownerModelID, connectionID), map[string]any{"openai_text_capability": "chat_completions_only"}, modelHeader(defaultProfileID))
	assertStatus(t, sameModeUpdateResponse, http.StatusOK)
	updated := connectionMutationConnection(t, sameModeUpdateResponse)
	...
	// OpenAI text capability is an exact owner-mode contract. A different
	// capability is rejected before persistence.
