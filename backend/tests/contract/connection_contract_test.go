...
func TestTargetRouteCRUD(t *testing.T) {
	...
	createSecondConnection := harness.requestJSON(t, harness.client, http.MethodPost, fmt.Sprintf("/api/models/%d/connections", sourceModelID), map[string]any{"endpoint_id": endpointBID, "openai_text_capability": "responses_only", "is_active": true, "name": "Owner Route Connection Two"}, modelHeader(defaultProfileID))
