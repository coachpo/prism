...
func TestConnectionCustomRequestParametersCreateSemantics(t *testing.T) {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			body:       map[string]any{"endpoint_id": endpointID, "openai_text_capability": "responses_only"},
			wantStatus: http.StatusCreated,
			wantNull:   true,
		},
		{
			name:       "null field normalizes to null",
			body:       map[string]any{"endpoint_id": endpointID, "openai_text_capability": "responses_only", "custom_request_parameters": nil},
			wantStatus: http.StatusCreated,
			wantNull:   true,
		},
		{
			name:       "empty object normalizes to null",
			body:       map[string]any{"endpoint_id": endpointID, "openai_text_capability": "responses_only", "custom_request_parameters": map[string]any{}},
			wantStatus: http.StatusCreated,
			wantNull:   true,
		},
		{
			name:         "valid object round-trips",
			body:         map[string]any{"endpoint_id": endpointID, "openai_text_capability": "responses_only", "custom_request_parameters": map[string]any{"provider": map[string]any{"only": []string{"deepinfra/turbo"}, "allow_fallbacks": false}, "temperature": nil}},
			wantStatus:   http.StatusCreated,
			wantNull:     false,
...
func TestConnectionCustomRequestParametersValidationErrors(t *testing.T) {
	...
	createBase := func(fieldValue any) map[string]any {
		return map[string]any{"endpoint_id": endpointID, "openai_text_capability": "responses_only", "custom_request_parameters": fieldValue}
	}
	...
	rawBody := `{"endpoint_id": ` + fmt.Sprint(endpointID) + `, "openai_text_capability": "responses_only", "custom_request_parameters": {"provider": }`
...
func TestConnectionCustomRequestParametersPatchSemantics(t *testing.T) {
	...
	createResponse := harness.requestJSON(t, harness.client, http.MethodPost, fmt.Sprintf("/api/models/%d/connections", ownerModelID), map[string]any{"endpoint_id": endpointID, "openai_text_capability": "responses_only", "custom_request_parameters": map[string]any{"provider": map[string]any{"only": []string{"deepinfra/turbo"}}, "temperature": 0.5}}, modelHeader(defaultProfileID))
