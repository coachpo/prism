...
func TestCompositeModelCreateContract(t *testing.T) {
	...
	t.Run("creates enabled model with derived capability and warnings", func(t *testing.T) {
		payload := s15JSON[map[string]any](t, harness, profileID, http.MethodPost, "/api/models", map[string]any{
			...
			"initial_terminal_target": map[string]any{
				...
				"custom_request_parameters": map[string]any{"provider": map[string]any{"only": []any{"deepinfra/turbo"}}},
			},
		}, http.StatusCreated)
		...
		customParameters := asMap(t, connection["custom_request_parameters"])
		if asMap(t, customParameters["provider"])["only"].([]any)[0] != "deepinfra/turbo" {
			t.Fatalf("expected composite custom request parameters to persist, got %+v", customParameters)
		}
	...
	t.Run("mismatched capability is rejected atomically", func(t *testing.T) {
...
func TestTerminalTargetCopyContract(t *testing.T) {
	...
	sourceConnectionID := modelInsertConnectionWithCapability(t, harness, profileID, sourceModelID, endpointID, "dual_native", 0, true)
	if _, err := harness.conn.Exec(context.Background(), `UPDATE connections SET custom_request_parameters = '{"provider":{"only":["deepinfra/turbo"]},"temperature":0.5}'::jsonb WHERE id = $1`, sourceConnectionID); err != nil {
		t.Fatalf("seed source custom request parameters: %v", err)
	}
	...
	t.Run("copies to multiple destinations transactionally with enable_copies default false", func(t *testing.T) {
		...
		if jsonInt(t, summary["endpoint_id"]) != endpointID || summary["openai_text_capability"] != "dual_native" || summary["is_active"] != true {
			t.Fatalf("expected copy to preserve endpoint/capability/active, got %+v", summary)
		}
		if jsonInt(t, summary["custom_request_parameter_count"]) != 2 {
			t.Fatalf("expected copy summary to expose custom request parameter count only, got %+v", summary)
		}
		var copiedParameters string
		if err := harness.conn.QueryRow(context.Background(), `SELECT custom_request_parameters::text FROM connections WHERE id = $1`, jsonInt(t, summary["id"])).Scan(&copiedParameters); err != nil {
			t.Fatalf("load copied custom request parameters: %v", err)
		}
		if !strings.Contains(copiedParameters, "deepinfra/turbo") || !strings.Contains(copiedParameters, "temperature") {
			t.Fatalf("expected copied custom request parameters to be preserved, got %s", copiedParameters)
		}
		...
		// Exact native mode matching means a dual-native source copied to dual-
		// native destination models needs no partial-coverage warning.
		warnings := payload["configuration_warnings"].([]any)
		if len(warnings) != 0 {
			t.Fatalf("expected no configuration warnings for exact mode match, got %+v", payload)
		}
