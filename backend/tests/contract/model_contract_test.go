...
// modelMutationModel unwraps the {model, configuration_warnings} envelope
// returned by model create/update mutations.
func modelMutationModel(t *testing.T, harness *contractHarness, profileID int, method string, path string, body any, wantStatus int) map[string]any {
	t.Helper()
	envelope := modelJSON[map[string]any](t, harness, profileID, method, path, body, wantStatus)
...
// accessTargetMutationEnvelopeTargets performs an access-target mutation and
// returns the access_targets array from the envelope.
func accessTargetMutationEnvelopeTargets(t *testing.T, harness *contractHarness, profileID int, method string, path string, body any, wantStatus int) []any {
	t.Helper()
	envelope := modelJSON[map[string]any](t, harness, profileID, method, path, body, wantStatus)
	targets, ok := envelope["access_targets"].([]any)
	if !ok {
		t.Fatalf("expected access-target mutation envelope to include access_targets, got %+v", envelope)
	}
	return targets
}
...
// connectionMutationConnection unwraps the {connection, access_targets,
// configuration_warnings} envelope returned by owner-scoped connection
// create/update mutations.
func connectionMutationConnection(t *testing.T, response *http.Response) map[string]any {
	t.Helper()
	var envelope map[string]any
	decodeJSONResponse(t, response, &envelope)
	connection, ok := envelope["connection"].(map[string]any)
	if !ok {
		t.Fatalf("expected connection mutation envelope to include connection, got %+v", envelope)
	}
	if _, ok := envelope["access_targets"]; !ok {
		t.Fatalf("expected connection mutation envelope to include access_targets, got %+v", envelope)
	}
	if _, ok := envelope["configuration_warnings"]; !ok {
		t.Fatalf("expected connection mutation envelope to include configuration_warnings, got %+v", envelope)
	}
	return connection
}

// accessTargetMutationTargets unwraps the {access_targets, configuration_warnings}
// envelope returned by access-target authoring mutations.
func accessTargetMutationTargets(t *testing.T, response *http.Response) []map[string]any {
	t.Helper()
	var envelope map[string]any
	decodeJSONResponse(t, response, &envelope)
	targets, ok := envelope["access_targets"].([]any)
	...
	if _, ok := envelope["configuration_warnings"]; !ok {
		t.Fatalf("expected access-target mutation envelope to include configuration_warnings, got %+v", envelope)
	}
	items := make([]map[string]any, 0, len(targets))
	for _, target := range targets {
		items = append(items, asMap(t, target))
	}
	return items
}
...
func TestModelCRUD(t *testing.T) {
	...
	draft := modelMutationModel(t, harness, profileID, http.MethodPost, "/api/models", openAIModelBody(strategyID, "s8-access-draft", map[string]any{"display_name": "S8 Access Draft"}), http.StatusCreated)
	assertOpenAIModelPayload(t, draft, strategyID, "S8 Access Strategy", "S8 Access Draft", false, nil)

	source := modelMutationModel(t, harness, profileID, http.MethodPost, "/api/models", openAIModelBody(strategyID, "s8-access-model", map[string]any{"display_name": "S8 Access Model"}), http.StatusCreated)
	...
	modelMutationModel(t, harness, profileID, http.MethodPut, modelPath(sourceModelID), map[string]any{"is_enabled": true}, http.StatusOK)
	...
	updated := modelMutationModel(t, harness, profileID, http.MethodPut, modelPath(sourceModelID), map[string]any{"display_name": nil}, http.StatusOK)
	...
	renamed := modelMutationModel(t, harness, profileID, http.MethodPut, modelPath(sourceModelID), map[string]any{"model_id": "s8-access-model-renamed", "display_name": nil}, http.StatusOK)
...
func TestModelTargetMetadataAndObsoleteFields(t *testing.T) {
	...
	createdTargets := accessTargetMutationEnvelopeTargets(t, harness, profileID, http.MethodPost, modelTargetListPath(sourceModelID), modelTargetBody("metadata-target", 0, true), http.StatusCreated)
	assertAccessTargets(t, map[string]any{"access_targets": createdTargets}, wantTargets)
	createdTargetID := jsonInt(t, asMap(t, createdTargets[0])["id"])
	assertStoredModelTargetFlat(t, harness, sourceModelID, terminalTargetID, 0, true)
	assertAccessTargets(t, map[string]any{"access_targets": accessTargetMutationEnvelopeTargets(t, harness, profileID, http.MethodPatch, modelTargetItemPath(sourceModelID, createdTargetID), map[string]any{"position": 0, "is_enabled": true}, http.StatusOK)}, wantTargets)
	...
	endpointID := modelInsertEndpoint(t, harness, profileID, "Target Metadata Connection Endpoint")
...
func TestModelValidationContracts(t *testing.T) {
	...
	t.Run("openai accepted format", func(t *testing.T) {
		...
		assertErrorResponse(t, response, http.StatusBadRequest, "at least one of openai_accepted_format or openai_image_operations is required when api_family is 'openai'")
	...
	t.Run("wrong profile target", func(t *testing.T) {
		...
		otherEndpointID := modelInsertEndpoint(t, harness, otherProfileID, "Other Profile Endpoint")
...
func TestModelDeleteContracts(t *testing.T) {
	...
	t.Run("delete owner and rollback", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "Task 5 Delete Owner Endpoint")
	...
	t.Run("delete still blocked when targeted", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "Task 5 Referenced Endpoint")
...
func TestModelConnectionTargetContracts(t *testing.T) {
	...
	t.Run("owner uniqueness", func(t *testing.T) {
		endpointID := modelInsertEndpoint(t, harness, profileID, "Connection Owner Endpoint")
	...
	t.Run("reject arbitrary connection assignment", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "Reject Connection Assignment Endpoint")
		connectionID := modelInsertStandaloneConnection(t, harness, profileID, "openai", endpointID, 0, true, nil)
	...
	t.Run("reject connection retarget", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "Reject Connection Retarget Endpoint")
		...
		assertAccessTargets(t, map[string]any{"access_targets": accessTargetMutationEnvelopeTargets(t, harness, profileID, http.MethodPatch, modelTargetItemPath(sourceModelID, targetID), map[string]any{"position": 1, "is_enabled": false}, http.StatusOK)}, []expectedAccessTarget{
	...
	t.Run("preserve owned private connection targets", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "Preserve Private Connection Endpoint")
		connectionID := modelInsertConnection(t, harness, profileID, sourceModelID, endpointID, 0, true, nil)
		connectionTargetID := modelLoadConnectionTargetID(t, harness, sourceModelID, connectionID)
		assertAccessTargets(t, map[string]any{"access_targets": accessTargetMutationEnvelopeTargets(t, harness, profileID, http.MethodPost, modelTargetListPath(sourceModelID), modelTargetBody("preserve-model-target", 1, true), http.StatusCreated)}, []expectedAccessTarget{
...
func TestModelEndpointHelpers(t *testing.T) {
	...
	t.Run("by endpoints", func(t *testing.T) {
		...
		endpointAID := modelInsertEndpoint(t, harness, profileID, "S8 Helper Endpoint A")
		endpointBID := modelInsertEndpoint(t, harness, profileID, "S8 Helper Endpoint B")
	...
	t.Run("by endpoint", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "S8 By Endpoint")
	...
	t.Run("batch dedupe", func(t *testing.T) {
		...
		endpointID := modelInsertEndpoint(t, harness, profileID, "Endpoint Batch Dedupe")
	...
	t.Run("reachable connection count", func(t *testing.T) {
		...
		endpointAID := modelInsertEndpoint(t, harness, profileID, "Reachable Count Endpoint A")
		endpointBID := modelInsertEndpoint(t, harness, profileID, "Reachable Count Endpoint B")
...
func modelInsertLoadbalanceStrategy(t *testing.T, harness *contractHarness, profileID int, name string) int {
	t.Helper()
	return modelInsertLoadbalanceStrategyWithType(t, harness, profileID, name, "single")
}

func modelInsertLoadbalanceStrategyWithType(t *testing.T, harness *contractHarness, profileID int, name string, strategyType string) int {
	t.Helper()
	now := time.Now().UTC()
	var strategyID int
	if err := harness.conn.QueryRow(
		...
		`INSERT INTO loadbalance_strategies (profile_id, name, legacy_strategy_type, failure_status_codes, ban_mode,
		 ...
		 VALUES ($1, $2, $3, $4::integer[], 'until_reset', 60000, 2.0, 0.2, 900000, 2, 4, 0, $5, $5)
		 RETURNING id`,
		profileID,
		name,
		strategyType,
...
func modelInsertEndpoint(t *testing.T, harness *contractHarness, profileID int, name string, _ ...int) int {
	t.Helper()
	now := time.Now().UTC()
	var endpointID int
	if err := harness.conn.QueryRow(context.Background(), `INSERT INTO endpoints (profile_id, name, base_url, api_key, config_revision, created_at, updated_at) VALUES ($1, $2, $3, $4, 1, $5, $6) RETURNING id`, profileID, name, fmt.Sprintf("https://%s.invalid", strings.ToLower(strings.ReplaceAll(name, " ", "-"))), "plain-api-key", now, now).Scan(&endpointID); err != nil {
...
func modelInsertRequestLog(t *testing.T, harness *contractHarness, profileID int, modelID string, apiFamily string, statusCode int, requestID string) {
	...
	if _, err := harness.conn.Exec(context.Background(), `INSERT INTO request_logs (profile_id, model_id, api_family, ingress_request_id, attempt_number, row_kind, url_scrub_provenance, attempt_trigger, attempt_result, is_winner, upstream_status_code, attempt_duration_ms, status_code, response_time_ms, is_stream, success_flag, pricing_status, pricing_evidence_trust, unpriced_reason, request_path, created_at) VALUES ($1, $2, $3, $4, 1, 'upstream', 'runtime_scrubbed', 'initial', $5, TRUE, $6, $7, $6, $7, FALSE, $8, $9, $10, NULL, $11, $12)`, profileID, modelID, apiFamily, requestID, attemptResult, statusCode, 120, statusCode >= 200 && statusCode < 300, requestStatus, requestTrust, "/v1/chat/completions", now); err != nil {
	...
	if _, err := harness.conn.Exec(context.Background(), `INSERT INTO usage_request_events (profile_id, ingress_request_id, model_id, api_family, endpoint_label_snapshot, status_code, success_flag, attempt_count, request_path, created_at, stream_outcome, pricing_status, pricing_evidence_trust, proxy_api_key_attribution_state) VALUES ($1, $2, $3, $4, 'Model Helper Endpoint', $5, $6, 1, '/v1/chat/completions', $7, 'not_streaming', $8, $9, 'unknown')`, profileID, requestID, modelID, apiFamily, statusCode, statusCode >= 200 && statusCode < 300, now, requestStatus, requestTrust); err != nil {
		t.Fatalf("insert usage event %q: %v", requestID, err)
	}
}
