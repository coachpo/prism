...
// TestReverseContractNoExactMatchOrTranslationRemnants pins the retired
// translation terminology across live runtime surfaces while preserving the
// current exact OpenAI text-mode authoring contract.
func TestReverseContractNoExactMatchOrTranslationRemnants(t *testing.T) {
	...
	t.Run("cross-mode capability write is rejected", func(t *testing.T) {
		...
		response := harness.requestJSON(t, harness.client, http.MethodPost, fmt.Sprintf("/api/models/%d/connections", modelConfigID), map[string]any{
