...
async function runOnHost(playwrightArgs) {
  ...
  const containerNodeModules = nodeModulesRoot;
  const dockerArgs = [
    "run",
    "--rm",
    "--init",
    ...
    "--tmpfs",
    `${nodeModulesRoot}/.tmp:rw,exec,nosuid,size=256m,uid=${uid},gid=${gid}`,
    "--tmpfs",
    `${nodeModulesRoot}/.vite:rw,exec,nosuid,size=512m,uid=${uid},gid=${gid}`,
    "--tmpfs",
    `${nodeModulesRoot}/.vite-temp:rw,exec,nosuid,size=256m,uid=${uid},gid=${gid}`,
    "--mount",
...
try {
  const exitCode =
    process.env.PRISM_CLOSED_LOOP_E2E_CONTAINER === "1"
