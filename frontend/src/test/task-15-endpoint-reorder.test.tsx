...
function referenceDetail(
  overrides?: Partial<EndpointReferenceDetail>,
): EndpointReferenceDetail {
  return {
    endpoint_id: 1,
    summary: {
      direct_reference_count: 0,
      referencing_model_count: 0,
      enabled_reference_count: 0,
...
describe("Endpoint direct-reference contract", () => {
  ...
  it("renders the compact table with fingerprint identity and no move controls", async () => {
          ...
          items: [
            ...
            },
            {
              ...
              summary: {
                ...
                referencing_model_count: 1,
                enabled_reference_count: 1,
                orphan_reference_count: 0,
              },
            },
          ],
    ...
    expect(
      screen.queryByRole("button", { name: /上移端点/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /下移端点/ }),
    ).not.toBeInTheDocument();
  ...
  it("fails closed when the references batch returns 503: no fake zero, no delete confirm", async () => {
    ...
    rewriteTestServer.use(
      ...
      http.get("/api/endpoints/1/references", () =>
        HttpResponse.json(referenceDetail(), { status: 503 }),
      ),
      http.delete("/api/endpoints/1", () =>
    ...
    expect(
      screen.getAllByRole("button", { name: "重试本行" }).length,
    ).toBeGreaterThan(0);
    ...
    await userEvent.click(
      within(table).getByRole("button", { name: /确定要删除/ }),
    );
    await waitFor(() => {
      expect(
        screen.queryByTestId("delete-endpoint-confirm"),
      ).not.toBeInTheDocument();
    });
  ...
  it("opens a fresh preflight on delete and blocks when references exist", async () => {
    ...
    rewriteTestServer.use(
      ...
      http.post("/api/endpoints/references/batch", () =>
        HttpResponse.json({
          items: [{ endpoint_id: 1, summary: detail.summary }],
        }),
      ),
    ...
    expect(
      await screen.findByTestId("delete-blocked-heading"),
    ).toBeInTheDocument();
  ...
  it("allows delete confirmation only after a fresh zero-reference preflight", async () => {
    ...
    rewriteTestServer.use(
      ...
      http.post("/api/endpoints/references/batch", () =>
        HttpResponse.json({
          items: [{ endpoint_id: 1, summary: zeroDetail.summary }],
        }),
      ),
      http.get("/api/endpoints/1/references", () =>
        HttpResponse.json(zeroDetail),
      ),
      http.delete("/api/endpoints/1", () =>
        HttpResponse.json({ deleted: true }),
      ),
