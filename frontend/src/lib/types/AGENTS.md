...
- Add contracts to the owning leaf and re-export through `../types.ts`. Models.dev metadata types stay in `model-catalog.ts`; Pi export/binding/search types stay in `model-export.ts`. OpenCode source/render types stay in `opencode-export.ts`, separating safe persisted catalog projection from Pi bindings. Separate catalog revisions, identities, and CAS tokens must not collapse into a weak shared union.
