...
export const models = {
  ...
  routeReadiness: () =>
    request<ModelRouteReadinessEnvelope<RawModelConfigListReadinessItem>>("/api/models?include=route_readiness").then((response) => ({
  ...
  create: (data: ManagedModelConfigCreate | ManagedModelConfigCompositeCreate) =>
...
export const loadbalanceStrategies = {
  ...
  createDefaults: () =>
    request<LoadbalanceStrategyDefaultsResponse>("/api/loadbalance/strategies/defaults", {
      method: "POST",
    }),
  setDefault: (strategyId: number, expectedDefaultStrategyId: number | null) =>
    request<StrategySetDefaultResponse>(`/api/loadbalance/strategies/${strategyId}/default`, {
      method: "PUT",
      body: JSON.stringify({ expected_default_strategy_id: expectedDefaultStrategyId } satisfies StrategySetDefaultRequest),
    }),
  impact: (strategyId: number, params: { limit?: number; cursor?: string }) => {
    const query = buildQuery(params);
    return request<StrategyImpactListResponse>(`/api/loadbalance/strategies/${strategyId}/models${query ? `?${query}` : ""}`);
  },
...
export { connections, endpoints, pricingTemplates } from "./management_resources";
