    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            "group/sidebar-wrapper flex min-h-svh w-full bg-background has-data-[variant=inset]:bg-inset",
            className
          )}
          {...props}
        >
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      size="icon-sm"
      className={cn(
        "rounded-lg border border-border bg-panel text-muted-foreground transition-[color,background-color,border-color,box-shadow] hover:bg-inset hover:text-foreground",
        className
      )}
      onClick={(event) => {
        onClick?.(event)
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn(
        "bg-sidebar-border data-[orientation=horizontal]:w-[calc(100%-1rem)] data-[orientation=horizontal]:self-center",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring transition-[width,height,padding] group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      size: {
        default: "h-9 text-sm",
