...
export const observeSearchSchema = z.object({
  ...
  // /observe/routing-health.
  tab: z
    .enum(["overview", "analytics", "events", "trend", "errors", "activity", "terminal_targets"])
    .catch("overview"),
  metric: z.enum(["requests", "errors", "ttft", "tokens", "cost"]).catch("requests"),
  group_by: z.enum(["none", "model", "endpoint", "terminal_target"]).catch("none"),
  interval: z.enum(["auto", "5m", "15m", "1h", "6h", "1d", "1w", "1mo", "1y"]).catch("auto"),
  ...
  preset: z.enum(["1h", "6h", "24h", "7d", "30d", "all", "custom"]).optional().catch(undefined),
  ...
  event_type: z.preprocess((value) => (value == null ? undefined : Array.isArray(value) ? value : [value]), z.array(z.string()).optional()).catch(undefined),
  event_failure_kind: z.preprocess((value) => (value == null ? undefined : Array.isArray(value) ? value : [value]), z.array(z.string()).optional()).catch(undefined),
  event_admission_reason: z.preprocess((value) => (value == null ? undefined : Array.isArray(value) ? value : [value]), z.array(z.string()).optional()).catch(undefined),
  ...
  runtime_state: z.preprocess((value) => (value == null ? undefined : Array.isArray(value) ? value : [value]), z.array(z.string()).optional()).catch(undefined),
...
export const requestLogSearchSchema = z.object({
  ...
  cost_segment_key: searchStringSchema.catch(""),
  chain_limit: z.coerce.number().int().min(1).max(50).optional().catch(undefined),
  chain_row_limit: z.coerce.number().int().min(1).max(200).optional().catch(undefined),
...
export const settingsSearchSchema = z.object({
  ...
  costing_action: z.enum(["currency_cutover", "repair_same_currency", "archive_unused_fx"]).optional(),
...
export const modelsListSearchSchema = z.object({
  ...
  sort_by: z
    .enum(["name", "api_family", "status", "targets", "strategy", "success", "p95", "requests", "spend"])
    .optional(),
...
export const prismRouteDefinitions = [
  ...
  { id: "system-proxy-keys", path: "/system/proxy-keys", scope: "protected-global", searchSchema: emptySearchSchema },
...
export const prismDynamicRouteDefinitions = [
  { id: "model-detail", path: "/route/models/$modelId", scope: "protected-selected-profile" },
