...
export const rewriteQueryKeys = {
  ...
  selectedProfile: (profileId: RewriteProfileId) => {
    ...
    return {
      ...
      requestLog: (requestId: string) => [...profile, "stats", "request-logs", requestId] as const,
      audit: () => [...profile, "audit"] as const,
      requestAudit: (requestId: string, search: unknown) => [...profile, "audit", "request-log", requestId, search] as const,
  ...
  global: {
    ...
    proxyApiKeyUsage: (keyId: number, window: string) =>
      [...globalRoot, "settings", "auth", "proxy-keys", keyId, "usage", window] as const,
