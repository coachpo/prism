...
export function TerminalTargetRuntimeSummary({
  ...
  const stateLabel =
    state.state === "retry_wait"
      ...
      : state.state === "banned"
        ? copy.bannedUntil(formatDateTime(state.banned_until_at))
        : copy.noCooldown

  return (
    <div className="flex flex-col gap-2 rounded-md border border-outline-variant bg-surface-container-low px-3 py-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <OperatorStatusBadge
            intent={state.state === "available" ? "neutral" : state.state === "banned" ? "danger" : "warning"}
            label={stateLabel}
            preserveLabel
          />
          {stale ? (
      ...
      {showReset ? (
        <p className="text-xs text-muted-foreground">{copy.resetCooldownDescription}</p>
      ) : null}
    </div>
  )
}
