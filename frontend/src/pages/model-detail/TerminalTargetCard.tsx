...
function coveragePresentation(coverage: string | undefined) {
  switch (coverage) {
    case "full":
      return { intent: "success" as const, label: "coverageFull" }
    case "partial":
      return { intent: "warning" as const, label: "coveragePartial" }
    case "none":
      return { intent: "danger" as const, label: "coverageNone" }
    default:
      return null
  }
}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
              ...
              {missingOperations.length > 0 ? (
                <span className="text-warning">{copy.missingOperations(missingOperations.join("、"))}</span>
              ) : null}
            </div>
          </div>
        </div>
