...
describe("route explanation",()=>{
 ...
 it("only samples on action and preserves stale facts after failure",async()=>{
  ...
  await userEvent.click(screen.getByRole("button",{name:"采样 / 刷新"}));
  expect(await screen.findByText(/部分运行时事实缺失/)).toBeInTheDocument();
  expect(screen.getByText(/运行时未观测/)).toBeInTheDocument();
 ...
 it("clears completed evidence and resets operation on same-model family changes",async()=>{
  ...
  await userEvent.click(screen.getByRole("button",{name:"采样 / 刷新"}));
  await screen.findByText(/部分运行时事实缺失/);
  view.rerender(panel("anthropic"));
  expect(screen.queryByText(/部分运行时事实缺失/)).not.toBeInTheDocument();
 ...
 it("clears completed evidence and cancels pending observations on auth epoch changes",async()=>{
  ...
  expect(screen.queryByText(/部分运行时事实缺失/)).not.toBeInTheDocument();
  expect(screen.getByRole("button",{name:"采样 / 刷新"})).toBeEnabled();
 });
});
