...
- `ConnectionDialog.tsx` keeps OpenAI Terminal Target capability equal to the owner model mode, absent modes included: an image-only owner (`openai_accepted_format=null`) hides the capability section entirely and submits `null`, and "从已有终端目标填充" prefills a draft that saves as an independent private Connection without importing the source capability.
...
- `RouteReadinessCard.tsx` opens with the explicit entry `model_id` and reads its direct identity summary from `buildUpstreamIdentitySummary` in `modelAccessTargetProjection.ts`: distinct known upstream identities, the exact case-sensitive decoupled count, and the unknown count over DIRECT Terminal Targets only. Model Target rows are logical edges that never contribute identities, the summary never follows them recursively, and a missing upstream value is unknown evidence — never backfilled from the entry `model_id`. Entries without direct Terminal Targets render the explicit 无直接终端目标 state instead of fabricated zeros.
...
├── modelAccessTargetProjection.ts # Access-target summary plus the direct upstream-identity projection
