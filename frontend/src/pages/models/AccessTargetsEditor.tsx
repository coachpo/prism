...
function resolveModelTargetLabel(
  ...
  return (
    modelOptions.find((model) => model.model_id === targetModelId)
      ?.display_name || targetModelId
  );
}
...
function textCapabilityLabel(
  ...
  if (capability === "chat_completions_only")
...
export function AccessTargetsEditor({
  ...
  const [committingTargetId, setCommittingTargetId] = useState<number | null>(
    null,
  );
  ...
  const persistedTargets = useMemo(
    () => sortAccessTargetsByPositionThenId(accessTargets),
    [accessTargets],
  );
  ...
  const orderedTargets = useMemo(() => {
    ...
    const reordered = draftOrder
      .map((id) => byId.get(id))
      .filter((target): target is ModelAccessTarget => Boolean(target));
    return reordered.length === persistedTargets.length
      ? reordered
      : persistedTargets;
  ...
  const movedCount = draftOrder
    ? orderedTargets.filter(
        (target, index) => persistedTargets[index]?.id !== target.id,
      ).length
    : 0;
  ...
  const selectedModelKeys = useMemo(
    () =>
      new Set(
        persistedTargets
          .filter(
            (target): target is typeof target & { target_model_id: string } =>
              target.target_type === "model" &&
              Boolean(target.target_model_id?.trim()),
          )
  ...
  const enabledTargetCount = persistedTargets.filter(
    (target) => target.is_enabled !== false,
  ).length;
  const showSingleTruncationWarning =
    strategyType === "single" && enabledTargetCount >= 2;
    ...
    >
      {error ? (
        <OperatorCallout
          intent="danger"
          description={error}
          data-testid="access-targets-error"
        />
      ) : null}
      ...
      {showSingleTruncationWarning ? (
        <OperatorCallout
          intent="warning"
          title={messages.loadbalanceStrategyCopy.singleTruncationWarning(
            ...
            <Table data-testid="access-targets-mixed-list">
              ...
              <TableBody>
                {orderedTargets.map((target, targetIndex) => {
                  const isTerminalTarget = isTerminalTargetAccessTargetType(
                    target.target_type,
                  );
                    ...
                    >
                      ...
                      <TableCell className="align-top">
                        {isTerminalTarget ? (
                          ...
                          <OperatorMissingValue
                            className="text-xs"
                            reason={copy.targetLimitsNotApplicable}
                          />
                        )}
                      </TableCell>
                      ...
                      <TableCell className="align-top">
                        {isTerminalTarget ? (
                          <TargetRuntime
                            ...
                            gap={
                              connectionId != null
                                ? currentStateGapByConnectionId.get(
                                    connectionId,
                                  )
                                : undefined
                            }
                            ...
                            resetting={
                              connectionId != null &&
                              isResettingConnection(connectionId)
                            }
                            state={runtime}
                            onReset={
                              connectionId != null && onResetCooldown
                                ? () => void onResetCooldown(connectionId)
                                : undefined
                            }
                          />
                        ) : (
                          <OperatorMissingValue
                            className="text-xs"
                            reason={copy.targetRuntimeNotApplicable}
                          />
                        )}
                      </TableCell>
                      ...
                      <TableCell className="align-top">
                        {!readOnlyConnection ? (
                          ...
                          <OperatorStatusBadge
                            intent={
                              target.is_enabled !== false ? "healthy" : "idle"
                            }
                            preserveLabel
                            label={
                              target.is_enabled !== false
                        ...
                        >
                          {canEditConnection ? (
                            <Button
                              ...
                              disabled={disabled || rowBusy}
                              onClick={() =>
                                onEditConnection?.(connection as Connection)
                              }
                            >
                          ...
                          {!readOnlyConnection ? (
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                {connection && onGeneratePricing ? (
                                  <DropdownMenuItem
                                    data-testid="terminal-pricing-action"
                                    onSelect={() =>
                                      onGeneratePricing(connection)
                                    }
                                  >
                                    <CircleDollarSign />
                                    {messages.modelCatalog.pricingMenuAction(
                                      name,
                                    )}
                                  </DropdownMenuItem>
                                ) : null}
          ...
          {movedCount > 0 ? (
            <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border bg-inset px-[var(--density-card-pad-x)] py-2 text-xs">
              <span className="mr-auto text-muted-foreground">
                {copy.targetOrderPending(formatNumber(movedCount))}
              </span>
              <Button
                type="button"
                variant="ghost"
      ...
      <div className="grid gap-3 rounded-lg border border-dashed bg-background p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <div className="grid gap-2">
          <Label htmlFor="access-target-select">
            {copy.selectSameFamilyModel}
          </Label>
          <Select
            value={pendingValue}
        ...
        <Button
          ...
          variant="outline"
          disabled={
            disabled || hasBusyAction || !effectivePendingValue || !onAddTarget
          }
...
function TargetCapability({
  ...
  if (!isTerminalTarget) {
    return (
      <OperatorMissingValue
        className="text-xs"
        reason={copy.targetCapabilityNotApplicableModel}
      />
    );
  }
  if (!connection) {
    return (
      <OperatorMissingValue
        className="text-xs"
        reason={copy.targetConnectionOutOfScope}
      />
    );
  }
  if (connection.api_family !== "openai") {
    return (
      <OperatorMissingValue
        className="text-xs"
        reason={copy.targetCapabilityNotApplicableFamily(
  ...
  if (labels.length === 0) {
    return (
      <OperatorMissingValue
        className="text-xs"
        reason={copy.targetCapabilityUnknown}
      />
    );
  }
  return (
...
function TargetLimits({
  ...
  const limits = [
    ...
    connection.max_in_flight_non_stream != null
      ? `${copy.nonStreamShort} ${connection.max_in_flight_non_stream}`
      : null,
    connection.max_in_flight_stream != null
      ? `${copy.streamShort} ${connection.max_in_flight_stream}`
      : null,
  ...
  if (!state) {
    ...
    if (!rowParticipatesInRouting) {
      return (
        <OperatorMissingValue
          className="text-xs"
          reason={copy.runtimeOutOfCohortReason}
        />
      );
    }
    if (completeness?.hasMore) {
      return (
        <OperatorClippedBadge
          label={copy.runtimeTruncated}
          reason={copy.runtimeTruncatedReason}
        />
      );
    }
    if (!completeness) {
    ...
    return (
      <OperatorMissingValue
        className="text-xs"
        reason={copy.runtimeAbsentFromCohortReason}
      />
    );
  }
    ...
    <div className="flex min-w-0 flex-col items-start gap-1">
      ...
      <span className="font-mono text-xs tabular-nums text-muted-foreground">
        {meteredNonStream ? (
          ...
          <OperatorMissingValue
            className="text-xs"
            reason={copy.inFlightNotMeteredReason}
          />
        )}
        {" / "}
        {meteredStream ? (
