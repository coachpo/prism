...
## WHERE TO LOOK
- Feature route and named proxy-key lifecycle owners: `../../features/proxy-keys/`
...
- Bootstrap auth settings and existing keys through the independent query lifecycle in `../../features/proxy-keys/useProxyKeyLedger.ts`.
- Patch the local key list after create, edit, rotate, and delete flows through the named mutation hooks under `../../features/proxy-keys/`, using `proxyKeyMutationReconciliation.ts` instead of reloading the whole page; reconcile capacity from the server snapshot returned by each mutation.
...
- The one-time secret session is handed off by `../../features/proxy-keys/useProxyKeySecretSession.ts` into `generatedSecretSession.ts`: list/auth/model/stats refetches never close it; Escape/mask/close/navigation/refresh are blocked until the operator acknowledges the key is saved; after finish/abandon all raw-key references are released.
