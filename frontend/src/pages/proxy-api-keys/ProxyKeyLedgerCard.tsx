...
import { KeyRound, MoreHorizontal, Pencil, RotateCcw, Trash2 } from "lucide-react";
 ...
 */
type LedgerSortColumn = "name" | "lifecycle" | "lastUsed" | "rotation" | "created";
...
function getSortValue(
  ...
  if (column === "lifecycle") return getProxyKeyLifecycleLabel(item, authEnabled);
...
function ProxyKeyUsageCell({ entry, item }: { entry: ProxyKeyUsageEntry | undefined; item: ProxyApiKey }) {
  ...
  if (entry.failed) {
    return (
      <span className="font-mono text-xs font-medium text-failing" title={copy.requests7dFailedReason}>
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      {clipped ? (
        <OperatorClippedBadge label={copy.requests7dClipped} reason={copy.requests7dClippedReason} />
      ) : null}
    </div>
  );
}
    ...
    <TableRow className={cn("group/row", operationalRowStripe(tier))}>
      <TableCell className="align-top">
        <div className="flex min-w-0 flex-col gap-0.5">
          ...
          <span className="truncate font-mono text-xs text-muted-foreground" title={item.key_preview}>
            {item.key_preview}
          </span>
          {note ? (
      ...
      <TableCell className="align-top">
        <div className="flex flex-col items-start gap-1">
          <OperatorStatusBadge intent={tier} label={getProxyKeyLifecycleLabel(item, authEnabled)} preserveLabel />
          <span
            className="text-xs text-muted-foreground"
            title={item.expires_at ? formatDateTime(item.expires_at) : undefined}
          >
            {expiryLine}
          </span>
        </div>
      ...
      <TableCell className="align-top">
        <div className="flex min-w-0 flex-col gap-0.5">
          {item.last_used_at ? (
            <span className="font-mono text-xs tabular-nums">{formatDateTime(item.last_used_at)}</span>
          ) : (
            <OperatorMissingValue className="text-xs" reason={copy.lastUsedNeverReason} />
          )}
          {item.last_used_ip ? (
            <span className="truncate font-mono text-xs text-muted-foreground">{item.last_used_ip}</span>
          ) : (
            <OperatorMissingValue className="text-xs" reason={copy.lastIpMissingReason} />
          )}
        </div>
      </TableCell>
      ...
      <TableCell className="align-top">
        {item.rotation_count > 0 && item.rotated_at ? (
          <div className="flex min-w-0 flex-col gap-0.5">
            <span className="font-mono text-xs tabular-nums">{formatNumber(item.rotation_count)}</span>
            <span className="truncate font-mono text-xs text-muted-foreground">
              {formatDateTime(item.rotated_at)}
            </span>
          </div>
        ) : (
          <OperatorMissingValue className="text-xs" reason={copy.rotatedAtMissingReason} />
        )}
      </TableCell>
...
export function ProxyKeyLedgerCard({
  ...
  const sorted = useMemo(
    () =>
      sortOperationalRows(filtered, sort, (item, column) => getSortValue(item, column, authEnabled), locale),
  ...
  useEffect(() => {
    onVisibleKeysChange(visibleKeySignature ? visibleKeySignature.split(",").map(Number) : []);
  ...
  const quotaPercent = capacity ? getProxyKeyUsagePercent(capacity.used, capacity.limit) : 0;

  const summary = (
    <>
      <span>{copy.ledgerSummary(formatNumber(displayedProxyKeys.length))}</span>
      ...
      {capacity ? (
        <>
          ...
          <Progress
            value={quotaPercent}
            aria-label={copy.capacityQuotaAria(formatNumber(capacity.used), formatNumber(capacity.limit))}
            className="h-1.5 w-24"
          />
          <span aria-hidden="true">·</span>
          <span className="font-mono tabular-nums">
            {copy.capacityCountedAt(formatDateTime(capacity.counted_at))}
          </span>
        </>
      ) : (
        <span className="text-degraded" title={copy.createBlockedCapacityUnknown}>
      ...
      {usageFailed ? (
        <>
          <span aria-hidden="true">·</span>
          <Button type="button" variant="outline" size="xs" onClick={onRetryUsage}>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <SortableTableHead sortKey="lifecycle" sort={sort} onSort={updateSort}>
                {copy.columnLifecycle}
              </SortableTableHead>
              <TableHead className="text-right" title={copy.columnRequests7dBasis}>
                <span className="inline-flex flex-wrap items-center justify-end gap-1">
                  {copy.columnRequests7d}
                  <span aria-hidden="true" className="text-text-disabled">
                    ?
                  </span>
                  <span className="sr-only">{copy.columnRequests7dBasis}</span>
                  {usageFailed ? (
                    <OperatorStalenessBadge
              ...
              <SortableTableHead sortKey="lastUsed" sort={sort} onSort={updateSort}>
                {copy.columnLastActivity}
              </SortableTableHead>
              <SortableTableHead sortKey="rotation" sort={sort} onSort={updateSort}>
                {copy.rotation}
              </SortableTableHead>
              <SortableTableHead sortKey="created" sort={sort} onSort={updateSort}>
          ...
          <TableBody>
            {loading ? <OperationalTableSkeletonRows columns={LEDGER_COLUMN_COUNT} rows={5} /> : null}
      ...
      {!loading && displayedProxyKeys.length === 0 ? (
        <div className="p-[var(--density-card-pad-x)]">
          <OperatorEmptyState
            ...
            action={<Button onClick={onIssue}>{copy.issueFirstKey}</Button>}
          />
        </div>
      ) : null}

      {!loading && displayedProxyKeys.length > 0 && sorted.length === 0 ? (
        <div className="p-[var(--density-card-pad-x)]">
          <OperatorEmptyState
