...
- Parent-cover local route clusters that do not need their own AGENTS file, including `__tests__/` and dense local helper folders already documented by the page leaves.
- When doing upgrade work, backward compatibility with the pre-upgrade implementation is not a goal unless explicitly requested. Prefer the best current implementation shape over preserving the old one. Do not add compatibility shims, dual paths, or fallback behavior solely to preserve the old interface.

## ANTI-PATTERNS
- Do not treat auth pages as protected-shell pages.
- Do not create extra AGENTS files for `__tests__/` or other local helper clusters already covered by their page parent.
