...
## OVERVIEW
`pages/statistics/` owns the shared usage-statistics implementation consumed by `../DashboardPage.tsx`. The dashboard analytics surface is backend-snapshot driven, with local persisted presentation state in `useUsageStatisticsPageState.ts`, REST polling orchestration in `useUsageStatisticsPageData.ts`, and shared rendering in `UsageStatisticsContent.tsx`.
...
├── useUsageStatisticsPageData.ts    # Snapshot loading, REST polling, and page-data orchestration
...
- Keep analytics polling and stale-snapshot handling in `useUsageStatisticsPageData.ts`, not in the dashboard shell.
