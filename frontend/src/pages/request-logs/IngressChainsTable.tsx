...
export function IngressChainsTable({
        ...
        const chainPageEnd =
                total > 0 && chainPageStart !== null
                        ? Math.min(
                                  chainPageStart + chainPageCounts.ingress,
                                  total,
                          )
                        : 0;
        const pageSummary =
        ...
        const toggle = (ingressRequestId: string) => {
                setExpanded((current) => {
                        ...
                        if (next.has(ingressRequestId))
                                next.delete(ingressRequestId);
                ...
                >
                        <div className="flex items-center justify-between gap-2 border-b border-border bg-inset px-[var(--density-card-pad-x)] py-2 text-xs text-muted-foreground">
                                <span data-testid="chain-page-counts">
                                        {copy.chainCounts(
                                                formatNumber(
                                                        chainPageCounts.ingress,
                                                ),
                                                formatNumber(
                                                        chainPageCounts.attempts,
                                                ),
                                                formatNumber(
                                                        chainPageCounts.rows,
                                                ),
                                        )}
                                </span>
                        </div>
                                        ...
                                        >
                                                <TableHeader>
                                                        <TableRow>
                                                                ...
                                                                <TableHead>
                                                                        {
                                                                                copy.chainColumnResult
                                                                        }
                                                                </TableHead>
                                                                <TableHead>
                                                                        {
                                                                                copy.chainColumnRequestedModel
                                                                        }
                                                                </TableHead>
                                                                <TableHead>
                                                                        {
                                                                                copy.chainColumnFinalTarget
                                                                        }
                                                                </TableHead>
                                                                <TableHead>
                                                                        {
                                                                                copy.chainColumnEndpoint
                                                                        }
                                                                </TableHead>
                                                                <TableHead className="text-right">
                                                                        {
                                                                                copy.chainColumnAttempts
                                                                        }
                                                                </TableHead>
                                                                <TableHead className="text-right">
                                                                        TTFT
                                                                ...
                                                                <TableHead className="text-right">
                                                                        {
                                                                                copy.chainColumnTokens
                                                                        }
                                                                </TableHead>
                                                                <TableHead className="text-right">
                                                                        {
                                                                                copy.chainColumnCost
                                                                        }
                                                                </TableHead>
                                                                <TableHead>
                                                                        {
                                                                                copy.chainColumnPricing
                                                                        }
                                                                </TableHead>
                                                        </TableRow>
                                                </TableHeader>
                                                        ...
                                                        {chains.map((chain) => (
                                                                ...
                                                                >
                                                                        ...
                                                                        {expanded.has(
                                                                                ...
                                                                                <TableRow
                                                                                        data-testid={`chain-${chain.ingress_request_id}`}
                                                                                >
                                                                                        <TableCell
                                                                                                colSpan={
                                                                                                        CHAIN_COLUMN_COUNT +
                                                                                        ...
                                                                                        >
                                                                                                <div className="flex flex-col gap-0.5 px-[var(--density-card-pad-x)] py-2">
                                                                                                        ...
                                                                                                        {!chain.retained_rows_page_complete &&
                                                                                                        chain.next_row_cursor ? (
                                                                                                                <Button
                                                                                                                        variant="ghost"
                                                                                                                        size="sm"
...
function ChainSummaryRow({
        ...
        const evidenceMissing =
                chain.finalized_evidence_state !== "authoritative" ||
                summary === null;
                ...
                >
                        <TableCell className="w-8 pr-0">
                                <button
                                        ...
                                        aria-label={copy.chainToggleAria(
                                                chain.ingress_request_id,
                                        )}
                                ...
                                <div className="flex flex-wrap items-center gap-1">
                                        {summary ? (
                                                ...
                                                <OperatorStatusBadge
                                                        intent="idle"
                                                        label={
                                                                copy.finalizedEvidenceUnavailableShort
                                                        }
                                                        preserveLabel
                                                />
                                        )}
                                        ...
                                        {chain.chain_complete === false ? (
                                                <OperatorClippedBadge
                                                        label={
                                                                copy.chainIncomplete
                                                        }
                                                        reason={
                                                                copy.chainIncompleteReason
                                                        }
                                                />
                                        ) : null}
                                        {chain.legacy_unknown_row_count > 0 ? (
                                                <OperatorTypeBadge
                                                        ...
                                                        label={copy.chainLegacyUnknown(
                                                                formatNumber(
                                                                        chain.legacy_unknown_row_count,
                                                                ),
                                                        )}
                                                        preserveLabel
                                                />
...
function ChainRowButton({
        ...
        const statusCode =
                ...
                row.gateway_status_code ??
                row.legacy_status_code;
        return (
                <button
                        type="button"
                ...
                >
                        ...
                        row.attempt_number !== undefined ? (
                                <span className="text-muted-foreground">
                                        {messages.requestLogs.attemptLabel(
                                                row.attempt_number,
                                        )}
                                </span>
                        ) : null}
