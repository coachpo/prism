...
function normalizeAPIFamilyAuditSettings(settings: AuditAPIFamilySetting[] | undefined): AuditAPIFamilySetting[] {
  const byFamily = new Map(settings?.map((setting) => [setting.api_family, setting]));

  return AUDIT_API_FAMILIES.map((api_family) => {
    const setting = byFamily.get(api_family);
    const audit_enabled = Boolean(setting?.audit_enabled);

    return {
      api_family,
      audit_enabled,
      audit_capture_bodies: audit_enabled && Boolean(setting?.audit_capture_bodies),
    };
  });
}
...
export function useAuditConfigurationData({
  ...
  const fetchAPIFamilyAuditSettings = useCallback(async () => {
    ...
    try {
      ...
      const normalized = normalizeAPIFamilyAuditSettings(response.settings);
  ...
  }, [enabled, fetchAPIFamilyAuditSettings, fetchUserAgentClientRules, fetchRules, revision]);
  ...
  const handleSaveAPIFamilyAuditSettings = async () => {
    const payload = { settings: normalizeAPIFamilyAuditSettings(apiFamilyAuditSettings) };

    setSavingAPIFamilyAuditSettings(true);
    try {
      const saved = await api.settings.audit.update(payload);
      const normalized = normalizeAPIFamilyAuditSettings(saved.settings);
      setApiFamilyAuditSettings(normalized);
      setSavedApiFamilyAuditSettings(normalized);
