...
export function CurrencyMigrationDialog({
  ...
  const submitMigration = useCallback(
    async (migration: PreparedMigration) => {
      ...
      const expectedEpoch = expectedEpochText
        ? Number(expectedEpochText)
        : null;
      if (
        !expectedUpdatedAt ||
        (expectedEpoch !== null &&
      ...
      const sealed =
        await api.settings.costing.currencyMigrationDraftSeal(draftId);
    ...
    [
      code,
      copy.previewFailed,
      currentCosting.expected_updated_at,
      currentCosting.reporting_currency_epoch,
      symbol,
    ],
  );
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      const currentCode = currentCosting.report_currency_code
        .trim()
        .toUpperCase();
        ...
        inventory &&
          (!hasEpoch ||
            (inventory.template_issue_count > 0 &&
              code.trim().toUpperCase() === currentCode)),
      );
      ...
      if (useInventory && inventory) {
        const loaded = await loadAllInventoryTemplatePages(
    ...
    codeError,
    copy.previewFailed,
    currentCosting.pricing_migration_inventory,
