    ...
    <Dialog open={endpoint != null} onOpenChange={(open) => { if (!open && !submitting) onOpenChange(false) }}>
      <DialogContent className="sm:max-w-xl">
        ...
        <DialogBody className="flex flex-col gap-4">
          ...
          {endpoint ? (
            <div className="flex flex-col gap-1 rounded-md border border-border bg-inset px-3 py-2">
          ...
          {destinationModel ? (
            <div className="flex flex-col gap-2">
              ...
              {preview && preview.coverage === "partial" ? (
                <p className="text-xs text-degraded">{routingCopy.missingOperations(preview.unsupportedAcceptedOperations.join("、"))}</p>
              ) : null}
