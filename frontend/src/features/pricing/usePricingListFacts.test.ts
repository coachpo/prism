...
import type { PricingTemplateListPage, PricingTemplateListPageItem } from "@/lib/types"
...
function makePage(items: PricingTemplateListPageItem[], nextCursor: string | null): PricingTemplateListPage {
  return {
    items,
    total_count: 3,
...
describe("usePricingListFacts", () => {
  ...
  it("stays inside the server page bound and follows the cursor to the end", async () => {
    ...
    expect(mocks.listPage).toHaveBeenNthCalledWith(1, { cursor: undefined, limit: 100 })
    expect(mocks.listPage).toHaveBeenNthCalledWith(2, { cursor: "cursor-2", limit: 100 })
  })
