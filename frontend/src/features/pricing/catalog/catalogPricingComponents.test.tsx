...
describe("catalog offering discovery", () => {
  ...
  it("stops at a bounded search when nothing matches exactly, and never auto-picks", async () => {
    resetHappyPath();
    matchPreviewMock.mockResolvedValue({
      committable: false,
      candidates: [],
    ...
    // A no-match must not resolve an offering, even though exactly one
    // candidate is available: the first candidate is never taken automatically.
    await waitFor(() => expect(matchPreviewMock).toHaveBeenCalled());
    expect(
      onResolved.mock.calls.filter(([resolved]) => resolved !== null),
    ).toHaveLength(0);
    // Discovery itself never previews or commits.
    expect(catalogPreviewMock).not.toHaveBeenCalled();
    expect(catalogCommitMock).not.toHaveBeenCalled();

    expect(
      await screen.findByText(/没有找到精确匹配的模型 ID/),
    ).toBeInTheDocument();
    await userEvent.type(screen.getByLabelText(/搜索候选/), "luna");

    const candidate = await screen.findByTestId(
      "catalog-candidate-codex-gpt-5.6-luna",
    );
  ...
  it("drops late candidates from a previously selected model", async () => {
    resetHappyPath();
    matchPreviewMock.mockResolvedValue({
      committable: false,
      candidates: [],
    ...
    const firstCandidates = deferred<
      Awaited<ReturnType<typeof modelsApi.catalog.candidates>>
    >();
    candidatesMock.mockImplementation((modelConfigId) => {
      if (modelConfigId === 7) return firstCandidates.promise;
    ...
    await userEvent.click(trigger);
    await userEvent.click(await screen.findByText(/Model B · model-b/));
    const currentCandidate = await screen.findByTestId(
      "catalog-candidate-provider-b-model-b",
    );
    ...
    await act(async () => {
      ...
      await firstCandidates.promise;
    });
    expect(
      screen.queryByTestId("catalog-candidate-provider-a-model-a"),
    ).not.toBeInTheDocument();

    await userEvent.click(currentCandidate);
    await waitFor(() =>
      expect(onResolved).toHaveBeenCalledWith({
        kind: "coordinates",
        providerId: "provider-b",
