...
export interface UseCatalogPricingImportResult {
  ...
  commit: () => Promise<CatalogPricingCommitResponse | null>;
}
...
export function useCatalogPricingImport({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await api.pricingTemplates.catalogPreview({
  ...
  const commit = useCallback(async () => {
    ...
    setCommitting(true);
    try {
      const response = await api.pricingTemplates.catalogCommit({
