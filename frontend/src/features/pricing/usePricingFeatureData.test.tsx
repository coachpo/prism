...
describe("usePricingFeatureData", () => {
  ...
  it("previews an import without writing, then commits with the server preview hash", async () => {
    ...
    mocks.importTemplates.mockResolvedValue({ created: 1, updated: 0, skipped: [], errors: [], preview_hash: "preview-hash", committable: true })
    mocks.importCommit.mockResolvedValue({ created: 1, updated: 0, skipped: [], errors: [] })
    ...
    expect(result.current.importPreview?.response.preview_hash).toBe("preview-hash")
    expect(result.current.pricingTemplates.map((template) => template.name)).toEqual(["Before import"])
