...
- `catalog/`: the shared models.dev source-linked pricing import, mounted by both `/route/pricing` and the model-detail Terminal Target action.
  ...
  - `CatalogOfferingDiscovery.tsx`: pricing-page model selection plus unique-match and paged candidate search, reusing `pages/model-detail/useCatalogCandidates.ts` so both entry points share offset append, stale-response isolation, single-flight, and retry semantics.
