...
- `catalog/`: the shared models.dev source-linked pricing import, mounted by both `/route/pricing` and the model-detail Terminal Target action.
  ...
  - `CatalogPricingDialog.tsx`: the one dialog shell for both surfaces. It keeps discovery mounted beside the preview so the operator can re-choose, renders explicit commit blockers rather than a bare disabled button, and locks closing and mutable controls while a commit is in flight. Success copy uses the commit response's authoritative stored template name.
  ...
  - `CatalogOfferingDiscovery.tsx`: pricing-page model selection plus unique-match and paged candidate search, reusing `pages/model-detail/useCatalogCandidates.ts` (the models.dev adapter over `shared/table/useAppendCandidatePager.ts`) so both entry points share offset append, stale-response isolation, single-flight, retry, and revision-rollover semantics.
