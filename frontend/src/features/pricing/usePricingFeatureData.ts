...
import type { PricingTemplate, PricingTemplateConnectionUsageItem, PricingTemplateImportRequest, PricingTemplateRevision } from "@/lib/types"
...
export function usePricingFeatureData(revision: number) {
  ...
  const [pricingTemplateUsageLoadError, setPricingTemplateUsageLoadError] = useState<string | null>(null)
  ...
  const [pricingTemplateHistoryTemplate, setPricingTemplateHistoryTemplate] = useState<PricingTemplate | null>(null)
  const [pricingTemplateHistoryRevisions, setPricingTemplateHistoryRevisions] = useState<PricingTemplateRevision[]>([])
  const [pricingTemplateHistoryLoading, setPricingTemplateHistoryLoading] = useState(false)
  ...
  const fetchPricingTemplates = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      const detail = error instanceof Error ? error.message : messages.pricingTemplatesData.loadFailed
  ...
  const handleViewPricingTemplateHistory = async (template: PricingTemplate) => {
    const messages = getStaticMessages()
    setPricingTemplateHistoryTemplate(template)
    setPricingTemplateHistoryLoading(true)
    setPricingTemplateHistoryRevisions([])
    try {
      setPricingTemplateHistoryRevisions(await api.pricingTemplates.revisions(template.id))
    } catch (error) {
      toast.error(error instanceof Error ? error.message : messages.pricingTemplatesData.historyLoadFailed)
    } finally {
  ...
  const handleViewPricingTemplateUsage = async (template: PricingTemplate) => {
    ...
    try {
      ...
      const detail = error instanceof Error ? error.message : messages.pricingTemplatesData.loadUsageFailed
      toast.error(detail)
  ...
  const handleImportPricingTemplates = async (request: PricingTemplateImportRequest) => {
    ...
    try {
      ...
      if (response.errors.length > 0 || !response.committable) {
        toast.error(response.errors[0]?.detail ?? messages.common.requestFailed)
        return false
      }
      return true
    } catch (error) {
  ...
  const commitImportPreview = async () => {
    ...
    if (response.errors.length > 0 || !response.committable || !response.preview_hash) {
  ...
  return { cancelImportPreview: () => setImportPreview(null), commitImportPreview, importPreview, closePricingTemplateDialog, deletePricingTemplateConfirm, deletePricingTemplateDisplay, deletePricingTemplateConflict, editingPricingTemplate, handleDeletePricingTemplate, handleDeletePricingTemplateClick, handleEditPricingTemplate, handleImportPricingTemplates, handleSavePricingTemplate, handleViewPricingTemplateHistory, handleViewPricingTemplateUsage, openCreatePricingTemplateDialog, pricingTemplateDeleting, pricingTemplateDialogOpen, pricingTemplateImportDialogOpen, pricingTemplateHistoryLoading, pricingTemplateHistoryRevisions, pricingTemplateHistoryTemplate, pricingTemplateImporting, pricingTemplatePreparingEditId, pricingTemplateSaving, pricingTemplateServerError, pricingTemplateUsageError, pricingTemplateUsageLoadError, pricingTemplateUsageLoading, pricingTemplateUsageRows, pricingTemplateUsageTemplate, pricingTemplates, pricingTemplatesError, pricingTemplatesLoading, fetchPricingTemplates, setDeletePricingTemplateConfirm, setDeletePricingTemplateConflict, setPricingTemplateDialogOpen, setPricingTemplateImportDialogOpen }
}
