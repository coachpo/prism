...
interface AuthFilesTableProps {
  ...
  onDeleteAuthFile: (snapshot: SidecarAuthFile, confirmName: string) => Promise<string | null>;
          ...
          <DialogBody>
            <div className="flex flex-col gap-4 rounded-lg border border-destructive/25 bg-destructive/5 p-4 text-sm">
              ...
              <div className="flex flex-col gap-2">
                ...
                {dialog?.errorMessage ? <p role="alert" className="rounded-md border border-destructive/30 bg-destructive/10 px-2 py-1.5 text-xs text-destructive">{dialog.errorMessage}</p> : null}
              </div>
            </div>
          </DialogBody>
          <DialogFooter>
...
export function AuthFilesTable({
  ...
  useEffect(() => {
    ...
    const refreshedSnapshot = authFiles.find((snapshot) => snapshot.auth_id === authDeleteDialog.snapshot.auth_id);
    if (!refreshedSnapshot) {
      return;
    }
    if (refreshedSnapshot.name !== authDeleteDialog.snapshot.name || refreshedSnapshot.observed_at !== authDeleteDialog.snapshot.observed_at) {
      setAuthDeleteDialog({ confirmName: "", errorMessage: authDeleteDialog.errorMessage, snapshot: refreshedSnapshot });
    }
  }, [authDeleteDialog, authFiles]);
  ...
  const submitDeleteDialog = async (event: FormSubmitEvent) => {
    ...
    const errorMessage = await onDeleteAuthFile(authDeleteDialog.snapshot, authDeleteDialog.confirmName);
    if (errorMessage === null) {
      closeDeleteDialog();
      return;
    }
    setAuthDeleteDialog((current) => current ? { ...current, confirmName: "", errorMessage } : current);
  };
  ...
  return (
    <Card data-testid="sidecar-auth-files">
            ...
            <div className="flex flex-col gap-3 border-t border-border/70 bg-muted/20 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  aria-label={paginationCopy.previousPage}
      ...
      <AuthDeleteDialog
        ...
        onConfirmNameChange={(value) => setAuthDeleteDialog((current) => current ? { ...current, confirmName: value, errorMessage: undefined } : current)}
