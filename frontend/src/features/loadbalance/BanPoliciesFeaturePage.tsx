...
export function BanPoliciesFeaturePage() {
  ...
  return (
    <OperatorPageShell data-testid="ban-policies-feature-page">
      <OperatorPageHeader title={messages.loadbalanceStrategiesPage.title} description={messages.loadbalanceStrategiesPage.description} />
...
function BanPolicyCurrentStatePanel({ revision }: { revision: number }) {
  ...
  return (
    <OperatorSectionCard
      ...
      description={`Inspect retry-window and ban state for one model config ID. Refresh after changes; revision ${revision}.`}
      contentClassName="flex flex-col gap-4"
    >
      <FieldGroup className="gap-4">
        <Field>
          <FieldLabel htmlFor="current-state-model-config-id">Model config ID</FieldLabel>
          <FieldDescription>Use the model row ID.</FieldDescription>
...
function BanPolicyEventsPanel({ revision }: { revision: number }) {
  ...
  return (
    <OperatorSectionCard
      ...
      description={`Review retry scheduled, retry exhausted, banned, unbanned, recovered, and admission rejected events for one model ID. Revision ${revision}.`}
      contentClassName="flex flex-col gap-4"
    >
      <FieldGroup className="gap-4">
        <Field>
