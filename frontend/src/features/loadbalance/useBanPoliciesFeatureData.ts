...
export function useBanPoliciesFeatureData(revision: number) {
  ...
  >(() => initialFragment<LoadbalanceStrategy[]>(STRATEGIES_QUERY_KEY));
  ...
  // Scoped single-flight for per-strategy impact reads: one in-flight page per
  // strategy, never a queue of duplicate appends.
      ...
      setStrategiesFragment((current) => ({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to load routing strategies",
  ...
  const createDefaults = async () => {
    ...
    try {
      ...
      setStrategiesFragment({
        phase: refreshed.length === 0 ? "empty" : "ready",
        data: sortStrategies(refreshed),
        stale: false,
        lastSuccessfulAt: new Date().toISOString(),
        error: null,
        semanticQueryKey: STRATEGIES_QUERY_KEY,
  ...
  const setDefault = async (strategyId: number) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        const conflictCurrentDefaultId =
  ...
  const loadImpactPage = async (strategyId: number, cursor?: string) => {
    // Scoped single-flight: a second click on 加载更多 while one page is in
    // flight is a no-op, not a duplicate request.
    ...
    setImpactStates((states) => {
      const previous =
        ...
        initialFragment<StrategyImpactListResponse>(queryKey);
      return {
        ...states,
        [strategyId]: {
          expanded: true,
    ...
    try {
      ...
      setImpactStates((states) => {
        // An append merges into what is already on screen with stable-ID
        // dedupe; a first page replaces whatever stale draft existed.
        const existing = cursor
          ? states[strategyId]?.fragment.data?.items
          : undefined;
        const data: StrategyImpactListResponse = {
          ...response,
          items: mergeStrategyImpactItems(existing, response.items),
        };
        return {
          ...states,
          [strategyId]: {
      ...
      setImpactStates((states) => {
        ...
        return {
          ...
          [strategyId]: {
            ...
            fragment: {
              ...previous,
              // An append failure keeps the loaded rows on screen in their
              // own phase; only a failed first read becomes an error surface.
              phase: previous.data !== null ? previous.phase : "error",
  ...
  const toggleImpact = async (strategyId: number) => {
    ...
    // Collapse an open list — unless this expansion is stuck on a failed read
    // with nothing committed, where a click means "retry", not "hide".
  ...
  const retryImpact = async (strategyId: number) => {
    const current = impactStates[strategyId];
    // A retry repeats the exact page attempt that failed: an append failure
    // re-fetches its own cursor, a failed first read restarts page one.
    await loadImpactPage(strategyId, current?.lastCursor ?? undefined);
  };
