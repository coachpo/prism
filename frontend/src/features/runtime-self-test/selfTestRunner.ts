...
export async function reconcileSelfTestTelemetry(
  ...
  for (let attempt = 0; attempt < SELF_TEST_POLL_ATTEMPTS; attempt += 1) {
    ...
    if (item) {
      const detail = await api.stats.requestDetail(item.id);
...
export function buildSelfTestResult(
  ...
  const execution: RuntimeSelfTestResult["execution"] = {
    state: detail
      ? detail.summary.status_code >= 200 && detail.summary.status_code < 300
        ? "completed"
        : "failed"
  ...
  const pricing: RuntimeSelfTestResult["pricing"] = {
    state: detail
      ? detail.usage.priced_flag === true
        ? "priced"
        : detail.usage.priced_flag === false
          ? "unpriced"
          : "unknown"
      : "evidence_pending",
    unpricedReason: detail?.usage.unpriced_reason ?? null,
    costMicros: detail?.costing.total_cost_user_currency_micros != null ? String(detail.costing.total_cost_user_currency_micros) : null,
    currency: detail?.costing.report_currency_symbol ?? null,
  };
