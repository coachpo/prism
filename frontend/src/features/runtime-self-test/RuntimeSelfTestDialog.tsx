...
function SelfTestResultPanel({ result }: { result: RuntimeSelfTestResult }) {
  ...
  const credentialText =
    result.credential.attributionState === "identified"
      ? copy.selfTestCredentialIdentified(result.credential.observedProxyApiKeyId ?? 0)
  ...
  const directTone =
    result.direct.state === "succeeded"
      ? copy.selfTestDirectSucceeded(result.direct.statusCode ?? 0)
      : result.direct.state === "http_error"
        ? copy.selfTestDirectHttpError(result.direct.statusCode ?? 0, result.direct.safeSummary ?? "")
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm">
        <span className={result.direct.state === "succeeded" ? "font-medium text-emerald-700" : "font-medium text-destructive"}>
          {directTone}
        </span>
