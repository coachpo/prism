 ...
 */
export function useProxyKeyUsage(keyIds: readonly number[]): ProxyKeyUsageState {
  const queryClient = useQueryClient()
  const stableIds = useMemo(() => [...keyIds].sort((left, right) => left - right), [keyIds])

  const results = useQueries({
    queries: stableIds.map((keyId) => ({
      queryKey: rewriteQueryKeys.global.proxyApiKeyUsage(keyId, USAGE_WINDOW),
      queryFn: () =>
        api.stats.requests({ proxy_api_key_id: keyId, time_range: USAGE_WINDOW, limit: 1 }),
      staleTime: 60_000,
    })),
  })
  ...
  const hasFailure = useMemo(() => results.some((result) => Boolean(result.error)), [results])
  const loading = useMemo(() => results.some((result) => result.isPending), [results])
