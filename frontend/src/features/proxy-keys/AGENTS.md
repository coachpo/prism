...
- Keep ledger/auth/capacity reads in `useProxyKeyLedger.ts`; create, edit, rotate, and delete have separate mutation hooks. Their cache reconciliation belongs to `proxyKeyMutationReconciliation.ts`; each mutation hook owns the safe inline error that keeps its dialog and draft available for recovery.
