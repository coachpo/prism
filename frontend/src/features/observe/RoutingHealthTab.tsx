 ...
 * the two can never be read as one window. Both paginate with the shared
 * honest-state contract: a page turn replaces the rows with skeletons, a
 * same-page refresh keeps matching rows, and only failure marks staleness.
 */
...
function GlobalCurrentStateFragment({
    ...
    const [confirmTarget, setConfirmTarget] =
        useState<GlobalCurrentStateItem | null>(null);
        ...
        async (kind: PageReadKind) => {
            ...
            try {
                const response = await api.loadbalance.listCurrentState(
                    {
                        model_id: modelId,
                        state:
                            states.length > 0 ? (states as never) : undefined,
                        endpoint_id: endpointId
                            ? Number(endpointId)
                            : undefined,
                        terminal_target_id: targetId
                            ? Number(targetId)
                            : undefined,
                        limit: 50,
                        cursor,
                ...
                if (current !== generation.current || controller.signal.aborted)
                    return;
                // Any empty result is an empty state. Gating this on `ready` left the
                // `no_config` case rendering neither the empty state nor the table, so a
                // profile with nothing configured showed an all-but-blank card.
                const phase = response.items.length === 0 ? "empty" : "ready";
                setFragment((fragment) =>
                    commitPagedRead(fragment, response, phase, semanticKey),
                );
            } catch (error) {
                if (current !== generation.current || controller.signal.aborted)
                    return;
    ...
    // Scope changes (filters/cursor) re-run through `load`'s identity; they are
    // replace reads unless nothing was committed yet. The header button issues a
    // same-scope refresh instead.
    useEffect(() => {
    ...
    const resetTarget = async (targetIdValue: number) => {
        ...
        try {
            const response =
                await api.loadbalance.resetCurrentState(targetIdValue);
                ...
                return {
                    ...
                    data: {
                        ...
                        items: current.data.items.map((item) => {
                            if (item.terminal_target.id !== targetIdValue)
                                return item;
            ...
            setResetError(
                error instanceof Error ? error.message : copy.resetFailed,
            );
        } finally {
    ...
    // Deep-linked pages have no client-side stack; the live status then names
    // the work instead of a fabricated page number.
    ...
    // rows; a failed refresh keeps rows behind the staleness badge above.
    ...
    return (
        <OperatorSectionCard
            ...
            actions={
                <div className="flex items-center gap-2">
                    ...
                    {/* A refresh keeps matching rows rendered and never marks them stale
              until its read actually fails. */}
                    <Button
                        type="button"
                        variant="outline"
                        size="sm"
        ...
        >
          ...
          body, and never degrades into "there is nothing here". */}
            ...
            {/* A failed replace read presents the target-page error instead of the
          previous page's rows; they describe a URL this page no longer has. */}
            ...
            {fragment.phase === "empty" && !fragment.reading ? (
                // "Nothing is configured to observe" and "targets exist but none has
                // been observed" are different answers to the operator's question.
          ...
          skeletons under aria-busy. */}
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>
                            {copy.resetCooldownConfirmTitle}
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            onClick={() => {
                                ...
                                if (target)
                                    void resetTarget(target.terminal_target.id);
                            }}
                        >
