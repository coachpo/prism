...
/**
 * Event timeline with honest cursor pagination (C1/C2). Clicking a page or
 * opening a deep link always issues a real read of the target page; a replace
 * read swaps the old rows for skeleton rows so they never masquerade as the
 * new URL's cohort; a same-page refresh keeps matching rows and marks them
 * stale only once its read has actually failed.
 */
export function LoadbalanceEventsFragment({
  ...
  // The last successfully loaded scope key; decides whether the next read is a
  // replace (new scope) or an initial read (nothing committed yet).
  ...
  const eventTypes = normalizeSearchArray(
    search.event_type,
  ) as LoadbalanceEventType[];
  const failureKinds = normalizeSearchArray(
    search.event_failure_kind,
  ) as ListEventsParams["failure_kind"];
  const admissionReasons = normalizeSearchArray(
    search.event_admission_reason,
  ) as ListEventsParams["admission_reason"];
  ...
  const loadEvents = useCallback(
    async (
      context: EventsQueryContextResponse,
      kind: PageReadKind,
      cursorOverride?: string,
    ) => {
      const current = ++generation.current;
      setFragment((fragment) => beginPagedRead(fragment, kind));
      try {
        ...
        // A superseded or aborted response never commits: the generation check
        // drops anything that lost its race to a newer read.
        if (current !== generation.current) return;
        let phase: PagedListState<LoadbalanceEventListResponse>["phase"] =
          "ready";
        if (response.items.length === 0) {
          phase = response.coverage.complete ? "empty" : "partial";
        }
        setFragment((fragment) => commitPagedRead(fragment, response, phase));
  ...
  // Rebuild the signed context when the window changes. A deep-linked cursor
  // survives this effect (the first run never resets paging); any *later*
  // genuine window change invalidates every outstanding cursor because a
  // cursor issued under one window does not address another window's rows.
  ...
  useEffect(() => {
    ...
    // windowKey drives context rebuilds only.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [windowKey]);

  // Page and filter reads: every change of scope (filters or cursor) issues a
  // real request for exactly the URL's target page. The kind follows what the
  // operator asked for — a new scope replaces the rows on screen, a re-run of
  // the same scope (fresh context identity) reads like a refresh.
  useEffect(() => {
    ...
    // fragment.data intentionally excluded: the kind is decided before the read.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contextState.phase, contextState.context, filtersKey, eventCursor]);
  ...
  // Deep-linked pages have no client-side stack, so their absolute index is
  // unknown; the live status then names the work, not a fabricated number.
  ...
  // A failed replace read presents the target-page error instead of rows that
  // belong to the previous page; a failed refresh keeps rows behind the
  // staleness badge in the header.
  ...
  return (
    ...
    >
      ...
      {/* First-read failure: nothing was committed, so the card body becomes
          the error surface rather than an empty table. */}
      {!fragment.reading &&
      fragment.error !== null &&
      fragment.data === null ? (
        <OperatorErrorState
      ...
      {/* Replace failure: the old rows are withheld — they describe the
          previous page, not the URL that failed to load. */}
      ...
      {/* The card supplies the outer border; the table only needs to scroll.
          While a replace read runs, the shell stays and the body shows row
          skeletons under aria-busy. */}
      {showTableShell ? (
                ...
                ? items.map((item) => (
                    <EventRow
                      ...
                      onOpenDetail={() => {
                        ...
                        updateSearch({ event_id: item.event_id }, false);
                      }}
                    />
                  ))
                : null}
      ...
      <LoadbalanceEventDetailSheet
        ...
        onRetryContext={() => {
          void issueContext().then((context) => {
