  ...
  return (
    <div className="flex flex-col gap-[var(--density-card-gap)]">
      {coverageClipped && data.coverage.gaps.length > 0 ? (
        <OperatorCallout intent="warning" data-testid="window-coverage-incomplete">
      ...
      <OperatorKpiCard
        ...
        detail={copy.knownCostCaption(copy.coverageLabel(pricing.pricing_coverage_state))}
        badges={<PricingBreakdown pricing={pricing} />}
      />
      </section>
    </div>
  );
...
function Money({ micros, symbol }: { micros: string | null | undefined; symbol: string }) {
 ...
 */
function PricingBreakdown({ pricing }: { pricing: UsageSummaryResponse["pricing_reconciliation"] }) {
  const { formatNumber, messages } = useLocale();
  const copy = messages.observe;
  const segments: { count: number; key: PricingSegmentKey; label: string }[] = [
    { count: pricing.priced_request_count, key: "priced", label: copy.pricingPriced },
    { count: pricing.unpriced_request_count, key: "unpriced", label: copy.pricingUnpriced },
    { count: pricing.pricing_ineligible_request_count, key: "ineligible", label: copy.pricingIneligible },
    { count: pricing.pricing_unknown_request_count, key: "unknown", label: copy.pricingUnknown },
  ];
  const total = segments.reduce((sum, segment) => sum + segment.count, 0);

  return (
    <div className="flex w-full min-w-0 flex-col gap-1" data-testid="pricing-breakdown">
      <div
        role="img"
      ...
      <div className="flex flex-wrap gap-x-2.5 gap-y-0.5 text-[11px] text-muted-foreground">
        {segments.map((segment) => (
          <span key={segment.key} data-testid={`pricing-${segment.key}`} className="inline-flex items-center gap-1">
            <span
              aria-hidden="true"
              className={cn("size-1.5 rounded-full", PRICING_SEGMENT_CLASS[segment.key])}
            />
            {segment.label}
            <span className="font-mono tabular-nums text-foreground">{formatNumber(segment.count)}</span>
          </span>
        ))}
      </div>
    </div>
