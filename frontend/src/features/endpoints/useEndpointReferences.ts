...
export function useEndpointReferences(endpointIds: number[]) {
  ...
  // Scoped single-flight for per-endpoint detail reads (expand, retry and
  // append all share one lane): one in-flight read per endpoint.
    ...
    async (ids: number[], generations: Record<number, number>) => {
      try {
        ...
        for (const id of ids) {
          ...
          if (!summary) {
            const error = new ApiError("Missing reference summary item", 500, {
            ...
            commitIfCurrent(id, generation, (current) => ({
              ...
              summaries: {
                ...current.summaries,
                [id]: { status: "error", error, generation },
              },
            }));
            continue;
          }
        ...
        for (const id of ids) {
          ...
          commitIfCurrent(id, generation, (current) => {
            ...
            return {
              ...
              summaries: {
                ...current.summaries,
                [id]: { status: "error", error: apiError, generation },
              },
            };
          });
        }
  ...
  const loadDetail = useCallback(
    async (endpointId: number) => {
      const generation = nextGeneration(
        generationByEndpoint.current[endpointId] ?? 0,
      );
      ...
      try {
        ...
        commitIfCurrent(endpointId, generation, (current) => {
          ...
          if (
            previousDetail &&
            (previousDetail.status === "ready" ||
              previousDetail.status === "stale")
          ) {
            return {
              ...current,
              details: {
                ...
                [endpointId]: {
                  status: "stale",
                  value: previousDetail.value,
                  error: apiError,
                  generation,
                  receivedAt: previousDetail.receivedAt,
                },
              },
            };
          }
          return {
            ...
            details: {
              ...current.details,
              [endpointId]: { status: "error", error: apiError, generation },
            },
          };
        });
      }
  ...
  // Load more along the same snapshot cursor. Scoped single-flight: one
  // in-flight page per endpoint. A retry after a failed append re-reads the
  // same cursor from the kept snapshot — a failure never turns the control
  ...
  const loadMore = useCallback(
    async (endpointId: number): Promise<EndpointReferenceDetail | null> => {
      ...
      setState((prev) => ({
        ...
        details: {
          ...prev.details,
          [endpointId]: { status: "loading", generation, previous: snapshot },
        },
      }));
      try {
        ...
        // The failed append keeps the loaded rows on screen as a stale
        // snapshot; the next 加载更多 click retries this same cursor.
        commitIfCurrent(endpointId, generation, (prev) => {
  ...
  const addEndpoint = useCallback((endpointId: number) => {
    generationByEndpoint.current[endpointId] = nextGeneration(
      generationByEndpoint.current[endpointId] ?? 0,
    );
    setState((current) => ({
      summaries: {
        ...
        [endpointId]: {
          status: "loading",
          generation: generationByEndpoint.current[endpointId],
        },
      },
  ...
  const retry = useCallback(() => {
    setState((current) => {
      ...
      for (const id of uniqueIds) {
        summaries[id] = {
          status: "loading",
          generation: generationByEndpoint.current[id] ?? 0,
        };
      }
