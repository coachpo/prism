...
export function EndpointsFeaturePage() {
  ...
  return (
    <main className="operator-page-transition flex flex-col gap-[var(--density-page-gap)]" data-testid="endpoints-feature-page">
      ...
      {!data.isLoading && showReviewToolbar ? (
        <div className="flex flex-col gap-3 rounded-xl border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full xl:max-w-sm">
            ...
            <Input name="endpoints_search" type="search" autoComplete="off" placeholder={copy.searchEndpoints} value={data.searchQuery} onChange={(event) => data.setSearchQuery(event.target.value)} className="h-9 pl-9" />
          </div>
      ...
      {data.isLoading ? (
        ...
        <EmptyState icon={<Plug className="h-6 w-6" />} title={copy.noEndpointsConfigured} description={copy.noEndpointsConfiguredDescription} action={<Button onClick={() => data.setIsCreateOpen(true)}><Plus data-icon="inline-start" />{copy.addEndpoint}</Button>} />
      ) : data.filteredEndpoints.length === 0 ? (
        <EmptyState icon={<Plug className="h-6 w-6" />} title={copy.noEndpointsMatchFilters} description={copy.noEndpointsMatchFiltersDescription} />
      ) : (
