...
export const zhCNMessages = {
  auth: {
    ...
    disabledWarning:
      "能够访问此 Prism 地址的客户端可以管理配置，并可在受支持的路由上发起代理请求。",
    disabledWarningDetails:
      "代理 API 密钥当前不会被强制校验；active 且未过期的 key 仅用于可选归属。",
  ...
  setup: {
    title: "首次配置",
    description:
      "按真实配置事实完成一条可验证的路由链路。读取失败不会被显示成零值或空态。",
  ...
  common: {
    ...
    notApplicable: "不适用",
    pageNotFound: "未找到页面",
    pageNotFoundDescription: (path: MessageArg) =>
      `本实例上没有 ${path} 这个页面。链接可能已过期，或者路径写错了。`,
    backToDashboard: "返回仪表盘",
    thisPage: "当前页面",
    // 路由级兜底：说清哪一页没渲染出来，并留下返回路径；原始报错折进「查看详情」。
    routeRenderFailedTitle: (page: MessageArg) => `${page}渲染失败`,
    routeRenderFailedDescription:
      "这一页没能渲染出来，因此下面不显示任何内容。可以重试，或返回仪表盘。",
  ...
  honesty: {
    ...
    outsideRetentionReason:
      "该时间范围早于当前保留 cutoff，结果不会用零值填补已删除历史。",
  ...
  routingHealth: {
    ...
    unobservedFoldDescription:
      "这些终端目标本进程还没有观测到任何路由结果，逐行读也读不出重试或封禁事实。",
    unobservedFoldExpand: "展开尚未观测的目标",
    unobservedFoldCollapse: "收起尚未观测的目标",
    ...
    currentStateDescriptionDetails:
      "展示当前已配置启用终端目标的跨模型状态；尚未观测的行不会伪装成可用。",
    eventsTitle: "负载均衡事件",
    eventsDescription:
    ...
    attemptsMissingReason: "本进程已观测该目标，但没有记录重试计数。",
    nextRetryAbsentReason: "该目标当前没有待执行的重试。",
    banUntilAbsentReason: "该目标当前没有生效中的封禁到期时间。",
    ...
    windowColumnHint:
      "按事件类型选取一个时间：已封禁看封禁截止，其余优先看下次重试，都没有时回落到上次成功。每行单元格上方标注了该行实际用的口径。",
    windowMissingReason: "该事件既没有封禁截止，也没有下次重试或上次成功时间。",
    viewDetail: "查看详情",
    detailTitle: "事件详情",
    ...
    // 不能和读不到的缺失挤进同一个 `?? "—"`。
    banModeOff: "未启用封禁",
    banModeTemporary: "临时封禁",
    banModeUntilReset: "封禁至手动重置",
    banModeUnknown: "未知封禁模式",
    banModeMissingReason: "该事件没有记录当时的封禁模式。",
    banUntilField: "封禁至",
    banUntilOffReason: "该路由未启用封禁，没有封禁截止时间。",
    banUntilNoneReason: "该目标当时不在封禁中。",
    lastSuccessField: "上次成功",
    lastSuccessMissingReason: "本进程尚未观测到该目标的成功。",
    policyCycleLimitField: "策略每轮上限",
    policyCycleLimitMissingReason: "该事件没有记录当时策略的每轮重试上限。",
    policyBanThresholdField: "策略封禁阈值",
    policyBanThresholdOffReason: "该路由未启用封禁，没有累计阈值。",
    ...
    eventSummary: {
      ...
      bannedReason: (
        ...
        `${failure} 使累计重试达到 ${cumulative} 次${threshold != null ? `，达到封禁阈值 ${threshold} 次` : ""}。`,
  ...
  pricingTierFields: {
    ...
    parityError: "档位价必须与基础费率保持同样的配置状态",
  },
  ...
  settingsDialogs: {
    ...
    userAgentClientRulesTooltip:
      "规则匹配后端提供的调用方与上游 User-Agent 字符串，并为请求日志标注客户端标签。",
  ...
  settingsRetentionDeletion: {
    ...
    retentionPolicyDescription:
      "选择请求日志、审计日志、统计数据和负载均衡事件在后台清理前应保留多久。",
    ...
    manualCleanupPreamble:
      "此操作不可撤销。提交前会先做一次预检，给出将匹配和将保留的行数；确认时需要输入关键词。",
    ...
    preflightDiscarded:
      "本次预检已作废，服务端不会再接受它的确认。请关闭本对话框后重新发起预检。",
    policyPreflightTitle: "确认保留策略变更",
    policyPreflightDescription:
      "此变更会缩短逻辑覆盖或开启自动清理。预检只在短时间内有效。",
    policyPreflightWarning:
      "确认后，新查询会立即按新的逻辑 cutoff 排除更早历史；已清理的记录无法恢复。",
    ...
    jobTerminalNotice: (
      ...
      `清理作业 ${jobId}（${dataset}）已进入终态：${state}。请打开作业中心查看结果。`,
  },
  settingsSaveState: {
    saved: "已保存",
  ...
  settingsAuthentication: {
    ...
    authenticationToggleDescription:
      "登录强制与代理密钥归属是两条独立轴；关闭时有效密钥仍可被识别，缺少密钥的请求会放行但不归属。",
    disableConfirmation:
      "关闭身份验证会让管理面与已注册运行时操作进入 permissive 状态，并清除当前会话。确定继续吗？",
    zeroKeyEnableConfirmation:
      "当前没有处于 30 秒安全窗口内的可用代理密钥。启用后没有有效密钥的运行时请求会被拒绝。仍要启用吗？",
    accountUpdateConfirmation:
      "更新已启用身份验证的操作员账户会使现有操作员会话失效，需要重新登录。确定继续吗？",
    authenticationReadiness: (
      ...
      `代理密钥准备度：${active} 个有效、${safe} 个可安全用于切换、${expired} 个已过期、${disabled} 个已停用。`,
  ...
  settingsAudit: {
    ...
    captureLimitsDescriptionDetails:
      "单次 ingress 原始正文最多 16 MiB。Header scrub 是不可关闭的底线，Header Blocklist 只能增加转发过滤；关闭捕获不会删除历史正文。",
  },
  settingsAuditData: {
  ...
  settingsBilling: {
    ...
    migrationRequired:
      "当前配置需要先完成迁移证据中的货币修复；普通计费保存暂时关闭。",
  },
  settingsCurrencyMigration: {
    archiveButton: "归档未使用的 FX 证据",
    archiveCommit: "确认归档",
    archiveDescription:
      "归档只会记录已证明未使用的遗留 FX 证据，不改变报告货币、epoch、模板修订或真实 FX 源行。",
    archivePreview: "预览归档",
    archiveSummary: (count: MessageArg, total: MessageArg) =>
      `已证明 ${count}/${total} 条遗留 FX 证据未被使用。`,
    archiveSucceeded: (count: MessageArg) => `已归档 ${count} 条遗留 FX 证据。`,
    cancel: "取消",
    code: "代码",
    commitButton: "确认迁移",
    commitFailed: "换币迁移提交失败。",
    commitSucceeded: (code: MessageArg, epoch: MessageArg) =>
      `已切换到 ${code}（epoch ${epoch}）。所有活跃模板已生成新修订。`,
    committing: "正在提交迁移...",
    description:
      "切换报告货币会为每个活跃价格模板生成不可变的新修订（vN+1），旧修订与历史数据保持不变。此操作不可撤销。",
    frozenRevisions: (epoch: MessageArg) =>
      `新修订冻结于 epoch ${epoch}，旧修订永不重算。`,
    ...
    repairDescriptionDetails:
      "请逐项明确输入五个价格；系统不会把 legacy raw 值自动当作新价格。",
  ...
  modelDetail: {
    ...
    modelFallbackTitle: (id: MessageArg) => `模型配置 #${id}`,
    banned: "已封禁",
    cancel: "取消",
    ...
    connectionsLoadOnDemandDescription:
      "终端目标指标按需加载，以避免页面初次打开时产生过大的请求突发。",
    ...
    authTypeGeminiBearer: "gemini（Bearer 兼容网关 / OAuth 令牌）",
    authTypeGeminiAPIKey:
      "gemini_api_key（x-goog-api-key，Google 官方 API Key）",
    delete: "删除",
    disabled: "已禁用",
    displayName: "显示名称",
    ...
    endpointSourceCreateHint:
      "为这个模型私有终端目标选择现有端点，或直接在这里新建一个端点。",
    ...
    requestsLabel: "请求",
    routingPriorityHint:
      "新终端目标会作为回退项追加。请在模型详情列表中拖动卡片以调整路由优先级。",
    ...
    setup: "设置",
    setupDescription:
      "选择此终端目标发送请求的端点，并决定 Prism 应如何标记它。",
    ...
    customRequestParametersDescription:
      "添加 Prism 应随此终端目标一起发送的可选 JSON 请求体参数。",
    customRequestParametersDescriptionDetails:
      "同名顶层字段会覆盖客户端值；模型、流式模式与请求内容字段不可设置。",
    customRequestParametersSummary: (count: MessageArg) =>
      `已配置 ${count} 个顶层参数`,
    ...
    customRequestParametersProtectedDetails:
      "受保护字段：model、models、stream、messages、input、contents、instructions、system、systemInstruction。",
    customRequestParametersNotSecretHint: "这里不是秘密存储。",
    customRequestParametersNotSecretDetails:
  ...
  modelDetailData: {
    ...
    modelConfigNotFoundTitle: "找不到这个模型配置",
    modelConfigNotFoundDescription: (id: MessageArg) =>
      `模型配置 #${id} 不存在，可能已被删除。列表里没有它不是数据问题。`,
    invalidModelRouteTitle: "无效的模型配置路由",
    invalidModelRouteDescription: (id: MessageArg) =>
      `模型配置 ID '${id}' 不是有效的标识符，这条链接抄错了。`,
  ...
  endpointsUi: {
    ...
    copyBaseUrl: (name: MessageArg) => `复制端点 ${name} 的基础 URL`,
    createDescription:
      "保存可复用的上游地址与凭证；创建后还需将它附加为某个模型的终端目标。",
    created: (date: MessageArg) => `创建于 ${date}`,
    deleteEndpoint: "删除端点",
    ...
    deleteEligibleConfirm:
      "这会永久删除基础 URL 与已保存的 API 密钥。此操作无法撤销。",
    deleteIntegrityError:
      "引用数据存在完整性错误，已停止删除以保护数据。请先在对应模型中检查终端目标。",
    deleteRetry: "重试",
    ...
    openReferences: (name: MessageArg, count: MessageArg) =>
      `展开 ${name} 的直接引用（${count} 条）`,
    orphanCleanup: "清理孤立配置",
    orphanCleanupConfirm: (_name: MessageArg, id: MessageArg) =>
      `确定清理孤立终端配置 #${id} 吗？仅删除此无所属模型的终端配置，不会删除端点。`,
  ...
  endpointsData: {
    ...
    updated: "端点已更新",
    updateFailed: "更新端点失败",
    endpointStale: "该端点已被其他人修改，已载入最新内容",
    verifyFailed: "验证失败",
  ...
  routing: {
    ...
    enableCopiesDescription:
      "将新目标设为参与路由。默认关闭，避免复制动作静默改变目标模型的流量。",
    enableCopiesImpact:
      "开启后新目标会参与各目标模型的路由；模型自身停用状态不会被改变。",
    ...
    runtimePartialObservationReason:
      "本进程见过该终端目标，但这次快照缺少部分字段，因此不展示可能误导的数值。",
    runtimeOutOfCohort: "不在观测范围",
    runtimeOutOfCohortReason:
      "该访问目标未参与路由，因此不进入本进程的运行态观测集合。",
    runtimeReadFailed: "运行态读取失败",
    runtimeReadFailedReason: (reason: MessageArg) =>
      `未能读取运行态，因此这里既不是「无观测」也不是「正常」：${reason}`,
    runtimeTruncated: "运行态已截断",
    runtimeTruncatedReason:
      "运行态列表已被分页截断；未列出的目标不能据此判断为没有观测。",
    runtimeNotReadYet: "运行态尚未读取成功，因此这里还不能断定该目标没有观测。",
    runtimeAbsentFromCohortReason:
      "本次读取成功，但返回的观测集合里没有这个终端目标；这与「本进程尚未观测」不是同一件事。",
    inFlightNotMetered: "未计量",
    inFlightNotMeteredReason:
      "该终端目标没有配置在途上限，运行时因此不对在途请求计数；这里的空缺表示未计量，而不是实测为零。",
    pricingTemplate: "价格模板",
  ...
  modelsUi: {
    ...
    targetColumnCapabilityBasis:
      "口径：各目标自己声明的能力。模型目标不直接声明，实际能力取决于它解析到的终端目标；是否真的可路由请看上方的路由就绪。",
    ...
    targetLimitsNotApplicable:
      "这一行不直接持有终端配置；限流配置在它解析到的终端目标上，按各终端目标分别生效。",
    targetRuntimeNotApplicable:
      "这一行自身没有运行态观测键；可观测的是它解析到的终端目标的状态。",
    targetCapabilityNotApplicableModel:
      "这一行不直接声明能力；实际能力取决于它解析到的终端目标。",
  ...
  pricingTemplatesUi: {
    ...
    description:
      "管理可在模型和端点间复用的价格模板；所有价格均按每 100 万令牌计",
    endpoint: "端点",
    ...
    listBasis:
      "「近 7 天有改动」按当前版本的生效时间统计；其余卡片是全量配置事实，不带时间窗。",
    ...
    rateBasisPeakValley: (timezone: MessageArg, windows: MessageArg) =>
      `费率列并列「峰段 – 谷段」。峰段由 ${timezone} 下的 ${windows} 段路由时段判定，其余时间按谷段计费。`,
    rateBasisPeakValleyUnknown:
      "费率列并列「峰段 – 谷段」。峰谷窗口缺失或读取不完整，无法确认某一时刻落在哪一档。",
    rateInput: "输入",
    rateOutput: "输出",
    ...
    kindPeakValley: "峰谷",
    unknownKind: "类型未知",
    multiCardSummary: (count: MessageArg) => `${count} 张价格卡`,
    cardStandard: "普通卡",
    cardTierBase: "阶梯基础卡",
    cardTierAbove: "阶梯超阈值卡",
    cardPeak: "峰段卡",
    cardOffpeak: "谷段卡",
    tierConfigured: "阶梯",
    ...
    rateUnconfiguredReason:
      "该专项费率没有配置值。它不等于 0：未配置时按基础费率计费口径处理。",

    // 引用计数（来自列表分页接口）。
    referencesUnavailable: "引用信息读取失败",
    referencesUnavailableReason:
      "这次引用信息读取没有成功，因此不显示计数；这不代表该模板没有被引用。",
    referencesSummary: (
      models: MessageArg,
    ...
    importPreviewStructureChanged: "计价结构改变",
  ...
  pricingTemplatesHistory: {
    empty: "该模板没有已记录的修订。",
    scheduleUnavailable: "峰谷窗口缺失或读取不完整，无法确认窗口归属。",
    unknownKind: "模板类型未知，无法安全展示修订。",
    loading: "加载修订历史...",
    subtitle: (name: MessageArg, version: MessageArg) =>
      `${name}（当前 v${version}）——不可变修订记录，历史永不重算。`,
    tableCurrency: "货币",
    ...
    tableKind: "类型",
    tableCards: "价格卡",
    structureChanged: "计价结构改变",
    // 「计价结构改变」标在做出改动的那一版上；哪一版此刻在计费另说。
    currentVersion: "当前生效",
  ...
  proxyApiKeys: {
    ...
    clearExpiry: "清除过期时间",
    neverRotated: "未轮换",
    authenticationOff: "身份验证已禁用",
    authenticationOn: "身份验证已启用",
    ...
    disabled: "已禁用",
    editDescription:
      "更新这个已发放密钥的名称、备注、过期时间和启用状态。轮换密钥是单独的操作。",
    ...
    accessNoEligibleModels:
      "当前没有已启用的可用模型。请先在模型中启用一个模型，再生成调用样例。",
    ...
    selfTestTitle: "运行时自检",
    selfTestDescription:
      "向 Prism 运行时发送一次真实的最小请求，验证密钥识别、模型路由、实际执行与定价状态。",
    selfTestRun: "发送测试请求",
    ...
    selfTestDirectSucceeded: (status: MessageArg) =>
      `调用成功（HTTP ${status}）`,
    selfTestDirectHttpError: (status: MessageArg, summary: MessageArg) =>
      `请求失败（HTTP ${status}）${summary}`,
    ...
    lifecycleRetireCredential:
      "立即不再被识别；auth-on 下拒绝，auth-off 下继续匿名放行",
    lifecycleRotateCredential:
      "旧凭证立即失效，密钥 ID 与用量归属不变，新密钥只显示一次",
    ...
    issueKey: "发放密钥",
    issueKeyDescription:
      "填写名称与可选备注即可发放一把新的客户端凭证。明文只显示一次。",
    verifyAccess: "验证接入",
    verifyAccessDescription:
      "粘贴一把客户端持有的代理密钥，向运行时发送一次真实的最小请求，确认它现在能否通过网关。",
    verifyAccessKeyLabel: "代理密钥明文",
    verifyAccessKeyPlaceholder: "粘贴密钥",
    verifyAccessKeyHelp:
      "台账不保存明文，这里粘贴的值只在本次验证中使用，不会被发送到管理接口。",
    verifyAccessNoKey: "不带密钥测试",
    verifyAccessNoKeyHelp:
      "只在身份验证关闭时有意义：验证匿名请求是否仍被放行。",
    ...
    columnRequests7dBasis:
      "口径：最近 7 天归属到该密钥的请求条数，与本表其余列的时间窗不同。",
  ...
  requestLogs: {
    ...
    auditWindowBasis:
      "以请求时间为中心的 ±12 小时；超出该范围的审计记录不会出现在下表。",
    ...
    auditCoverageEffective: (from: MessageArg, to: MessageArg) =>
      `实际覆盖 ${from} — ${to}`,
    auditCoverageIncompleteTitle: "窗口内未返回审计记录，且覆盖不完整",
    auditCoverageIncompleteDescription: (gaps: MessageArg) =>
      `此请求的审计查询窗口内没有返回记录；其中 ${gaps} 段区间已超出保留期，不能确认没有审计记录。`,
    auditCoverageIncompleteNoGaps:
      "此请求的审计查询窗口内没有返回记录；后端报告本次查询覆盖不完整，不能确认没有审计记录。",
    auditRequestLoading: "请求信息读取中",
    auditListLoading: "审计记录读取中",
    auditPayloadLoading: "审计载荷读取中",
    auditDisabledSettingsLink: "打开审计日志设置",
    ...
    viewBasisIngressChains: "口径：按入口请求聚合，一次入口请求算一行",
    ...
    pricingCardRole: "价格卡角色",
    allPricingCardRoles: "所有价格卡角色",
    pricingCardStandard: "普通卡",
    pricingCardTierBase: "阶梯基础卡",
    pricingCardTierAbove: "阶梯超阈值卡",
    pricingCardPeak: "峰段卡",
    pricingCardOffpeak: "谷段卡",
    pricingSelectionState: "价格卡判定态",
    ...
    pricingSelectionUnresolved: "无法求值",
    statusFamily: "状态族",
    finalResult: "最终结果",
    ...
    chainViewAuditAria: (id: MessageArg) => `查看入口请求 ${id} 的审计记录`,
    chainCopyRequestIdAria: (id: MessageArg) => `复制入口请求 ${id} 的请求 ID`,
    viewAudit: "查看审计",
    copyRequestId: "复制请求 ID",
    chainRequestLogIdUnavailable: "该链没有可用的请求日志行，无法打开详情或审计",
    ...
    headerMalformed: (title: MessageArg) =>
      `${title}格式异常，已降级显示；请切换到原始 JSON 查看。`,
    ...
    unpricedCauseNonStreamNoUsage: "上游响应体里没有 usage 字段，无法计价。",
    unpricedCausePricingDisabled: "该连接未绑定价格模板，按不计费处理。",
    unpricedCauseStreamNoUsage:
      "流式响应正常结束，但上游没有在流里返回令牌用量。Prism 已自动为 OpenAI 流式请求附加 stream_options.include_usage，该上游未按约定返回；也可能是客户端显式关闭了该字段。",
    ...
    pricingSelection: "价格卡选择",
    pricingSelectionUnavailable: "选择证据不可用",
    pricingSelectorThreshold: "选择器阈值",
    pricingSelectorBasis: "选择器基数",
    ...
    auditCaptureDisabledForVendor:
      "此请求执行时审计日志处于关闭状态，因此 Prism 没有存储单独的审计行。",
    auditDisabledAtRequest: "请求开始时已禁用审计",
    auditDisabledDescription:
      "由于该请求开始时审计日志处于关闭状态，因此这里只保留了请求日志元数据。",
    auditFullCapture: "完整捕获",
    auditFullCaptureDescription:
      "该请求开始时审计日志和正文捕获都已开启。即使如此，流式响应仍可能没有可存储的响应正文。",
  ...
  observe: {
    ...
    readMissingField: "本次读取没有返回该字段。",
    ttftNoSample: "窗口内没有可测的首字耗时样本。",
    ttftNoQuantile: (samples: MessageArg) =>
      `窗口内有 ${samples} 个首字耗时样本，但本次读取没有返回 P95 分位值。`,
    outputRateNoSample: "窗口内没有可测的输出速率样本。",
    outputRateNoAverage: (samples: MessageArg) =>
      `窗口内有 ${samples} 个输出速率样本，但本次读取没有返回平均值。`,
    costValueUnparsable: "本次读取返回的成本数值无法解析。",
    seriesNoBucketObservation: "该序列在这个时间桶没有观测。",
    seriesNoWindowSample: "该序列在窗口内没有可用样本。",
    bucketNoLatencySample: "该时间桶没有可用的耗时样本。",
    bucketComponentNotReported: "该时间桶没有回报这一分量。",
    activityNoTtft: "这条请求没有记录首字耗时。",
    activityNoTokens: "这条请求没有回报令牌用量。",
    ttNoLatencySample: "该终端目标在窗口内没有可测的耗时样本。",
    nowTpmNoTokenSample: "滚动窗口内的请求没有回报可用的令牌用量。",
    ...
    pageTitle: "仪表盘",
    pageDescription:
      "系统总览与健康状态：先看此刻是否繁忙或受限，再用统一时间窗检查请求、延迟与花费。",
    overviewTab: "概览",
    analyticsTab: "分析",
    eventsTab: "事件",
    ...
    bucketBasis: (name: MessageArg) => `${name}桶`,
    ...
    workbenchScopeBasis:
      "口径：只在已加载的最近一页流水中匹配，不是该条目在整个窗口内的总数；完整集合请用「在请求日志中查看全部」。",
    workbenchNoMatches:
      "已加载的这一页里没有匹配的请求。窗口内可能仍有更早的匹配。",
    streamFailures: "流失败",
  ...
  opencodeExport: {
    ...
    description: "导出 OpenCode 1.18.27 自定义 Provider；模型身份、协议、网关地址和当前价格由 Prism 决定，安全元数据来自已保存的 models.dev 来源与人工覆盖。",
    commonSettingsDescription: "客户端将通过这里填写的 Prism Gateway origin 访问入口模型。导出键使用独立的 prism 或 prism- 前缀，避免覆盖 OpenCode 内置 Provider。",
    providerIdHint: "默认 prism；也可使用 prism- 开头的小写字母、数字、连字符或下划线键名，前缀后首位须为字母或数字。",
    providerIdInvalid: "请输入 prism，或 prism- 后接小写字母或数字开头的键名；其余字符仅允许小写字母、数字、- 和 _。",
    protocolColumn: "客户端 SDK / API 路径",
    metadataColumn: "最终元数据与来源",
    metadataDetails: "查看最终值与来源",
    metadataHint: "只读取已持久化的 models.dev 安全字段；Pi 绑定与实时目录可达性不影响此导出。缺少有效的正数上下文或输出上限时，该模型不可选。",
    automaticReasoningHint: "OpenCode 可能依据 SDK、模型 ID 与能力字段自动选择推理默认行为；这不代表已完成人工参数适配。本页不配置自定义推理选项。",
    completeMetadata: "到模型详情补全 models.dev 元数据",
    emptyHint: "调整筛选条件，或先在模型配置中启用可路由的文本入口并补全上下文、输出上限。",
    fieldLabel: "字段",
    sourceValue: "已保存来源",
    overrideValue: "人工覆盖",
    finalValue: "最终值",
    originLabel: "最终值来源 / 缺失原因",
    absent: "—",
    trueValue: "是",
    falseValue: "否",
    emptyList: "空列表",
    ...
    unknownValue: "不支持的值",
    unknownSource: "来源未知",
    missingField: "没有可导出的有效值；此字段将省略。",
    invalidField: "值无效，未作为最终值采用。",
    unknownReason: "当前模型不符合导出条件，请检查模型配置。",
    keyModeNoneHint: "输出 env=[PRISM_API_KEY] 并省略 apiKey；启动 OpenCode 前设置 PRISM_API_KEY 环境变量。",
    costZeroDisclaimer: "只在所有可达目标的标准美元每百万 token 价格完整、一致且推理价等于输出价时导出 cost。阶梯、峰谷、缺失或冲突会省略整组；OpenCode 可能显示零估算，这不等于免费。",
    providerMergeHint: "合并片段是单数 provider 下的一组自定义 Provider。将它合入现有 opencode.json 的 provider 对象，保留其他键；也可用 OPENCODE_CONFIG 指向完整文件。已有项目与用户配置的合并优先级仍会影响最终设置。",
    ...
    fields: {
      ...
      tool_call: "工具调用",
      temperature: "温度参数",
      modalities_input: "输入模态",
      modalities_output: "输出模态",
      limit_context: "上下文上限（token）",
      limit_input: "输入上限（token）",
      limit_output: "输出上限（token）",
    },
    provenance: {
      prism_display_name: "Prism 显示名称",
      models_dev_source: "models.dev 已保存来源",
      models_dev_override: "models.dev 人工覆盖",
      model_id: "Prism 模型 ID",
      none: "无有效来源",
    },
    modalities: {
      text: "文本",
    ...
    issues: {
      missing: "字段缺失",
      invalid_json: "已保存值无法解析",
      invalid_string: "文本值无效",
      invalid_boolean: "不是布尔值",
      invalid_limit: "上限必须为有效数值，上下文与输出须大于零",
      unsupported_modality: "包含不受支持的模态",
      exceeds_context: "输入或输出上限大于上下文上限",
    },
    unselectableReasons: {
      model_disabled: "模型未启用",
      no_accepted_text_operations: "未配置文本操作",
      no_reachable_terminal_target: "主文本操作没有静态可达的终端目标",
      invalid_metadata_limits: "缺少有效且成组一致的上下文 / 输出上限，请到模型详情补全 models.dev 元数据",
      unaddressable_model_id: "当前协议无法通过 Prism 路径表达此完整模型 ID",
    },
  },
  modelExportPage: {
    title: "导出客户端配置",
    ...
    searchPlaceholder: "搜索模型 ID、显示名称或合并名称...",
    familyFilterLabel: "API 家族",
    familyAll: "全部家族",
    metadataFilterLabel: "元信息完整度",
    metadataAll: "全部元信息状态",
    metadataComplete: "元信息完整",
    metadataIncomplete: "元信息有缺失",
    priceCompleteOnly: "仅显示价格完整",
    refreshSource: "刷新导出源",
    refreshingSource: "正在刷新导出源...",
    ...
    candidateEvidenceTitle: "候选模板证据",
    candidateFieldAbsent: "pi.dev 候选未提供该字段",
    rebindAction: "重新绑定",
    rebindDialogTitle: "重新绑定 Pi 来源",
    rebindDialogDescription:
      "从当前完整 model_id 且 API 兼容的目录坐标中明确选择；不会自动沿用、合并或选择首项。",
    rebindCurrentLabel: "当前绑定",
    rebindNoCandidates: "当前目录没有完整 ID 且 API 兼容的候选。",
    ...
    overrideDialogDescription:
      "逐字段核对绑定来源、当前覆盖和最终生效值；只有明确选择手动值或恢复来源的字段会写入。",
    ...
    overrideModeValue: "写入手动值",
    overrideModeRestore: "恢复绑定来源",
    overrideWillNotChange: "不会写入此字段。",
    overrideWillRestore: "保存后清除此字段的手动覆盖。",
    overrideBooleanPlaceholder: "选择是或否",
    overrideBooleanTrue: "是",
    overrideBooleanFalse: "否",
    overrideInputPlaceholder: "text, image",
    overrideBooleanRequired: "请选择是或否。",
    overrideNameRequired: "名称不能为空。",
    overrideInputInvalid: "仅支持 text 和 image，以逗号分隔。",
    ...
    overrideClearAllDescription:
      "此操作会删除这个模型的全部手动覆盖，并立即恢复使用冻结来源值。",
    ...
    warnNotUsd: "非 USD 计价",
    warnNotPerMillion: "计价单位非 PER_1M",
    warnIncomplete: "价格组件未配置完整",
    warnReasoningMismatch: "reasoning 价 ≠ output 价",
    warnTargetConflict: "多目标价格不一致",
    warnPeakValley: "峰谷价格无法表达",
    warnTierUnrepresentable: "阶梯价格该客户端不支持",
    warnMetadataInvalid: "部分已保存元数据无效，对应字段已省略；查看最终值与来源了解原因。",
    warnMetadataIncomplete: "元信息不完整，缺失字段保持省略",
    warnPiSourceFieldsDropped:
      "pi.dev 来源包含不安全或不受支持的字段，已丢弃并保留证据",
    warnUnsupportedInputModality: "存在客户端不支持的输入模态，已安全省略",
    warnMixedBaseUrls: "多个 base URL，仅逐模型携带",
    warnGeneric: "存在风险提示",

    keyDialogTitle: "密钥嵌入方式",
    keyDialogDescription:
      "密钥只在最终确认时使用并去除首尾空白；不会进入 source、URL、数据库、缓存或日志。客户端展示的 0 不代表免费，缺失的价格组表示未配置。",
    keyModeLegend: "最终代理密钥",
    keyModeNone: "不嵌入密钥",
    keyModeNoneHint: "生成的文件不含任何凭据字段，由客户端自行配置认证。",
    keyModeManual: "手动输入统一密钥",
    keyModeManualHint:
      "为客户端显式写入本次手动填写的 Prism 代理密钥字段；密钥必填，确认时去除首尾空白。",
    ...
    warningsTitle: "本次导出省略或降级的事实",
    noWarnings: "无提示",
    rawViewButton: "在新标签页查看原始 JSON",
    copied: "已复制",
    copyButton: "复制",
    ...
    closeSheet: "关闭并清除",
    costZeroDisclaimer:
      "提示：文件中显式的 0 是已配置的零价；缺失的 cost 组表示价格未配置或不完整，客户端显示 0 不代表免费。",
  },
};
