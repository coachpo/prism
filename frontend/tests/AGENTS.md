...
- Pi export/binding typed-client seam coverage lives in `lib/model_export_api_contract.test.mjs` (public `api.modelExport`, single-model GET, literal routes, `no-store`, the `PiRefreshCommitRequest` `expected_*` body shape, search `offset`/`checked_at`, and AbortSignal transport ownership).
- Catalog/Pi component behavior coverage lives in `../src/test/catalog-metadata-card.test.tsx` (hook-level last-good retention, honest loading/error/stale/unbound, unbind snapshot/conflict, and timezone stamps), `../src/features/models/catalog/pi/PiBindingSourceDialog.test.tsx` (no preselect, stale read-only, model-scoped reset, append retry single-flight, revision-rollover selection reset/acknowledgement), `../src/features/models/detail/ExternalCatalogSourcesSection.test.tsx` (per-source independence, Pi last-good and identity integrity), and `../src/shared/table/useAppendCandidatePager.test.tsx` (replace/append/retry/dedupe/single-flight/A→B→A/old-append/rollover, atomic evidence, and `models.dev` vs `pi.dev` revision-key isolation).
...
- Shared contract seams: `lib/*.test.mjs`
...
- Tests that remain must run in CI or document why they do not. Wire commands by glob: `pnpm exec vitest run`, `pnpm run test:lib`, and the Playwright journey specs are CI gates.
