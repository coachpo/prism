...
async function installObserveRoutes(page: import("@playwright/test").Page) {
  ...
  await page.route("**/api/stats/query-context**", (route) => fulfill(route, {
  ...
  await page.route("**/api/stats/usage-summary**", (route) => fulfill(route, {
    generated_at: "2026-08-09T00:00:00Z",
  ...
  await page.route("**/api/stats/usage-series**", (route) => fulfill(route, { generated_at: "2026-08-09T00:00:00Z", coverage: { requested_preset: "24h", from_time: "2026-08-08T00:00:00Z", to_time: "2026-08-09T00:00:00Z", source: "raw", complete: true, gaps: [] }, metric: "requests", group_by: "none", selection_basis: "request_count", interval: "1h", series_limit: 6, truncated: false, series: [] }));
  await page.route("**/api/stats/dashboard/now**", (route) => fulfill(route, { generated_at: "2026-08-09T00:00:00Z", health: { stale: false, cache_lag_ms: null }, rolling: { window_minutes: 30, coverage: { requested_preset: "rolling", from_time: "2026-08-09T00:00:00Z", to_time: "2026-08-09T00:30:00Z", source: "raw", complete: true, gaps: [] }, request_count: 0, token_sample_count: 0, token_coverage_complete: true, token_count: null, rpm: null, tpm: null }, enabled_model_count: 0 }));
  await page.route("**/api/stats/observe-activity**", (route) => fulfill(route, { generated_at: "2026-08-09T00:00:00Z", coverage: { requested_preset: "24h", from_time: "2026-08-08T00:00:00Z", to_time: "2026-08-09T00:00:00Z", source: "raw", complete: true, gaps: [] }, items: [], has_more: false }));
  await page.route("**/api/stats/usage-errors**", (route) => fulfill(route, { generated_at: "2026-08-09T00:00:00Z", coverage: { requested_preset: "24h", from_time: "2026-08-08T00:00:00Z", to_time: "2026-08-09T00:00:00Z", source: "raw", complete: true, gaps: [] }, requests_context: { view: "ingress_chains", query_context: "narrow-observe-context", final_from_time: "2026-08-08T00:00:00Z", final_to_time: "2026-08-09T00:00:00Z", base_request_filters: {} }, summary: { request_count: 0, http_error_count: 0, stream_error_count: 0, failed_count: 0, client_disconnected_count: 0, diagnostic_stream_anomaly_count: 0 }, timeline: [], http_statuses: [], stream_outcomes: [], groups: [], other: { http_statuses: { count: 0, denominator: 0, percentage: null, request_filters: null }, stream_outcomes: { count: 0, denominator: 0, percentage: null, request_filters: null }, groups: { count: 0, denominator: 0, percentage: null, request_filters: null } } }));
  await page.route("**/api/loadbalance/current-state**", (route) => fulfill(route, { generated_at: "2026-08-09T00:00:00Z", scope: "process", instance_id: "narrow", configuration_revision: "1", completeness: { state: "no_config", complete: true, configured_target_count: 0, observed_target_count: 0, unobserved_target_count: 0, observed_subset_counts: null }, items: [], has_more: false, next_cursor: null }));
  await page.route("**/api/loadbalance/events/query-context**", (route) => fulfill(route, { query_context: "narrow-events-context", requested_preset: "24h", event_bounds: { from_time: "2026-08-08T00:00:00Z", to_time: "2026-08-09T00:00:00Z" }, coverage: { complete: true, gaps: [] }, source_status: { delivery: "best_effort", transition_ledger_complete: false, dropped_event_count: null }, generated_at: "2026-08-09T00:00:00Z" }));
  await page.route("**/api/loadbalance/events**", (route) => fulfill(route, { items: [], has_more: false, next_cursor: null, coverage: { complete: true, gaps: [] } }));
}
...
test("narrow 390x844 observe page has no horizontal overflow and all tabs reachable by keyboard", async ({ page }) => {
...
test("narrow 390x844 request logs table keeps identity column reachable without horizontal page scroll", async ({ page }) => {
  await page.setViewportSize(NARROW_VIEWPORT);
...
test("reduced motion preference does not break observe page rendering", async ({ page }) => {
  await page.setViewportSize(NARROW_VIEWPORT);
  ...
  await page.screenshot({ path: "artifacts/evidence/observe-390-reduced-motion.png", fullPage: true });
});
...
test("narrow viewport visual evidence captures", async ({ page }) => {
  ...
  await page.goto("/observe");
  await expect(page.getByTestId("observe-page")).toBeVisible();
  await page.screenshot({ path: "artifacts/evidence/observe-390-overview.png", fullPage: true });
  ...
  await page.screenshot({ path: "artifacts/evidence/observe-390-events.png", fullPage: true });

  await page.goto("/observe/requests?view=attempts");
  await expect(page.getByTestId("request-logs-table")).toBeVisible();
  await page.screenshot({ path: "artifacts/evidence/requests-390-list.png", fullPage: true });
});

test("connection dialog visual evidence at 1440x900 and 390x844", async ({ page }) => {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (pathname === "/api/endpoints/connections") return fulfillJson({ items: [{ id: 1, endpoint_id: 1, name: "OpenRouter" }] });
    if (pathname === "/api/models/5/connections") return fulfillJson([connection]);
    if (pathname === "/api/models/5/targets") return fulfillJson(modelDetail.access_targets);
    if (pathname === "/api/models/5" && request.method() === "GET") return fulfillJson(modelDetail);
    if (pathname === "/api/models" && request.method() === "GET") return fulfillJson([modelDetail]);
    if (pathname.startsWith("/api/models/5/routing-diagnostics")) return fulfillJson({ model_config_id: 5, openai_accepted_format: "dual_native", strategy: { id: 11, type: "fill-first" }, accepted_operations: ["openai.chat_completions"], targets: [], operation_routes: [], configuration_warnings: [] });
    return fulfillJson({});
  });
  ...
  await page.screenshot({ path: "artifacts/evidence/connection-dialog-1440.png" });
  ...
  expect(noHorizontalOverflow).toBe(true);
  await page.screenshot({ path: "artifacts/evidence/connection-dialog-390.png" });
});
