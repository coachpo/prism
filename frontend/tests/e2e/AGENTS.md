...
- Model access-target authoring journey: `models-access-target-authoring.spec.ts` (including entry/upstream identity follow, manual decoupling, handoff, hydration, prefill, clear rejection, and the URL-backed entry/Model Target/all inventory views)
- Request-log + audit journey: `request-log-dedicated-audit-page.spec.ts` (including hidden-by-default upstream identity, detail/final summary, and server CSV download); shared request-log/audit fixture builders: `request-log-dedicated-audit-fixtures.ts`
