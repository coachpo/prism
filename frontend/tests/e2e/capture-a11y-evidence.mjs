...
async function installRoutes(page) {
  await page.route("**/*", async (route) => {
    ...
    if (pathname === "/api/endpoints" && request.method() === "GET") return fulfill([endpointOne, endpointTwo]);
    if (pathname === "/api/endpoints/references/batch" && request.method() === "POST") {
      return fulfill({ items: [{ endpoint_id: 21, summary: summary(2, 1, 1) }, { endpoint_id: 22, summary: summary(0, 0) }] });
    }
    if (pathname === "/api/endpoints/21/references" && request.method() === "GET") return fulfill(detail21);
    if (pathname === "/api/endpoints/21" && request.method() === "DELETE") {
      return fulfill({ detail: { code: "endpoint_in_use", message: "Endpoint is referenced by Terminal Targets", endpoint_id: 21, summary: summary(2, 1, 1), reference_page: detail21.reference_page, references_url: "/api/endpoints/21/references" } }, 409);
    }
...
async function run() {
  ...
  const trigger = page.getByTestId("endpoint-row-21").getByRole("button", { name: /展开 endpoint-one/ });
  check("reference trigger has aria-expanded=false initially", (await trigger.getAttribute("aria-expanded")) === "false");
  ...
  await trigger.focus();
  check("reference trigger focusable via keyboard", await page.evaluate(() => document.activeElement?.getAttribute("aria-expanded") === "false"));
  await page.keyboard.press("Enter");
  await page.waitForSelector('[data-testid="endpoint-references-21"]');
  check("Enter expands disclosure", (await page.getByTestId("endpoint-row-21").getByRole("button", { name: /展开 endpoint-one/ }).getAttribute("aria-expanded")) === "true");
  ...
  const urlCode = page.getByTestId("endpoint-row-21").locator("code[tabindex='0']").first();
  await urlCode.focus();
  check("base URL cell keyboard-focusable", await page.evaluate(() => document.activeElement?.textContent?.includes("https://one.example") === true));
  ...
  check("blocked delete heading focused", await page.evaluate(() => document.activeElement?.getAttribute("data-testid") === "delete-blocked-heading"));
  check("blocked delete has no destructive submit", (await page.getByTestId("delete-endpoint-confirm").count()) === 0);
  ...
  check("orphan cleanup uses separate AlertDialog", await page.getByTestId("orphan-cleanup-confirm").isVisible());
  ...
  check("references row renders role=status content", (await page.getByRole("status").count()) >= 0);
