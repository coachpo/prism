...
test.describe("statistics endpoint avg token rate", () => {
  test("renders backend token-rate values in top-level and expanded endpoint rows", async ({ page }) => {
    ...
    await expect(headerRow.locator(":scope > div").nth(1)).toHaveText("Avg Token Rate");
    await expect(headerRow.locator(":scope > div").nth(2)).toHaveText("Requests");
    await expect(headerRow.locator(":scope > div").nth(3)).toHaveText("Success Rate");
    await expect(headerRow.locator(":scope > div").nth(4)).toHaveText("Total Tokens");
    ...
    await expect(endpointModelHeaders.nth(1)).toHaveText("Avg Token Rate");
    await expect(endpointModelHeaders.nth(2)).toHaveText("Requests");
    await expect(endpointModelHeaders.nth(3)).toHaveText("Success Rate");
    await expect(endpointModelHeaders.nth(4)).toHaveText("Total Tokens");
