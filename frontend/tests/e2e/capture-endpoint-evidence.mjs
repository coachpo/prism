...
async function installRoutes(page, mode) {
  await page.route("**/*", async (route) => {
    ...
    if (pathname === "/api/endpoints" && request.method() === "GET")
      return fulfill([endpointOne, endpointTwo, endpointThree, endpointFour]);
    if (
...
async function capture() {
  ...
  for (const viewport of viewports) {
    ...
    await page.screenshot({
      path: `${evidenceDir}/ux-ep-list-${viewport.name}.png`,
      fullPage: false,
    });
    ...
    if (viewport.width < 640) {
      await page
        .getByTestId("endpoint-mobile-card-21")
    ...
    await page.waitForTimeout(600);
    await page.screenshot({
      path: `${evidenceDir}/ux-ep-references-${viewport.name}.png`,
      fullPage: false,
    });
    ...
    await page.screenshot({
      path: `${evidenceDir}/ux-ep-delete-blocked-${viewport.name}.png`,
      fullPage: false,
    });
    ...
    await page.waitForTimeout(400);
    await page.screenshot({
      path: `${evidenceDir}/ux-ep-verify-${viewport.name}.png`,
      fullPage: false,
    });
    await page.keyboard.press("Escape");
