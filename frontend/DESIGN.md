...
- **Show values, not comparisons between them.** When both values are on screen, do not add a label saying whether they match (`名称相同`, `服务名称不同`, `与入口同名`) or that a setting equals its default (`均衡默认值`). A state that holds for every row in normal operation renders nothing; only its deviations are marked — `部分覆盖` and `不兼容` carry a badge, full coverage does not; a request's `未计价` sits beside its cost, `已计价` gets no column. Internal revision numbers such as a price template's `v2` do not appear in lists.
...
- **Model-config exit mapping** (models list): the targets cell projects the DIRECT `access_targets` rows in shared `(position, id)` order and shows the first two — Terminal Target rows as `服务 → 服务提供的模型名称`, Model Target rows prefixed `转发到其他模型 →` — with the count and the first row sharing one line and the second row sharing a line with the remainder, capped at two lines. When exactly one row would remain, render it instead of folding: folding the last one saves no height. Otherwise the remainder folds into a `还有 N 项，见详情` pointer to the model-config detail. The upstream identity is shown as-is, without a label saying whether it matches the client model name. Non-participating (`未参与`) rows carry text, never color alone. A missing endpoint or upstream identity renders a reasoned `—`; the owning model configuration's `model_id` is never substituted for it, and the summary never follows Model Target rows recursively.
- **Row actions**: faded out by default under a hovering pointer, faded in on row hover or focus, reachable by keyboard; the overflow-menu trigger stays visible (dimmed) so a row never looks actionless, and under `(hover:none)` every row action is visible. More than three actions collapse into an overflow menu. An action that cannot apply to a row is not rendered there as a disabled button, and a table whose rows have no applicable action drops the actions column.
...
- First-use tasks are connect a model, configure cost recording when desired, and connect a client. Existing backend readiness facts determine their states; configuration completion does not claim a successful provider request. Once configuration is complete, the usage page drops the first-use card while the selected window already holds a completed request.
