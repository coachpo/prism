...
- `release.sh` is the root-owned local monorepo release helper: it keeps `VERSION`, `backend/VERSION`, `frontend/VERSION`, and `frontend/package.json` aligned, runs a backend version-metadata check and the frontend build, and commits/tags/pushes one root-repo release.
...
- Keep repo-level version docs aligned with the root release helper keeping `VERSION`, `backend/VERSION`, `frontend/VERSION`, and `frontend/package.json` in sync inside this monorepo while running a backend version-metadata check and the frontend build.
