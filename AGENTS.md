...
- Model-owned context routing, overflow-promotion authoring, exact facade routing, and OpenAI sibling-operation translation were hard-deleted. OpenAI text attempts are native-only: the model accepted format and Terminal Target capability must both support the ingress operation. Keep ordinary operation-registered routing, Terminal Targets, Ban Policy strategies, flat final-target observability, and historical `operation_translation_mode` reads as the live contract.
