...
default_database_compose_project() {
    local branch normalized

    branch="$(git -C "$ROOT_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null || true)"
    normalized="$(printf '%s' "$branch" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9-]/-/g; s/^-*//; s/-*$//')"
    if [[ -z "$normalized" || "$normalized" == "head" ]]; then
        printf '%s\n' "prism"
        return
    fi
    printf 'prism-%s\n' "$normalized"
...
if [[ -z "${PRISM_DATABASE_COMPOSE_PROJECT:-}" ]]; then
    PRISM_DATABASE_COMPOSE_PROJECT="$(default_database_compose_project)"
fi
